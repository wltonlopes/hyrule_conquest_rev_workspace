...
SCRUB.HQ = function(Config)
{
	...
	this.basesManager = new SCRUB.BasesManager(this.Config);
	this.attackManager = new SCRUB.AttackManager(this.Config);
	this.buildManager = new SCRUB.BuildManager();
	this.defenseManager = new SCRUB.DefenseManager(this.Config);
	this.tradeManager = new SCRUB.TradeManager(this.Config);
	this.navalManager = new SCRUB.NavalManager(this.Config);
	this.researchManager = new SCRUB.ResearchManager(this.Config);
	this.diplomacyManager = new SCRUB.DiplomacyManager(this.Config);
	this.garrisonManager = new SCRUB.GarrisonManager(this.Config);
	this.victoryManager = new SCRUB.VictoryManager(this.Config);
	this.emergencyManager = new SCRUB.EmergencyManager(this.Config);
...
SCRUB.HQ.prototype.init = function(gameState, queues)
{
	this.territoryMap = SCRUB.createTerritoryMap(gameState);
	...
	this.borderMap = SCRUB.createBorderMap(gameState);
 ...
 */
SCRUB.HQ.prototype.postinit = function(gameState)
{
 ...
 */
SCRUB.HQ.prototype.getSeaBetweenIndices = function(gameState, index1, index2)
{
...
SCRUB.HQ.prototype.checkEvents = function(gameState, events)
{
	...
	for (let evt of events.OwnershipChanged)   // capture events
	{
		...
		{
			ent.setMetadata(PlayerID, "role", SCRUB.Worker.ROLE_TRADER);
			ent.setMetadata(PlayerID, "route", undefined);
		}
		if (ent.hasClass("Worker"))
		{
			ent.setMetadata(PlayerID, "role", SCRUB.Worker.ROLE_WORKER);
			ent.setMetadata(PlayerID, "subrole", SCRUB.Worker.SUBROLE_IDLE);
		}
		if (ent.hasClass("Ship"))
			SCRUB.setSeaAccess(gameState, ent);
	...
	for (let evt of events.TrainingFinished)
	{
		for (let entId of evt.entities)
		{
			...
			if (plan !== undefined && plan >= 0)
			{
				...
				if (!attack || attack.state !== SCRUB.AttackPlan.STATE_UNEXECUTED)
					ent.setMetadata(PlayerID, "plan", -1);
			}
		}
	}
...
SCRUB.HQ.prototype.handleNewBase = function(gameState)
{
...
SCRUB.HQ.prototype.checkPhaseRequirements = function(gameState, queues)
{
	...
	for (let entityReq of requirements)
	{
		...
		case "Town":
			    ...
			    !queues.militaryBuilding.hasQueuedUnits())
			{
				...
				{
					plan = new SCRUB.ConstructionPlan(gameState, "structures/{civ}/market", { "phaseUp": true });
				...
				{
					plan = new SCRUB.ConstructionPlan(gameState, "structures/{civ}/temple", { "phaseUp": true });
					queue = "economicBuilding";
					break;
				}
				...
				{
					plan = new SCRUB.ConstructionPlan(gameState, "structures/{civ}/forge", { "phaseUp": true });
		...
		default:
			...
			if (!queues[queue].hasQueuedUnits())
			{
				...
				if (structure && this.canBuild(gameState, structure))
					plan = new SCRUB.ConstructionPlan(gameState, structure, { "phaseUp": true });
			}
		}
...
SCRUB.HQ.prototype.OnPhaseUp = function(gameState, phase)
{
};

/** This code trains citizen workers, trying to keep close to a ratio of worker/soldiers */
SCRUB.HQ.prototype.trainMoreWorkers = function(gameState, queues)
{
	// default template
	...
	gameState.getOwnUnits().forEach(ent => {
		if (ent.getMetadata(PlayerID, "role") === SCRUB.Worker.ROLE_WORKER && ent.getMetadata(PlayerID, "plan") === undefined)
		{
			++numberOfWorkers;
	...
	gameState.getOwnTrainingFacilities().forEach(function(ent) {
		for (let item of ent.trainingQueue())
		{
			...
			if (item.metadata && item.metadata.role && item.metadata.role === SCRUB.Worker.ROLE_WORKER &&
	...
	if (this.attackManager.rushNumber < this.attackManager.maxRushes || this.attackManager.upcomingAttacks[SCRUB.AttackPlan.TYPE_RUSH].length)
		alpha = 0.7;
	...
	if (!template && templateDef)
		queues.villager.addPlan(new SCRUB.TrainingPlan(gameState, templateDef, { "role": SCRUB.Worker.ROLE_WORKER, "base": 0, "support": true }, size, size));
	else if (template)
		queues.citizenSoldier.addPlan(new SCRUB.TrainingPlan(gameState, template, { "role": SCRUB.Worker.ROLE_WORKER, "base": 0 }, size, size));
};

/** picks the best template based on parameters and classes */
SCRUB.HQ.prototype.findBestTrainableUnit = function(gameState, classes, requirements)
{
	let units;
	...
	units.sort((a, b) => {
		...
		for (let param of parameters)
		{
			if (param[0] == "strength")
			{
				aValue += SCRUB.getMaxStrength(a[1], gameState.ai.Config.debug, gameState.ai.Config.DamageTypeImportance) * param[1];
				bValue += SCRUB.getMaxStrength(b[1], gameState.ai.Config.debug, gameState.ai.Config.DamageTypeImportance) * param[1];
			}
			else if (param[0] == "siegeStrength")
			{
				aValue += SCRUB.getMaxStrength(a[1], gameState.ai.Config.debug, gameState.ai.Config.DamageTypeImportance, "Structure") * param[1];
				bValue += SCRUB.getMaxStrength(b[1], gameState.ai.Config.debug, gameState.ai.Config.DamageTypeImportance, "Structure") * param[1];
			}
 ...
 */
SCRUB.HQ.prototype.bulkPickWorkers = function(gameState, baseRef, number)
{
	return this.basesManager.bulkPickWorkers(gameState, baseRef, number);
};

SCRUB.HQ.prototype.getTotalResourceLevel = function(gameState, resources, proximity)
{
 ...
 */
SCRUB.HQ.prototype.GetCurrentGatherRates = function(gameState)
{
 ...
 */
SCRUB.HQ.prototype.GetWantedGatherRates = function(gameState)
{
 ...
 */
SCRUB.HQ.prototype.pickMostNeededResources = function(gameState, allowedResources = [])
{
 ...
 */
SCRUB.HQ.prototype.findEconomicCCLocation = function(gameState, template, resource, proximity, fromStrategic)
{
	...
	// obstruction map
	let obstructions = SCRUB.createObstructionMap(gameState, 0, template);
	let halfSize = 0;
	...
	for (let j = 0; j < this.territoryMap.length; ++j)
	{
		...
		else
		{
			...
			for (let cc of ccList)
			{
				...
				if (dist < minDist)
					minDist = dist;
				accessible = accessible || index == SCRUB.getLandAccess(gameState, cc.ent);
			}
			if (norm == 0)
				continue;
			...
			oversea = !accessible && dpList.some(dp => SCRUB.getLandAccess(gameState, dp.ent) == index);
			if (!oversea)
			{
		...
		if (this.borderMap.map[j] & SCRUB.fullBorder_Mask)	// disfavor the borders of the map
			norm *= 0.5;
 ...
 */
SCRUB.HQ.prototype.findStrategicCCLocation = function(gameState, template)
{
	...
	for (let j = 0; j < this.territoryMap.length; ++j)
	{
		...
		if (this.borderMap.map[j] & SCRUB.fullBorder_Mask)
			currentVal += 10000;
 ...
 */
SCRUB.HQ.prototype.findMarketLocation = function(gameState, template)
{
	...
	for (let j = 0; j < this.territoryMap.length; ++j)
	{
		...
		if (this.borderMap.map[j] & SCRUB.narrowFrontier_Mask)
			continue;
		...
		for (let market of markets)
		{
			...
			{
				if (SCRUB.getSeaAccess(gameState, market) != gameState.ai.accessibility.getAccessValue(pos, true))
					continue;
				gainMultiplier = traderTemplatesGains.navalGainMultiplier;
			}
			else if (SCRUB.getLandAccess(gameState, market) == index &&
				!SCRUB.isLineInsideEnemyTerritory(gameState, market.position(), pos))
 ...
 */
SCRUB.HQ.prototype.findDefensiveLocation = function(gameState, template)
{
	...
	for (let j = 0; j < this.territoryMap.length; ++j)
	{
		if (!wonderMode)
		{
			...
			if (!(this.borderMap.map[j] & SCRUB.fullFrontier_Mask))
				continue;
			if (this.borderMap.map[j] & SCRUB.largeFrontier_Mask && isTower)
				continue;
		}
...
SCRUB.HQ.prototype.buildTemple = function(gameState, queues)
{
	...
	if (this.canBuild(gameState, "structures/{civ}/temple_vesta"))
		...
		return;
	queues.economicBuilding.addPlan(new SCRUB.ConstructionPlan(gameState, templateName));
};

SCRUB.HQ.prototype.buildMarket = function(gameState, queues)
{
	if (gameState.getOwnEntitiesByClass("Market", true).hasEntities() ||
		!this.canBuild(gameState, "structures/{civ}/market"))
		return;
	...
	let plan = new SCRUB.ConstructionPlan(gameState, "structures/{civ}/market");
	plan.queueToReset = "economicBuilding";
	queues.economicBuilding.addPlan(plan);
};

/** Build a farmstead */
SCRUB.HQ.prototype.buildFarmstead = function(gameState, queues)
{
	...
	queues.economicBuilding.addPlan(new SCRUB.ConstructionPlan(gameState, "structures/{civ}/farmstead"));
};
 ...
 */
SCRUB.HQ.prototype.buildWonder = function(gameState, queues, force = false)
{
	...
	queues.wonder.addPlan(new SCRUB.ConstructionPlan(gameState, "structures/{civ}/wonder"));
};

/** Build a corral, and train animals there */
SCRUB.HQ.prototype.manageCorral = function(gameState, queues)
{
	                ...
	                nCorral < this.currentPhase && gameState.getPopulation() > 30 * nCorral)
	{
		...
		{
			queues.corral.addPlan(new SCRUB.ConstructionPlan(gameState, "structures/{civ}/corral"));
			return;
		}
		if (!nCorral)
			return;
	...
	for (let corral of gameState.getOwnEntitiesByClass("Corral", true).values())
	{
		...
		for (let trainable of trainables)
		{
			...
			if (count > nCorral)
				continue;
			queues.corral.addPlan(new SCRUB.TrainingPlan(gameState, trainable, { "trainer": corral.id() }));
			return;
		}
	}
};
 ...
 */
SCRUB.HQ.prototype.buildMoreHouses = function(gameState, queues)
{
	...
	{
		let plan = new SCRUB.ConstructionPlan(gameState, houseTemplateString);
...
SCRUB.HQ.prototype.checkBaseExpansion = function(gameState, queues)
{
...
SCRUB.HQ.prototype.buildNewBase = function(gameState, queues, resource)
{
	...
	queues.civilCentre.addPlan(new SCRUB.ConstructionPlan(gameState, template, { "base": -1, "resource": resource }));
	return true;
};

/** Deals with building fortresses and towers along our border with enemies. */
SCRUB.HQ.prototype.buildDefenses = function(gameState, queues)
{
	...
	if (!this.saveResources && (this.currentPhase > 2 || gameState.isResearching(gameState.getPhaseName(3))))
	{
		...
		if (this.canBuild(gameState, "structures/{civ}/fortress"))
		{
				...
				gameState.getOwnFoundationsByClass("Fortress").length < 2)
			{
				...
				let plan = new SCRUB.ConstructionPlan(gameState, "structures/{civ}/fortress");
	...
	if (this.Config.Military.numSentryTowers && this.currentPhase < 2 && this.canBuild(gameState, "structures/{civ}/sentry_tower"))
	{
		...
		if (numTowers < this.Config.Military.numSentryTowers && gameState.ai.elapsedTime > towerLapseTime + this.fortStartTime)
		{
			...
			queues.defenseBuilding.addPlan(new SCRUB.ConstructionPlan(gameState, "structures/{civ}/sentry_tower"));
		}
		return;
	}
		...
		gameState.getOwnFoundationsByClass("Tower").length < 3)
	{
		...
		let plan = new SCRUB.ConstructionPlan(gameState, "structures/{civ}/defense_tower");
		plan.queueToReset = "defenseBuilding";
		queues.defenseBuilding.addPlan(plan);
	}
};

SCRUB.HQ.prototype.buildForge = function(gameState, queues)
{
	...
	if (this.canBuild(gameState, "structures/{civ}/forge"))
		queues.militaryBuilding.addPlan(new SCRUB.ConstructionPlan(gameState, "structures/{civ}/forge"));
};
 ...
 */
SCRUB.HQ.prototype.constructTrainingBuildings = function(gameState, queues)
{
	    ...
	    this.phasing == 2 && gameState.getOwnStructures().filter(API3.Filters.byClass("Village")).length < 5)
	{
		...
		if (numBarracks + numRanges == 0)
		{
			...
			if (template)
			{
				...
				let plan = new SCRUB.ConstructionPlan(gameState, template, { "militaryBase": true });
		...
		{
			queues.militaryBuilding.addPlan(new SCRUB.ConstructionPlan(gameState, kennelTemplate, { "militaryBase": true }));
			return;
		}
		...
		if (numBarracks + numRanges == 1 && this.getAccountedPopulation(gameState) > this.Config.Military.popForBarracks2)
		{
			...
			if (template)
			{
				queues.militaryBuilding.addPlan(new SCRUB.ConstructionPlan(gameState, template, { "militaryBase": true }));
				return;
			}
		}
	...
	{
		queues.militaryBuilding.addPlan(new SCRUB.ConstructionPlan(gameState, "structures/{civ}/elephant_stable", { "militaryBase": true }));
		return;
	}

	if (this.canBuild(gameState, "structures/{civ}/arsenal") && !gameState.getOwnEntitiesByClass("Arsenal", true).hasEntities())
	{
		queues.militaryBuilding.addPlan(new SCRUB.ConstructionPlan(gameState, "structures/{civ}/arsenal", { "militaryBase": true }));
		return;
	}
	...
	if (!nAdvanced || nAdvanced < this.bAdvanced.length && this.getAccountedPopulation(gameState) > 110)
	{
		for (let advanced of this.bAdvanced)
		{
			...
			if (template.hasDefensiveFire() || template.trainableEntities(civ))
				queues.militaryBuilding.addPlan(new SCRUB.ConstructionPlan(gameState, advanced, { "militaryBase": true }));
			else	// not a military building, but still use this queue
				queues.militaryBuilding.addPlan(new SCRUB.ConstructionPlan(gameState, advanced));
			return;
		}
	}
};
 ...
 */
SCRUB.HQ.prototype.findBestBaseForMilitary = function(gameState)
{
	...
	for (let cce of ccEnts)
	{
		...
		if (enemyFound && !gameState.isPlayerEnemy(cce.owner()))
			continue;
		let access = SCRUB.getLandAccess(gameState, cce);
		let isEnemy = gameState.isPlayerEnemy(cce.owner());
		for (let cc of ccEnts)
		{
			if (cc.owner() != PlayerID)
				continue;
			if (SCRUB.getLandAccess(gameState, cc) != access)
				continue;
 ...
 */
SCRUB.HQ.prototype.trainEmergencyUnits = function(gameState, positions)
{
	...
	const metadata = { "role": SCRUB.Worker.ROLE_WORKER, "base": nearestAnchor.getMetadata(PlayerID, "base"), "plan": -1, "trainer": nearestAnchor.id() };
	if (autogarrison)
		metadata.garrisonType = SCRUB.GarrisonManager.TYPE_PROTECTION;
	gameState.ai.queues.emergency.addPlan(new SCRUB.TrainingPlan(gameState, templateFound[0], metadata, 1, 1));
	return true;
};

SCRUB.HQ.prototype.canBuild = function(gameState, structure)
{
...
SCRUB.HQ.prototype.updateTerritories = function(gameState)
{
	...
	for (let j = 0; j < this.territoryMap.length; ++j)
	{
		if (this.borderMap.map[j] & SCRUB.outside_Mask)
			continue;
		if (this.borderMap.map[j] & SCRUB.fullFrontier_Mask)
			this.borderMap.map[j] &= ~SCRUB.fullFrontier_Mask;	// reset the frontier
		...
		else
		{
			...
			for (let a of around)
			{
				...
				if (jz < 0 || jz >= width)
					continue;
				if (this.borderMap.map[jx+width*jz] & SCRUB.outside_Mask)
					continue;
				...
				{
					this.borderMap.map[j] |= SCRUB.narrowFrontier_Mask;
					break;
				}
			...
			if (onFrontier && !(this.borderMap.map[j] & SCRUB.narrowFrontier_Mask))
				this.borderMap.map[j] |= SCRUB.largeFrontier_Mask;
 ...
 */
SCRUB.HQ.prototype.getBaseByID = function(baseID)
{
 ...
 */
SCRUB.HQ.prototype.numActiveBases = function()
{
	return this.basesManager.numActiveBases();
};

SCRUB.HQ.prototype.hasActiveBase = function()
{
	return this.basesManager.hasActiveBase();
};

SCRUB.HQ.prototype.numPotentialBases = function()
{
	return this.basesManager.numPotentialBases();
};

SCRUB.HQ.prototype.hasPotentialBase = function()
{
	return this.basesManager.hasPotentialBase();
};

SCRUB.HQ.prototype.isDangerousLocation = function(gameState, pos, radius)
{
...
SCRUB.HQ.prototype.isNearInvadingArmy = function(pos)
{
...
SCRUB.HQ.prototype.isUnderEnemyFire = function(gameState, pos, radius = 0)
{
...
SCRUB.HQ.prototype.updateCaptureStrength = function(gameState)
{
	...
	for (let ent of gameState.getOwnUnits().values())
	{
		...
		if (!this.capturableTargets.has(targetId))
			this.capturableTargets.set(targetId, {
				"strength": ent.captureStrength() * SCRUB.getAttackBonus(ent, target, "Capture"),
				"ents": new Set([ent.id()])
			});
		else
		{
			let capturableTarget = this.capturableTargets.get(target.id());
			capturableTarget.strength += ent.captureStrength() * SCRUB.getAttackBonus(ent, target, "Capture");
	...
	for (let [targetId, capturableTarget] of this.capturableTargets)
	{
		...
		for (let entId of capturableTarget.ents)
		{
			...
			if (allowCapture === undefined)
				allowCapture = SCRUB.allowCapture(gameState, ent, target);
			let orderData = ent.unitAIOrderData();
 ...
 */
SCRUB.HQ.prototype.isDefendable = function(ent)
{
 ...
 */
SCRUB.HQ.prototype.getAccountedPopulation = function(gameState)
{
 ...
 */
SCRUB.HQ.prototype.getAccountedWorkers = function(gameState)
{
	if (this.turnCache.accountedWorkers == undefined)
	{
		let workers = gameState.getOwnEntitiesByRole(SCRUB.Worker.ROLE_WORKER, true).length;
		for (let ent of gameState.getOwnTrainingFacilities().values())
		{
			for (let item of ent.trainingQueue())
			{
...
SCRUB.HQ.prototype.baseManagers = function()
{
 ...
 */
SCRUB.HQ.prototype.baseAtIndex = function(territoryIndex)
{
 ...
 */
SCRUB.HQ.prototype.update = function(gameState, queues, events)
{
	...
	*/
	...
	if (gameState.ai.playedTurn % 3 == 0)
	{
		...
		if (this.Config.difficulty > SCRUB.DIFFICULTY_SANDBOX)
	...
	if (this.Config.difficulty > SCRUB.DIFFICULTY_SANDBOX && (this.hasActiveBase() || !this.canBuildUnits))
...
SCRUB.HQ.prototype.Serialize = function()
{
	let properties = {
		"phasing": this.phasing,
...
SCRUB.HQ.prototype.Deserialize = function(gameState, data)
{
