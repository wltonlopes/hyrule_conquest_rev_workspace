...
SCRUB.ScrubBot = function(settings)
{
	...
	this.Config = new SCRUB.Config(settings.difficulty, settings.behavior);

	this.savedEvents = {};
};

SCRUB.ScrubBot.prototype = Object.create(API3.BaseAI.prototype);

SCRUB.ScrubBot.prototype.CustomInit = function(gameState)
{
	...
	else
	{
		...
		for (let i in this.Config.priorities)
			this.queues[i] = new SCRUB.Queue();

		this.queueManager = new SCRUB.QueueManager(this.Config, this.queues);
...
SCRUB.ScrubBot.prototype.OnUpdate = function(sharedScript)
{
	...
	{
		if (i == "AIMetadata")   // not used inside scrub
			continue;
	...
	{
		Engine.ProfileStart("ScrubBot bot (player " + this.player +")");
...
SCRUB.ScrubBot.prototype.Serialize = function()
{
	let savedEvents = {};
	for (let key in this.savedEvents)
	{
...
SCRUB.ScrubBot.prototype.Deserialize = function(data, sharedScript)
{
