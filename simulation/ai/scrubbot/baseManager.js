...
SCRUB.BaseManager = function(gameState, basesManager)
{
	...
	this.neededDefenders = this.Config.difficulty > SCRUB.DIFFICULTY_EASY ? 3 + 2*(this.Config.difficulty - 3) : 0;
...
SCRUB.BaseManager.STATE_WITH_ANCHOR = "anchored";

/**
 * New base with a foundation anchor.
 */
SCRUB.BaseManager.STATE_UNCONSTRUCTED = "unconstructed";

/**
 * Captured base with an anchor.
 */
SCRUB.BaseManager.STATE_CAPTURED = "captured";

/**
 * Anchorless base, currently with dock.
 */
SCRUB.BaseManager.STATE_ANCHORLESS = "anchorless";

SCRUB.BaseManager.prototype.init = function(gameState, state)
{
	if (state === SCRUB.BaseManager.STATE_UNCONSTRUCTED)
		this.constructing = true;
	else if (state !== SCRUB.BaseManager.STATE_CAPTURED)
		this.neededDefenders = 0;
	this.workerObject = new SCRUB.Worker(this);
	// entitycollections
	this.units = gameState.getOwnUnits().filter(API3.Filters.byMetadata(PlayerID, "base", this.ID));
	this.workers = this.units.filter(API3.Filters.byMetadata(PlayerID, "role", SCRUB.Worker.ROLE_WORKER));
...
SCRUB.BaseManager.prototype.reset = function(gameState, state)
{
	if (state === SCRUB.BaseManager.STATE_UNCONSTRUCTED)
		this.constructing = true;
	else
		this.constructing = false;
	if (state !== SCRUB.BaseManager.STATE_CAPTURED || this.Config.difficulty < SCRUB.DIFFICULTY_MEDIUM)
		this.neededDefenders = 0;
	else
		this.neededDefenders = 3 + 2 * (this.Config.difficulty - 3);
};

SCRUB.BaseManager.prototype.assignEntity = function(gameState, ent)
{
...
SCRUB.BaseManager.prototype.setAnchor = function(gameState, anchorEntity)
{
	if (!anchorEntity.hasClass("CivCentre"))
		API3.warn("Error: SCRUB base " + this.ID + " has been assigned " + ent.templateName() + " as anchor.");
	else
	{
	...
	this.accessIndex = SCRUB.getLandAccess(gameState, anchorEntity);
	return true;
};

/* we lost our anchor. Let's reassign our units and buildings */
SCRUB.BaseManager.prototype.anchorLost = function(gameState, ent)
{
	this.anchor = undefined;
	this.anchorId = undefined;
	this.neededDefenders = 0;
...
SCRUB.BaseManager.prototype.setAnchorlessEntity = function(gameState, ent)
{
	if (!this.buildings.hasEntities())
	{
		if (!SCRUB.getBuiltEntity(gameState, ent).resourceDropsiteTypes())
			API3.warn("Error: SCRUB base " + this.ID + " has been assigned " + ent.templateName() + " as origin.");
		this.accessIndex = SCRUB.getLandAccess(gameState, ent);
	}
	else if (this.accessIndex !== SCRUB.getLandAccess(gameState, ent))
		API3.warn(" Error: SCRUB base " + this.ID + " with access " + this.accessIndex +
		          " has been assigned " + ent.templateName() + " with access" + SCRUB.getLandAccess(gameState, ent));
 ...
 */
SCRUB.BaseManager.prototype.assignResourceToDropsite = function(gameState, dropsite)
{
	...
	if (this.ID == this.basesManager.baselessBase().ID)
		accessIndex = SCRUB.getLandAccess(gameState, dropsite);
	...
	for (let type of dropsite.resourceDropsiteTypes())
	{
		...
		resources.forEach(function(supply)
		{
			...
			if (SCRUB.getLandAccess(gameState, supply) != accessIndex)
				return;
...
SCRUB.BaseManager.prototype.removeFromAssignedDropsite = function(ent)
{
...
SCRUB.BaseManager.prototype.removeDropsite = function(gameState, ent)
{
	if (!ent.id())
		return;
 ...
 */
SCRUB.BaseManager.prototype.findBestDropsiteAndLocation = function(gameState, resource)
{
	let bestResult = {
		"quality": 0,
		"pos": [0, 0]
 ...
 */
SCRUB.BaseManager.prototype.findBestDropsiteLocation = function(gameState, resource, templateName)
{
	...
	let obstructions = SCRUB.createObstructionMap(gameState, this.accessIndex, template);
	...
	for (const foundation of gameState.getOwnFoundations().toEntityArray())
		if (SCRUB.getBuiltEntity(gameState, foundation).isResourceDropsite(resource))
...
SCRUB.BaseManager.prototype.getResourceLevel = function(gameState, type, distances = ["nearby", "medium", "faraway"])
{
	let count = 0;
	let check = {};
...
SCRUB.BaseManager.prototype.checkResourceLevels = function(gameState, queues)
{
		...
		if (type == "food")
		{
			...
			if (gameState.ai.HQ.canBuild(gameState, "structures/{civ}/field"))	// let's see if we need to add new farms.
			{
				...
				if (numFarms + numQueue == 0)	// starting game, rely on fruits as long as we have enough of them
				{
					if (count < 600)
					{
						queues.field.addPlan(new SCRUB.ConstructionPlan(gameState, "structures/{civ}/field", { "favoredBase": this.ID }));
				...
				else if (!gameState.ai.HQ.maxFields || numFarms + numQueue < gameState.ai.HQ.maxFields)
				{
					...
					queues.corral.addPlan(new SCRUB.ConstructionPlan(gameState, "structures/{civ}/corral", { "favoredBase": this.ID }));
				continue;
			}
		...
		if (total > 150 || total > 60 && type != "wood")
		{
			...
			if (ratio > 0.15)
			{
				...
				if (newDP.quality > 50 && gameState.ai.HQ.canBuild(gameState, newDP.templateName))
					queues.dropsites.addPlan(new SCRUB.ConstructionPlan(gameState, newDP.templateName, { "base": this.ID, "type": type }, newDP.pos));
...
SCRUB.BaseManager.prototype.addGatherRates = function(gameState, currentRates)
{
	for (let res in currentRates)
	{
		...
		if (res == "food")
		{
			this.workersBySubrole(gameState, SCRUB.Worker.SUBROLE_HUNTER).forEach(ent => {
				if (ent.isIdle() || !ent.position())
					return;
				let gRate = ent.currentGatherRate();
				if (gRate)
			...
			this.workersBySubrole(gameState, SCRUB.Worker.SUBROLE_FISHER).forEach(ent => {
				if (ent.isIdle() || !ent.position())
					return;
				let gRate = ent.currentGatherRate();
				if (gRate)
...
SCRUB.BaseManager.prototype.assignRolelessUnits = function(gameState, roleless)
{
	if (!roleless)
 ...
 */
SCRUB.BaseManager.prototype.setWorkersIdleByPriority = function(gameState)
{
 ...
 */
SCRUB.BaseManager.prototype.switchGatherer = function(gameState, from, to, number)
{
	let num = number;
	let only;
...
SCRUB.BaseManager.prototype.reassignIdleWorkers = function(gameState, idleWorkers)
{
	// Search for idle workers, and tell them to gather resources based on demand
	if (!idleWorkers)
	{
		const filter = API3.Filters.byMetadata(PlayerID, "subrole", SCRUB.Worker.SUBROLE_IDLE);
	...
	for (let ent of idleWorkers)
	{
			...
			else if (ent.isGatherer())
			{
				...
				for (let needed of mostNeeded)
				{
					...
					if (needed.type != "food" && this.basesManager.isResourceExhausted(needed.type))
						continue;
					ent.setMetadata(PlayerID, "subrole", SCRUB.Worker.SUBROLE_GATHERER);
		...
		else if (SCRUB.isFastMoving(ent) && ent.canGather("food") && ent.canAttackClass("Animal"))
			ent.setMetadata(PlayerID, "subrole", SCRUB.Worker.SUBROLE_HUNTER);
		else if (ent.hasClass("FishingBoat"))
			ent.setMetadata(PlayerID, "subrole", SCRUB.Worker.SUBROLE_FISHER);
	}
};

SCRUB.BaseManager.prototype.workersBySubrole = function(gameState, subrole)
{
...
SCRUB.BaseManager.prototype.gatherersByType = function(gameState, type)
{
	return gameState.updatingCollection("workers-gathering-" + type +"-base-" + this.ID, API3.Filters.byMetadata(PlayerID, "gather-type", type), this.workersBySubrole(gameState, SCRUB.Worker.SUBROLE_GATHERER));
};
 ...
 */
SCRUB.BaseManager.prototype.pickBuilders = function(gameState, workers, number)
{
	...
	availableWorkers.sort((a, b) => {
		let vala = 0;
		let valb = 0;
		if (a.getMetadata(PlayerID, "subrole") === SCRUB.Worker.SUBROLE_BUILDER)
			vala = 100;
		if (b.getMetadata(PlayerID, "subrole") === SCRUB.Worker.SUBROLE_BUILDER)
			valb = 100;
		if (a.getMetadata(PlayerID, "subrole") === SCRUB.Worker.SUBROLE_IDLE)
			vala = -50;
		if (b.getMetadata(PlayerID, "subrole") === SCRUB.Worker.SUBROLE_IDLE)
			valb = -50;
	...
	for (let i = 0; i < needed; ++i)
	{
		...
		availableWorkers[i].setMetadata(PlayerID, "subrole", SCRUB.Worker.SUBROLE_IDLE);
 ...
 */
SCRUB.BaseManager.prototype.assignToFoundations = function(gameState, noRepair)
{
	...
	const builderWorkers = this.workersBySubrole(gameState, SCRUB.Worker.SUBROLE_BUILDER);
	...
	if (workers.length < 3)
	{
		...
		if (fromOtherBase)
		{
			...
			fromOtherBase.forEach(worker => {
				...
				worker.setMetadata(PlayerID, "subrole", SCRUB.Worker.SUBROLE_BUILDER);
	...
	for (let target of foundations.values())
	{
		...
		if (SCRUB.isNotWorthBuilding(gameState, target))
			continue;
		...
		let nonBuilderWorkers = workers.filter(function(ent) {
			if (ent.getMetadata(PlayerID, "subrole") === SCRUB.Worker.SUBROLE_BUILDER)
				return false;
			if (!ent.position())
				return false;
			if (ent.getMetadata(PlayerID, "plan") == -2 || ent.getMetadata(PlayerID, "plan") == -3)
		...
		while (assigned < targetNB && builderTot < maxTotalBuilders && current < nonBuilderTot)
		{
			...
			ent.stopMoving();
			ent.setMetadata(PlayerID, "subrole", SCRUB.Worker.SUBROLE_BUILDER);
			ent.setMetadata(PlayerID, "target-foundation", target.id());
		}
	}
...
SCRUB.BaseManager.prototype.update = function(gameState, queues, events)
{
	if (this.ID == this.basesManager.baselessBase().ID)
	{
		...
		if (this.basesManager.hasActiveBase())
		{
			...
			{
				let bestBase = SCRUB.getBestBase(gameState, ent);
			...
			for (let ent of this.buildings.values())
			{
				...
				if (!bestBase)
				{
					if (ent.hasClass("Dock"))
						API3.warn("SCRUB: dock in 'noBase' baseManager. It may be useful to do an anchorless base for " + ent.templateName());
					continue;
				}
	...
	if (!this.anchor)   // This anchor has been destroyed, but the base may still be usable
	{
		if (!this.buildings.hasEntities())
		{
			...
			for (let ent of this.units.values())
			{
				let base = SCRUB.getBestBase(gameState, ent, false, this.ID);
		...
		for (let ent of this.buildings.values())
		{
			if (!ent.position())
				continue;
			let base = SCRUB.getBestBase(gameState, ent);
			if (base.anchor)
				reassignedBase = base;
			break;
		}
...
SCRUB.BaseManager.prototype.AddTCGatherer = function(supplyID)
{
	return this.basesManager.AddTCGatherer(supplyID);
};

SCRUB.BaseManager.prototype.RemoveTCGatherer = function(supplyID)
{
	this.basesManager.RemoveTCGatherer(supplyID);
};

SCRUB.BaseManager.prototype.GetTCGatherer = function(supplyID)
{
	return this.basesManager.GetTCGatherer(supplyID);
};

SCRUB.BaseManager.prototype.Serialize = function()
{
	return {
		"ID": this.ID,
...
SCRUB.BaseManager.prototype.Deserialize = function(gameState, data)
{
	for (let key in data)
		this[key] = data[key];
