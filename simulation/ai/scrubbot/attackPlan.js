 ...
 */
SCRUB.AttackPlan = function(gameState, Config, uniqueID, type = SCRUB.AttackPlan.TYPE_DEFAULT, data)
{
	this.Config = Config;
	this.name = uniqueID;
	this.type = type;
	this.state = SCRUB.AttackPlan.STATE_UNEXECUTED;
	...
	for (const base of gameState.ai.HQ.baseManagers())
	{
		if (!base.anchor || !base.anchor.position())
			continue;
		let access = SCRUB.getLandAccess(gameState, base.anchor);
		if (!rallyPoint)
		{
	...
	if (!rallyPoint)	// no base ?  take the position of any of our entities
	{
		for (let ent of gameState.getOwnEntities().values())
		{
			if (!ent.position())
				continue;
			let access = SCRUB.getLandAccess(gameState, ent);
	...
	if (gameState.ai.HQ.navalMap)
	{
		for (let structure of gameState.getEnemyStructures().values())
		{
			...
			if (!structure.position())
				continue;
			let access = SCRUB.getLandAccess(gameState, structure);
			...
			else if (!this.overseas)
			{
				...
				if (!sea)
				{
					if (this.target)
					{
						API3.warn("SCRUB: " + this.type + " " + this.name + " has an inaccessible target " +
	...
	if (type === SCRUB.AttackPlan.TYPE_RUSH)
	{
		priority = 150;
		this.unitStat.Infantry = { "priority": 2, "minSize": 15, "targetSize": 20, "batchSize": 5, "classes": ["Infantry"],
			"interests": [["strength", 1], ["costsResource", 0.5, "stone"], ["costsResource", 0.6, "metal"]] };
		this.unitStat.FastMoving = { "priority": 3, "minSize": 15, "targetSize": 20, "batchSize": 5, "classes": ["FastMoving"],
			"interests": [["strength", 1]] };
		if (data && data.targetSize)
			this.unitStat.Infantry.targetSize = data.targetSize;
		this.neededShips = 1;
	}
	else if (type === SCRUB.AttackPlan.TYPE_RAID)
	{
		priority = 250;
		this.unitStat.FastMoving = { "priority": 4, "minSize": 15, "targetSize": 20, "batchSize": 5, "classes": ["FastMoving+CitizenSoldier"],
			"interests": [ ["strength", 1] ] };
		this.unitStat.Infantry = { "priority": 3, "minSize": 10, "targetSize": 15, "batchSize": 5, "classes": ["Ranged"],
			"interests": [ ["strength", 1] ] };
		this.unitStat.Beetle = { "priority": 2, "minSize": 5, "targetSize": 10, "batchSize": 5, "classes": ["Beetle"],
			"interests": [ ["strength", 1] ] };
		this.unitStat.Creature = { "priority": 1, "minSize": 1, "targetSize": 5, "batchSize": 2, "classes": ["Creature"],
			"interests": [ ["strength", 1] ] };
		this.neededShips = 3;
	}
	else
	{
		priority = 70;
		this.unitStat.RangedInfantry = { "priority": 1, "minSize": 5, "targetSize": 20, "batchSize": 5, "classes": ["Infantry+Ranged"],
			"interests": [["canGather", 1], ["strength", 1.6], ["costsResource", 0.3, "stone"], ["costsResource", 0.3, "metal"]] };
		this.unitStat.MeleeInfantry = { "priority": 1, "minSize": 5, "targetSize": 20, "batchSize": 5, "classes": ["Infantry+Melee"],
			"interests": [["canGather", 1], ["strength", 1.6], ["costsResource", 0.3, "stone"], ["costsResource", 0.3, "metal"]] };
	...
	if (this.Config.difficulty < SCRUB.DIFFICULTY_EASY)
	{
		priority *= 0.4;
		variation *= 0.2;
	}
	else if (this.Config.difficulty < SCRUB.DIFFICULTY_MEDIUM)
	{
		priority *= 0.8;
		variation *= 0.6;
	}
	...
	this.siegeState = SCRUB.AttackPlan.SIEGE_NOT_TESTED;
...
SCRUB.AttackPlan.PREPARATION_FAILED = 0;
SCRUB.AttackPlan.PREPARATION_KEEP_GOING = 1;
SCRUB.AttackPlan.PREPARATION_START = 2;

SCRUB.AttackPlan.SIEGE_NOT_TESTED = 0;
SCRUB.AttackPlan.SIEGE_NO_TRAINER = 1;

/**
 * Siege added in build orders
 */
SCRUB.AttackPlan.SIEGE_ADDED = 2;

SCRUB.AttackPlan.STATE_UNEXECUTED = "unexecuted";
SCRUB.AttackPlan.STATE_COMPLETING = "completing";
SCRUB.AttackPlan.STATE_ARRIVED = "arrived";

SCRUB.AttackPlan.TYPE_DEFAULT = "Attack";
SCRUB.AttackPlan.TYPE_RAID = "Raid";
SCRUB.AttackPlan.TYPE_RUSH = "Rush";

SCRUB.AttackPlan.prototype.init = function(gameState)
{
...
SCRUB.AttackPlan.prototype.getName = function()
{
	return this.name;
};

SCRUB.AttackPlan.prototype.getType = function()
{
	return this.type;
};

SCRUB.AttackPlan.prototype.isStarted = function()
{
	return this.state !== SCRUB.AttackPlan.STATE_UNEXECUTED && this.state !== SCRUB.AttackPlan.STATE_COMPLETING;
};

SCRUB.AttackPlan.prototype.isPaused = function()
{
	return this.paused;
};

SCRUB.AttackPlan.prototype.setPaused = function(boolValue)
{
 ...
 */
SCRUB.AttackPlan.prototype.canStart = function()
{
	if (!this.canBuildUnits)
		return true;
...
SCRUB.AttackPlan.prototype.mustStart = function()
{
	if (this.isPaused())
		return false;
	...
	if (MinReachedEverywhere)
		return this.type === SCRUB.AttackPlan.TYPE_RAID && this.target && this.target.foundationProgress() &&
		                                             this.target.foundationProgress() > 50;
	return false;
};

SCRUB.AttackPlan.prototype.forceStart = function()
{
	for (let unitCat in this.unitStat)
	{
		let Unit = this.unitStat[unitCat];
...
SCRUB.AttackPlan.prototype.emptyQueues = function()
{
	this.queue.empty();
...
SCRUB.AttackPlan.prototype.removeQueues = function(gameState)
{
...
SCRUB.AttackPlan.prototype.addBuildOrder = function(gameState, name, unitStats, resetQueue)
{
	if (!this.isStarted())
	{
...
SCRUB.AttackPlan.prototype.addSiegeUnits = function(gameState)
{
	if (this.siegeState === SCRUB.AttackPlan.SIEGE_ADDED || this.state !== SCRUB.AttackPlan.STATE_UNEXECUTED)
		return false;

	let civ = gameState.getPlayerCiv();
	const classes = [["Siege+Melee"], ["Creature+Siege"], ["Siege+Ranged"]];
	...
	{
		this.siegeState = SCRUB.AttackPlan.SIEGE_NO_TRAINER;
		return false;
	}
	...
	this.siegeState = SCRUB.AttackPlan.SIEGE_ADDED;
	let targetSize;
	if (this.Config.difficulty < SCRUB.DIFFICULTY_MEDIUM)
		targetSize = this.type === SCRUB.AttackPlan.TYPE_R_ATTACK ? Math.max(this.Config.difficulty, 1) : Math.max(this.Config.difficulty - 1, 0);
	else
		targetSize = this.type === SCRUB.AttackPlan.TYPE_RUSH ? this.Config.difficulty + 1 : this.Config.difficulty - 1;
	targetSize = Math.max(Math.round(this.Config.popScaling * targetSize), this.type === SCRUB.AttackPlan.TYPE_RUSH ? 1 : 0);
	if (!targetSize)
		return true;
	// no minsize as we don't want the plan to fail at the last minute though.
	let stat = { "priority": 1, "minSize": 0, "targetSize": targetSize, "batchSize": Math.min(targetSize, 8),
...
SCRUB.AttackPlan.prototype.updatePreparation = function(gameState)
{
	...
	if (this.state === SCRUB.AttackPlan.STATE_COMPLETING)
	{
		...
		else
		{
			...
			if (this.waitingForTransport())
				return SCRUB.AttackPlan.PREPARATION_KEEP_GOING;
			...
			if (gameState.ai.elapsedTime < this.maxCompletingTime && this.hasForceOrder())
				return SCRUB.AttackPlan.PREPARATION_KEEP_GOING;
			return SCRUB.AttackPlan.PREPARATION_START;
		}
	}
	...
	if (this.type !== SCRUB.AttackPlan.TYPE_RAID || !this.forced)    // Forced Raids have special purposes (as relic capture)
		this.assignUnits(gameState);
	if (this.type !== SCRUB.AttackPlan.TYPE_RAID && gameState.ai.HQ.attackManager.getAttackInPreparation(SCRUB.AttackPlan.TYPE_RAID) !== undefined)
	...
	{
		let lengthMin = 5;
		if (gameState.getPopulationMax() < 100)
			lengthMin -= Math.floor(8 * (100 - gameState.getPopulationMax()) / 100);
		...
		else	// Abort the plan so that its units will be reassigned to other plans.
		{
			if (this.Config.debug > 1)
			{
				...
				API3.warn(" attacks upcoming: raid " + am.upcomingAttacks[SCRUB.AttackPlan.TYPE_RAID].length +
					  " rush " + am.upcomingAttacks[SCRUB.AttackPlan.TYPE_RUSH].length +
					  " attack " + am.upcomingAttacks[SCRUB.AttackPlan.TYPE_DEFAULT].length);
				API3.warn(" attacks started: raid " + am.startedAttacks[SCRUB.AttackPlan.TYPE_RAID].length +
					  " rush " + am.startedAttacks[SCRUB.AttackPlan.TYPE_RUSH].length +
					  " attack " + am.startedAttacks[SCRUB.AttackPlan.TYPE_DEFAULT].length);
			}
			return SCRUB.AttackPlan.PREPARATION_FAILED;
		}
	}
	...
	else
	{
		if (this.canBuildUnits)
		{
			...
			if (this.siegeState === SCRUB.AttackPlan.SIEGE_NOT_TESTED ||
				this.siegeState === SCRUB.AttackPlan.SIEGE_NO_TRAINER && gameState.ai.playedTurn % 5 == 0)
			...
			if (!this.buildOrders.length)
				return SCRUB.AttackPlan.PREPARATION_FAILED;	// will abort the plan
		}
		return SCRUB.AttackPlan.PREPARATION_KEEP_GOING;
	}

	// if we're here, it means we must start
	this.state = SCRUB.AttackPlan.STATE_COMPLETING;
	...
	if (this.type === SCRUB.AttackPlan.TYPE_RAID)
		this.maxCompletingTime = this.forced ? 0 : gameState.ai.elapsedTime + 20;
	else
	{
		if (this.type === SCRUB.AttackPlan.TYPE_RUSH || this.forced)
	...
	for (let ent of this.unitCollection.values())
	{
		...
		ent.setMetadata(PlayerID, "role", SCRUB.Worker.ROLE_ATTACK);
		ent.setMetadata(PlayerID, "subrole", SCRUB.Worker.SUBROLE_COMPLETING);
		let queued = false;
		if (ent.resourceCarrying() && ent.resourceCarrying().length)
			queued = SCRUB.returnResources(gameState, ent);
		let index = SCRUB.getLandAccess(gameState, ent);
		if (index == rallyIndex)
...
SCRUB.AttackPlan.prototype.trainMoreUnits = function(gameState)
{
	...
	if (firstOrder[0] < firstOrder[3].targetSize)
	{
		...
		if (queue.length() <= 5)
		{
			...
			else
			{
				...
				data.role = gameState.getTemplate(template).hasClass("CitizenSoldier") ? SCRUB.Worker.ROLE_WORKER : SCRUB.Worker.ROLE_ATTACK;
				let trainingPlan = new SCRUB.TrainingPlan(gameState, template, data, max, max);
...
SCRUB.AttackPlan.prototype.assignUnits = function(gameState)
{
	let plan = this.name;
	let added = false;
	...
	if (this.type === SCRUB.AttackPlan.TYPE_RAID)
	{
		...
		for (let ent of gameState.getOwnUnits().values())
		{
			if (!ent.hasClass("FastMoving") || !this.isAvailableUnit(gameState, ent))
				continue;
			if (!ent.hasClass("Beetle") || !this.isAvailableUnit(gameState, ent))
				continue;
			if (!ent.hasClass("Ranged") || !this.isAvailableUnit(gameState, ent))
				continue;
			if (num++ < 2)
				continue;
			ent.setMetadata(PlayerID, "plan", plan);
	...
	let num = 0;
	const numbase = {};
	let keep = this.type !== SCRUB.AttackPlan.TYPE_RUSH ?
		6 + 4 * gameState.getNumPlayerEnemies() + 8 * this.Config.personality.defensive : 8;
	keep = Math.round(this.Config.popScaling * keep);
	for (const ent of gameState.getOwnEntitiesByRole(SCRUB.Worker.ROLE_WORKER, true).values())
	{
		...
		{
			API3.warn("SCRUB problem ent without base ");
			SCRUB.dumpEntity(ent);
			continue;
		}
		if (num++ < keep || numbase[baseID] < 5)
			continue;
		if (this.type !== SCRUB.AttackPlan.TYPE_RUSH && ent.getMetadata(PlayerID, "subrole") !== SCRUB.Worker.SUBROLE_IDLE)
			continue;
		ent.setMetadata(PlayerID, "plan", plan);
		this.unitCollection.updateEnt(ent);
		added = true;
...
SCRUB.AttackPlan.prototype.isAvailableUnit = function(gameState, ent)
{
	if (!ent.position())
		return false;
...
SCRUB.AttackPlan.prototype.reassignFastUnit = function(gameState)
{
	for (let ent of this.unitCollection.values())
	{
		...
		if (!ent.hasClasses(["FastMoving", "CitizenSoldier"]))
			continue;
		const raid = gameState.ai.HQ.attackManager.getAttackInPreparation(SCRUB.AttackPlan.TYPE_RAID);
...
SCRUB.AttackPlan.prototype.chooseTarget = function(gameState)
{
	...
	let targetIndex = SCRUB.getLandAccess(gameState, this.target);
 ...
 */
SCRUB.AttackPlan.prototype.getNearestTarget = function(gameState, position, sameLand)
{
	...
	else
	{
		...
		else if (this.type === SCRUB.AttackPlan.TYPE_RUSH || this.type === SCRUB.AttackPlan.TYPE_DEFAULT)
		{
	...
	for (let ent of targets.values())
	{
		...
		if (this.type !== SCRUB.AttackPlan.TYPE_RUSH && this.type !== SCRUB.AttackPlan.TYPE_RAID && ent.hasClass("Field"))
			dist += 100000;
		if (dist < minDist)
		{
			minDist = dist;
 ...
 */
SCRUB.AttackPlan.prototype.defaultTargetFinder = function(gameState, playerEnemy)
{
...
SCRUB.AttackPlan.prototype.isValidTarget = function(ent)
{
	if (!ent.position())
		return false;
	if (this.sameLand && SCRUB.getLandAccess(this.gameState, ent) != this.sameLand)
		return false;
...
SCRUB.AttackPlan.prototype.rushTargetFinder = function(gameState, playerEnemy)
{
	let targets = new API3.EntityCollection(gameState.sharedScript);
	let buildings;
	...
	if (!targets.hasEntities() && this.type === SCRUB.AttackPlan.TYPE_RUSH && playerEnemy)
...
SCRUB.AttackPlan.prototype.raidTargetFinder = function(gameState)
{
	let targets = new API3.EntityCollection(gameState.sharedScript);
 ...
 */
SCRUB.AttackPlan.prototype.checkTargetObstruction = function(gameState, target, position)
{
	if (SCRUB.getLandAccess(gameState, target) != gameState.ai.accessibility.getAccessValue(position))
		return target;
...
SCRUB.AttackPlan.prototype.getPathToTarget = function(gameState, fixedRallyPoint = false)
{
	let startAccess = gameState.ai.accessibility.getAccessValue(this.rallyPoint);
	let endAccess = SCRUB.getLandAccess(gameState, this.target);
...
SCRUB.AttackPlan.prototype.setRallyPoint = function(gameState)
{
 ...
 */
SCRUB.AttackPlan.prototype.StartAttack = function(gameState)
{
	if (this.Config.debug > 1)
	...
	for (let ent of this.unitCollection.values())
	{
		ent.setMetadata(PlayerID, "subrole", SCRUB.Worker.SUBROLE_WALKING);
	...
	let targetAccess = SCRUB.getLandAccess(gameState, this.target);
...
SCRUB.AttackPlan.prototype.update = function(gameState, events)
{
	...
	if (this.state === SCRUB.AttackPlan.STATE_ARRIVED)
	{
		...
		this.unitCollection.forEach(ent => {
			ent.stopMoving();
			ent.setMetadata(PlayerID, "subrole", SCRUB.Worker.SUBROLE_ATTACKING);
		});
		if (this.type === SCRUB.AttackPlan.TYPE_RUSH)   // try to find a better target for rush
		{
	...
	if (this.state == "")
	{
		...
		for (let evt of events.Attacked)
		{
			...
			{	// if our siege units are attacked, we'll send some units to deal with enemies.
				...
				{
					if (SCRUB.isSiegeUnit(ent))	// needed as mauryan elephants are not filtered out
						continue;

					let allowCapture = SCRUB.allowCapture(gameState, ent, attacker);
				...
				if (SCRUB.isSiegeUnit(attacker) && attacker.hasClass("Melee") && ourUnit.hasClass("Melee") && ourUnit.canAttackTarget(attacker, SCRUB.allowCapture(gameState, ourUnit, attacker)))
				{
					ourUnit.attack(attacker.id(), SCRUB.allowCapture(gameState, ourUnit, attacker));
			...
			else
			{
				...
				else if (SCRUB.isSiegeUnit(attacker))
				...
				else
				{
					...
					{
						let allowCapture = SCRUB.allowCapture(gameState, ent, attacker);
						if (SCRUB.isSiegeUnit(ent) || !ent.canAttackTarget(attacker, allowCapture))
							continue;
					...
					let allowCapture = SCRUB.allowCapture(gameState, ourUnit, attacker);
		...
		for (let ent of this.unitCollection.values())
		{
			...
			{
				if (SCRUB.isSiegeUnit(target) || target.hasClass("Hero"))
		...
		if (this.type === SCRUB.AttackPlan.TYPE_RUSH)
		...
		{
			targetClassesUnit.avoid = targetClassesUnit.avoid.concat("House", "Storehouse", "Farmstead", "Field", "Forge", "Market");
			targetClassesSiege.avoid = targetClassesSiege.avoid.concat("House", "Storehouse", "Farmstead", "Field", "Forge", "Market");
		}
		...
		for (let check = 0; check < lgth; check++)
		{
			...
			let siegeUnit = SCRUB.isSiegeUnit(ent);
			if (ent.isIdle())
				needsUpdate = true;
			...
			if (this.isBlocked)
			{
				...
				range += 30;
			range *= range;
			let entAccess = SCRUB.getLandAccess(gameState, ent);
			// Checking for gates if we're a siege unit.
			if (siegeUnit)
			{
				let mStruct = enemyStructures.filter(enemy => {
					if (!enemy.position() || !ent.canAttackTarget(enemy, SCRUB.allowCapture(gameState, ent, enemy)))
						return false;
					if (API3.SquareVectorDistance(enemy.position(), ent.position()) > range)
						return false;
					if (enemy.foundationProgress() == 0)
						return false;
					if (SCRUB.getLandAccess(gameState, enemy) != entAccess)
						return false;
					return true;
				}).toEntityArray();
				if (mStruct.length)
				{
					...
					if (mStruct[0].hasClass("Gate"))
						ent.attack(mStruct[0].id(), SCRUB.allowCapture(gameState, ent, mStruct[0]));
					else
					{
						let rand = randIntExclusive(0, mStruct.length * 0.2);
						ent.attack(mStruct[rand].id(), SCRUB.allowCapture(gameState, ent, mStruct[rand]));
					}
				}
				else
				{
			...
			else
			{
				...
				if (mUnit.length)
				{
					...
					ent.attack(mUnit[rand].id(), SCRUB.allowCapture(gameState, ent, mUnit[rand]));
				}
					...
					else if (needsUpdate)  // really nothing   let's try to help our nearest unit
					{
						...
						this.unitCollection.forEach(unit => {
							...
							if (dist > distmin)
								return;
							distmin = dist;
							if (!ent.canAttackTarget(target, SCRUB.allowCapture(gameState, ent, target)))
								return;
							attacker = target;
						});
						if (attacker)
							ent.attack(attacker.id(), SCRUB.allowCapture(gameState, ent, attacker));
					}
				}
			}
		}
...
SCRUB.AttackPlan.prototype.UpdateTransporting = function(gameState, events)
{
	let done = true;
	for (let ent of this.unitCollection.values())
	{
	...
	if (done)
	{
		this.state = SCRUB.AttackPlan.STATE_ARRIVED;
		return;
	}
...
SCRUB.AttackPlan.prototype.UpdateWalking = function(gameState, events)
{
...
SCRUB.AttackPlan.prototype.UpdateTarget = function(gameState)
{
	...
	if (this.target && this.target.hasClass("Unit"))
	{
		...
		if (!this.targetPos)
		{
			let holder = SCRUB.getHolder(gameState, this.target);
			...
			for (let attackType in attackManager.startedAttacks)
			{
				for (let attack of attackManager.startedAttacks[attackType])
				{
					...
					if (!attack.target || !gameState.getEntityById(attack.target.id()) ||
						...
						continue;
					if (accessIndex != SCRUB.getLandAccess(gameState, attack.target))
						continue;
...
/** reset any units */
SCRUB.AttackPlan.prototype.Abort = function(gameState)
{
	...
	if (this.unitCollection.hasEntities())
	{
		...
		if (this.isStarted())
		{
			...
			for (const base of gameState.ai.HQ.baseManagers())
			{
				if (!base.anchor || !base.anchor.position())
					continue;
				if (SCRUB.getLandAccess(gameState, base.anchor) != access)
					continue;
		...
		for (let ent of this.unitCollection.values())
		{
			if (ent.getMetadata(PlayerID, "role") === SCRUB.Worker.ROLE_ATTACK)
				ent.stopMoving();
			if (rallyPoint)
...
SCRUB.AttackPlan.prototype.removeUnit = function(ent, update)
{
	if (ent.getMetadata(PlayerID, "role") === SCRUB.Worker.ROLE_ATTACK)
	{
		if (ent.hasClass("CitizenSoldier"))
			ent.setMetadata(PlayerID, "role", SCRUB.Worker.ROLE_WORKER);
		else
...
SCRUB.AttackPlan.prototype.checkEvents = function(gameState, events)
{
	for (let evt of events.EntityRenamed)
	{
		if (!this.target || this.target.id() != evt.entity)
			continue;
		if (this.type === SCRUB.AttackPlan.TYPE_RAID && !this.isStarted())
			this.target = undefined;
		else
	...
	if (!this.overseas || this.state !== SCRUB.AttackPlan.STATE_UNEXECUTED)
		return;
...
SCRUB.AttackPlan.prototype.waitingForTransport = function()
{
	for (let ent of this.unitCollection.values())
		if (ent.getMetadata(PlayerID, "transport") !== undefined)
			return true;
...
SCRUB.AttackPlan.prototype.hasSiegeUnits = function()
{
	for (let ent of this.unitCollection.values())
		if (SCRUB.isSiegeUnit(ent))
			return true;
	return false;
};

SCRUB.AttackPlan.prototype.hasForceOrder = function(data, value)
{
	for (let ent of this.unitCollection.values())
	{
 ...
 */
SCRUB.AttackPlan.prototype.getAttackAccess = function(gameState)
{
	for (let ent of this.unitCollection.filterNearest(this.position, 1).values())
		return SCRUB.getLandAccess(gameState, ent);

	return 0;
};

SCRUB.AttackPlan.prototype.debugAttack = function()
{
...
SCRUB.AttackPlan.prototype.Serialize = function()
{
	let properties = {
		"name": this.name,
		"type": this.type,
...
SCRUB.AttackPlan.prototype.Deserialize = function(gameState, data)
{
