 ...
 */
SCRUB.DefenseArmy = function(gameState, foeEntities, type)
{
 ...
 */
SCRUB.DefenseArmy.prototype.addFoe = function(gameState, enemyId, force)
{
 ...
 */
SCRUB.DefenseArmy.prototype.removeFoe = function(gameState, enemyId, enemyEntity)
{
 ...
 */
SCRUB.DefenseArmy.prototype.addOwn = function(gameState, id, force)
{
	...
	if (subrole === undefined || subrole !== SCRUB.Worker.SUBROLE_DEFENDER)
		ent.setMetadata(PlayerID, "formerSubrole", subrole);
	ent.setMetadata(PlayerID, "subrole", SCRUB.Worker.SUBROLE_DEFENDER);
	return true;
};

SCRUB.DefenseArmy.prototype.removeOwn = function(gameState, id, Entity)
{
	...
	if (ent.getMetadata(PlayerID, "transport") !== undefined)
	{
		...
		if (plan && plan.state === SCRUB.TransportPlan.BOARDING && ent.position())
 ...
 */
SCRUB.DefenseArmy.prototype.clear = function(gameState)
{
	...
	else
	{
		...
		for (let struct of gameState.getAllyStructures().values())
		{
			...
			if (!pos || !gameState.isPlayerMutualAlly(gameState.ai.HQ.territoryMap.getOwner(pos)))
				continue;
			if (SCRUB.getLandAccess(gameState, struct) !== armyAccess)
				continue;
...
SCRUB.DefenseArmy.prototype.assignUnit = function(gameState, entID)
{
	...
	for (let id of this.foeEntities)
	{
		...
		if (!ent.canAttackTarget(eEnt, SCRUB.allowCapture(gameState, ent, eEnt)))
			continue;
		...
		if (this.assignedAgainst[id].length > 10 ||
			this.assignedAgainst[id].length > 8 && !eEnt.hasClass("Hero") && !SCRUB.isSiegeUnit(eEnt))
			continue;
	...
	let ownIndex = SCRUB.getLandAccess(gameState, ent);
	...
	if (ownIndex == foeIndex || ent.hasClass("Ship"))
	{
		...
		ent.attack(idFoe, SCRUB.allowCapture(gameState, ent, foeEnt), queued);
	}
	else
...
SCRUB.DefenseArmy.prototype.getType = function()
{
	return this.type;
};

SCRUB.DefenseArmy.prototype.getState = function()
{
 ...
 */
SCRUB.DefenseArmy.prototype.merge = function(gameState, otherArmy)
{
...
SCRUB.DefenseArmy.prototype.needsDefenders = function(gameState)
{
	let defenseRatio;
...
SCRUB.DefenseArmy.prototype.recalculatePosition = function(gameState, force)
{
...
SCRUB.DefenseArmy.prototype.recalculateStrengths = function(gameState)
{
	this.ownStrength = 0;
	this.foeStrength = 0;
...
SCRUB.DefenseArmy.prototype.evaluateStrength = function(ent, isOwn, remove)
{
	if (!ent)
		return;
	...
	if (ent.hasClass("Structure"))
	{
		...
		entStrength = SCRUB.getMaxStrength(ent, this.Config.debug, this.Config.DamageTypeImportance);
...
SCRUB.DefenseArmy.prototype.checkEvents = function(gameState, events)
{
...
SCRUB.DefenseArmy.prototype.update = function(gameState)
{
	for (let entId of this.ownEntities)
	{
		let ent = gameState.getEntityById(entId);
		...
		else if (orderData.length && orderData[0].target && orderData[0].attackType && orderData[0].attackType === "Capture")
		{
			...
			if (target && !SCRUB.allowCapture(gameState, ent, target))
...
SCRUB.DefenseArmy.prototype.Serialize = function()
{
	return {
		"ID": this.ID,
		"type": this.type,
...
SCRUB.DefenseArmy.prototype.Deserialize = function(data)
{
	for (let key in data)
		this[key] = data[key];
};
