...
SCRUB.BasesManager = function(Config)
{
	this.Config = Config;
...
SCRUB.BasesManager.prototype.init = function(gameState)
{
	...
	this.noBase = new SCRUB.BaseManager(gameState, this);
	this.noBase.init(gameState, SCRUB.BaseManager.STATE_WITH_ANCHOR);
	...
	for (const cc of gameState.getOwnStructures().filter(API3.Filters.byClass("CivCentre")).values())
		if (cc.foundationProgress() === undefined)
			this.createBase(gameState, cc, SCRUB.BaseManager.STATE_WITH_ANCHOR);
		else
			this.createBase(gameState, cc, SCRUB.BaseManager.STATE_UNCONSTRUCTED);
};

/**
 * Initialization needed after deserialization (only called when deserialising).
 */
SCRUB.BasesManager.prototype.postinit = function(gameState)
{
 ...
 */
SCRUB.BasesManager.prototype.createBase = function(gameState, ent, type = SCRUB.BaseManager.STATE_WITH_ANCHOR)
{
	const access = SCRUB.getLandAccess(gameState, ent);
	let newbase;
	for (const base of this.baseManagers)
	{
		if (base.accessIndex != access)
			continue;
		if (type !== SCRUB.BaseManager.STATE_ANCHORLESS && base.anchor)
			continue;
		if (type !== SCRUB.BaseManager.STATE_ANCHORLESS)
		{
	...
	if (!newbase)
	{
		newbase = new SCRUB.BaseManager(gameState, this);
...
SCRUB.BasesManager.prototype.checkEvents = function(gameState, events)
{
	let addBase = false;
	...
	for (const evt of events.EntityRenamed)
	{
		...
		if (ent.getMetadata(PlayerID, "role") === SCRUB.Worker.ROLE_WORKER)
		{
	...
	for (const evt of events.Create)
	{
		...
		if (ent.getMetadata(PlayerID, "base") == -1)	// Standard base around a cc
		{
			...
			const newbase = this.createBase(gameState, ent, SCRUB.BaseManager.STATE_UNCONSTRUCTED);
		...
		{
			const newbase = this.createBase(gameState, ent, SCRUB.BaseManager.STATE_ANCHORLESS);
			// Let's get a few units from other bases there to build this.
	...
	for (const evt of events.OwnershipChanged)
	{
		...
		{
			SCRUB.getBestBase(gameState, ent).assignEntity(gameState, ent);
			continue;
		}
		if (ent.hasClass("CivCentre"))   // build a new base around it
		{
			...
			if (ent.foundationProgress() !== undefined)
				newbase = this.createBase(gameState, ent, SCRUB.BaseManager.STATE_UNCONSTRUCTED);
			else
			{
				newbase = this.createBase(gameState, ent, SCRUB.BaseManager.STATE_CAPTURED);
				addBase = true;
			}
		...
		else
		{
			...
			if (!ent.decaying() && ent.resourceDropsiteTypes())
				base = this.createBase(gameState, ent, SCRUB.BaseManager.STATE_ANCHORLESS);
			else
				base = SCRUB.getBestBase(gameState, ent) || this.noBase;
	...
	for (const evt of events.TrainingFinished)
	{
		for (const entId of evt.entities)
		{
			...
			if (ent.getMetadata(PlayerID, "role") === SCRUB.Worker.ROLE_WORKER)
			{
				...
				if (ent.getMetadata(PlayerID, "base") === undefined)
				{
					base = SCRUB.getBestBase(gameState, ent);
					base.assignEntity(gameState, ent);
				}
				else
			...
			else if (ent.resourceSupplyType() && ent.position())
			{
				...
				{
					if (!dropsite.position() || SCRUB.getLandAccess(gameState, dropsite) != access)
						continue;
 ...
 */
SCRUB.BasesManager.prototype.bulkPickWorkers = function(gameState, baseRef, number)
{
 ...
 */
SCRUB.BasesManager.prototype.getTotalResourceLevel = function(gameState, resources = Resources.GetCodes(), proximity = ["nearby", "medium"])
{
	const total = {};
 ...
 */
SCRUB.BasesManager.prototype.GetCurrentGatherRates = function(gameState)
{
...
SCRUB.BasesManager.prototype.AddTCGatherer = function(supplyID)
{
...
SCRUB.BasesManager.prototype.RemoveTCGatherer = function(supplyID)
{
...
SCRUB.BasesManager.prototype.GetTCGatherer = function(supplyID)
{
	if (this.turnCache.resourceGatherer && this.turnCache.resourceGatherer[supplyID])
...
SCRUB.BasesManager.prototype.AddTCResGatherer = function(resource)
{
...
SCRUB.BasesManager.prototype.GetTCResGatherer = function(resource)
{
	const check = "resourceGatherer-" + resource;
	if (this.turnCache[check])
 ...
 */
SCRUB.BasesManager.prototype.isResourceExhausted = function(resource)
{
 ...
 */
SCRUB.BasesManager.prototype.numActiveBases = function()
{
...
SCRUB.BasesManager.prototype.hasActiveBase = function()
{
	return !!this.numActiveBases();
};

SCRUB.BasesManager.prototype.numPotentialBases = function()
{
	if (!this.turnCache.base)
		this.updateBaseCache();
...
SCRUB.BasesManager.prototype.hasPotentialBase = function()
{
 ...
 */
SCRUB.BasesManager.prototype.updateBaseCache = function()
{
...
SCRUB.BasesManager.prototype.resetBaseCache = function()
{
	this.turnCache.base = undefined;
};

SCRUB.BasesManager.prototype.baselessBase = function()
{
	return this.noBase;
};
 ...
 */
SCRUB.BasesManager.prototype.getBaseByID = function(baseID)
{
 ...
 */
SCRUB.BasesManager.prototype.assignGatherers = function()
{
	for (const base of this.baseManagers)
 ...
 */
SCRUB.BasesManager.prototype.assignEntity = function(gameState, ent, territoryIndex)
{
	let bestbase;
	for (const base of this.baseManagers)
	{
	...
	if (!bestbase)	// entity outside our territory
	{
		if (ent.hasClass("Structure") && !ent.decaying() && ent.resourceDropsiteTypes())
			bestbase = this.createBase(gameState, ent, SCRUB.BaseManager.STATE_ANCHORLESS);
		else
			bestbase = SCRUB.getBestBase(gameState, ent) || this.noBase;
 ...
 */
SCRUB.BasesManager.prototype.addGatherRates = function(gameState, rates)
{
	for (const base of this.baseManagers)
 ...
 */
SCRUB.BasesManager.prototype.baseAtIndex = function(territoryIndex)
{
 ...
 * @param {number} territoryIndex
 */
SCRUB.BasesManager.prototype.removeBaseFromTerritoryIndex = function(territoryIndex)
{
 ...
 */
SCRUB.BasesManager.prototype.addTerritoryIndexToBase = function(gameState, territoryIndex, passabilityMap)
{
...
SCRUB.BasesManager.prototype.reassignTerritories = function(deletedBase, territoryMap)
{
	...
	for (let j = 0; j < territoryMap.length; ++j)
	{
		...
		{
			API3.warn("SCRUB reassignTerritories: should never happen");
 ...
 */
SCRUB.BasesManager.prototype.update = function(gameState, queues, events)
{
...
SCRUB.BasesManager.prototype.Serialize = function()
{
	const properties = {
...
SCRUB.BasesManager.prototype.Deserialize = function(gameState, data)
{
	...
	for (const basedata of data.baseManagers)
	{
		...
		const newbase = new SCRUB.BaseManager(gameState, this);
		newbase.Deserialize(gameState, basedata);
		newbase.init(gameState, SCRUB.BaseManager.STATE_WITH_ANCHOR);
		newbase.Deserialize(gameState, basedata);
		this.baseManagers.push(newbase);
	}
};
