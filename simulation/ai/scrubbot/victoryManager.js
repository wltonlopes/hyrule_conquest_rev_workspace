...
SCRUB.VictoryManager = function(Config)
{
	this.Config = Config;
 ...
 */
SCRUB.VictoryManager.prototype.init = function(gameState)
{
 ...
 */
SCRUB.VictoryManager.prototype.checkEvents = function(gameState, events)
{
	if (gameState.getVictoryConditions().has("wonder"))
	{
	...
	if (gameState.getVictoryConditions().has("regicide"))
	{
		...
		for (let evt of events.TrainingFinished)
			for (let entId of evt.entities)
			{
				...
				if (ent && ent.isOwn(PlayerID) && ent.getMetadata(PlayerID, "role") === SCRUB.Worker.ROLE_CRITICAL_ENT_HEALER)
	...
	for (let evt of events.UnGarrison)
	{
		...
		if ((ent.getMetadata(PlayerID, "role") === SCRUB.Worker.ROLE_CRITICAL_ENT_HEALER ||
		     ent.getMetadata(PlayerID, "role") === SCRUB.Worker.ROLE_CRITICAL_ENT_GUARD) && !this.guardEnts.get(evt.entity))
		{
...
SCRUB.VictoryManager.prototype.removeCriticalEnt = function(gameState, criticalEntId)
{
 ...
 */
SCRUB.VictoryManager.prototype.manageCriticalEntHealers = function(gameState, queues)
{
	...
	for (let data of this.criticalEnts.values())
	{
		...
		queues.healer.addPlan(new SCRUB.TrainingPlan(gameState, template, { "role": SCRUB.Worker.ROLE_CRITICAL_ENT_HEALER, "base": 0 }, 1, 1));
		return;
	}
};
 ...
 */
SCRUB.VictoryManager.prototype.manageCriticalEntGuards = function(gameState)
{
	let numWorkers = gameState.getOwnEntitiesByRole(SCRUB.Worker.ROLE_WORKER, true).length;
	if (numWorkers < 20)
	{
		for (let data of this.criticalEnts.values())
		{
			for (let guardId of data.guards.keys())
			{
				...
				if (!guardEnt || !guardEnt.hasClass("CitizenSoldier") ||
				    guardEnt.getMetadata(PlayerID, "role") !== SCRUB.Worker.ROLE_CRITICAL_ENT_GUARD)
					continue;
...
SCRUB.VictoryManager.prototype.tryAssignMilitaryGuard = function(gameState, guardEnt, criticalEnt, checkForSameAccess)
{
	if (guardEnt.getMetadata(PlayerID, "plan") !== undefined ||
	    ...
	    SCRUB.getLandAccess(gameState, criticalEnt) != SCRUB.getLandAccess(gameState, guardEnt)))
		return false;
	...
	guardEnt.setMetadata(PlayerID, "role", SCRUB.Worker.ROLE_CRITICAL_ENT_GUARD);
	return true;
};

SCRUB.VictoryManager.prototype.pickCriticalEntRetreatLocation = function(gameState, criticalEnt, emergency)
{
	...
	let accessIndex = SCRUB.getLandAccess(gameState, criticalEnt);
	let bestBase = SCRUB.getBestBase(gameState, criticalEnt, true);
 ...
 */
SCRUB.VictoryManager.prototype.assignGuardToCriticalEnt = function(gameState, guardEnt, criticalEntId)
{
	...
	let guardEntAccess = SCRUB.getLandAccess(gameState, guardEnt);
	let criticalEntAccess = SCRUB.getLandAccess(gameState, criticalEnt);
	if (guardEntAccess == criticalEntAccess)
	{
		let queued = SCRUB.returnResources(gameState, guardEnt);
		guardEnt.guard(criticalEnt, queued);
		const guardRole = guardEnt.getMetadata(PlayerID, "role") === SCRUB.Worker.ROLE_CRITICAL_ENT_HEALER ? "healer" : "guard";
...
SCRUB.VictoryManager.prototype.resetCaptureGaiaRelic = function(gameState)
{
...
SCRUB.VictoryManager.prototype.update = function(gameState, events, queues)
{
 ...
 */
SCRUB.VictoryManager.prototype.captureGaiaRelic = function(gameState, relic)
{
	...
	for (let plan of plans)
	{
		...
		for (let ent of attack.unitCollection.values())
			capture += ent.captureStrength() * SCRUB.getAttackBonus(ent, relic, "Capture");
	}
	// No need to make a new attack if already enough units
	if (capture > sumCapturePoints / 50)
		return;
	let relicPosition = relic.position();
	let access = SCRUB.getLandAccess(gameState, relic);
	let units = gameState.getOwnUnits().filter(ent => {
		...
		{
			let attack = gameState.ai.HQ.attackManager.getPlan(plan);
			if (attack && (attack.state !== SCRUB.AttackPlan.STATE_UNEXECUTED || attack.type === SCRUB.AttackPlan.TYPE_RAID))
				return false;
		}
...
SCRUB.VictoryManager.prototype.abortCaptureGaiaRelic = function(gameState, relicId)
{
...
SCRUB.VictoryManager.prototype.Serialize = function()
{
	return {
...
SCRUB.VictoryManager.prototype.Deserialize = function(data)
{
	for (let key in data)
		this[key] = data[key];
};
