...
SCRUB.Worker.ROLE_ATTACK = "attack";
SCRUB.Worker.ROLE_TRADER = "trader";
SCRUB.Worker.ROLE_SWITCH_TO_TRADER = "switchToTrader";
SCRUB.Worker.ROLE_WORKER = "worker";
SCRUB.Worker.ROLE_CRITICAL_ENT_GUARD = "criticalEntGuard";
SCRUB.Worker.ROLE_CRITICAL_ENT_HEALER = "criticalEntHealer";

SCRUB.Worker.SUBROLE_DEFENDER = "defender";
SCRUB.Worker.SUBROLE_IDLE = "idle";
SCRUB.Worker.SUBROLE_BUILDER = "builder";
SCRUB.Worker.SUBROLE_COMPLETING = "completing";
SCRUB.Worker.SUBROLE_WALKING = "walking";
SCRUB.Worker.SUBROLE_ATTACKING = "attacking";
SCRUB.Worker.SUBROLE_GATHERER = "gatherer";
SCRUB.Worker.SUBROLE_HUNTER = "hunter";
SCRUB.Worker.SUBROLE_FISHER = "fisher";
SCRUB.Worker.SUBROLE_GARRISONING = "garrisoning";

SCRUB.Worker.prototype.update = function(gameState, ent)
{
	...
	if (ent.getMetadata(PlayerID, "transport") !== undefined)
	{
		...
		if (subrole === SCRUB.Worker.SUBROLE_BUILDER && ent.getMetadata(PlayerID, "target-foundation") !== undefined)
		{
			...
			if (!target && plan && plan.state === SCRUB.TransportPlan.BOARDING && ent.position())
				plan.removeUnit(gameState, ent);
		}
		// and gatherer if there are no more dropsite accessible in the base the ent is going to
		if (subrole === SCRUB.Worker.SUBROLE_GATHERER || subrole === SCRUB.Worker.SUBROLE_HUNTER)
		{
			let plan = gameState.ai.HQ.navalManager.getPlan(ent.getMetadata(PlayerID, "transport"));
			if (plan.state === SCRUB.TransportPlan.BOARDING && ent.position())
			{
				let hasDropsite = false;
				let gatherType = ent.getMetadata(PlayerID, "gather-type") || "wood";
				for (let structure of gameState.getOwnStructures().values())
				{
					if (SCRUB.getLandAccess(gameState, structure) != plan.endIndex)
						continue;
					let resourceDropsiteTypes = SCRUB.getBuiltEntity(gameState, structure).resourceDropsiteTypes();
				...
				if (!hasDropsite)
				{
					...
					{
						if (!unit.position() || SCRUB.getLandAccess(gameState, unit) != plan.endIndex)
							continue;
	...
	this.entAccess = SCRUB.getLandAccess(gameState, ent);
	...
	if ((subrole === SCRUB.Worker.SUBROLE_HUNTER || subrole === SCRUB.Worker.SUBROLE_GATHERER) &&
	     ...
	     unitAIState == "INDIVIDUAL.COMBAT.APPROACHING"))
	{
		...
		if (unitAIState == "INDIVIDUAL.COMBAT.APPROACHING" && ent.unitAIOrderData().length)
		{
			...
			if (orderData && orderData.target)
			{
				...
				if (target && target.resourceSupplyType() && target.resourceSupplyType().generic == "wood")
				{
					...
					{
						let distanceSquare = SCRUB.isFastMoving(ent) ? 90000 : 30000;
						let targetAccess = SCRUB.getLandAccess(gameState, target);
						let woodDropsites = gameState.playerData.hasSharedDropsites ?
						                    gameState.getAnyDropsites("wood") : gameState.getOwnDropsites("wood");
						let hasWoodDropsiteWithinDistance = false;
						for (let dropsite of woodDropsites.values())
						{
							...
							if (owner != PlayerID && (!dropsite.isSharedDropsite() || !gameState.isPlayerMutualAlly(owner)))
								continue;
							if (targetAccess != SCRUB.getLandAccess(gameState, dropsite))
								continue;
							if (API3.SquareVectorDistance(target.position(), dropsite.position()) < distanceSquare)
							{
								hasWoodDropsiteWithinDistance = true;
								break;
							}
						}
						if (!hasWoodDropsiteWithinDistance)
						{
							 if (this.retryWorking(gameState, subrole))
								return;
							ent.stopMoving();
	...
	{
		if (subrole === SCRUB.Worker.SUBROLE_FISHER)
			...
			!ent.getMetadata(PlayerID, "PartOfArmy"))
		{
			...
			{
				let target = gameState.getEntityById(orderData.target);
				if (target && (!target.position() || SCRUB.getLandAccess(gameState, target) != this.entAccess))
				{
					if (this.retryWorking(gameState, subrole))
						return;
					ent.stopMoving();
			...
			!ent.getMetadata(PlayerID, "PartOfArmy"))
		{
			...
			if (orderData && orderData.target && orderData.attackType && orderData.attackType == "Capture")
			{
				...
				if (target && target.owner() > 0 && !gameState.isPlayerAlly(target.owner()))
					ent.attack(orderData.target, SCRUB.allowCapture(gameState, ent, target));
			}
		}
		return;
	}
	...
	if (subrole === SCRUB.Worker.SUBROLE_GATHERER)
	{
		if (ent.isIdle())
		{
			...
			else if (!SCRUB.returnResources(gameState, ent))     // try to deposit resources
			{
		...
		else if (unitAIStateOrder == "GATHER")
		{
			...
			if (ent.unitAIOrderData().length)
			{
				...
				if (supply && !supply.hasClass(["Field"]) &&
			...
			if (unitAIState == "INDIVIDUAL.GATHER.RETURNINGRESOURCE.APPROACHING")
			{
				if (gameState.ai.playedTurn % 10 == 0)
				{
					...
					if (dropsite && dropsite.position() && this.entAccess != SCRUB.getLandAccess(gameState, dropsite))
						SCRUB.returnResources(gameState, this.ent);
				}
	...
	else if (subrole === SCRUB.Worker.SUBROLE_BUILDER)
	{
		...
		{
			let goalAccess = SCRUB.getLandAccess(gameState, target);
			let queued = SCRUB.returnResources(gameState, ent);
	...
	else if (subrole === SCRUB.Worker.SUBROLE_HUNTER)
	{
		...
		else	// Perform some sanity checks
		{
			if (unitAIStateOrder == "GATHER")
			{
				...
				else if (unitAIState == "INDIVIDUAL.GATHER.RETURNINGRESOURCE.APPROACHING")
				{
					...
					let dropsite = gameState.getEntityById(ent.unitAIOrderData()[0].target);
					if (dropsite && dropsite.position() && this.entAccess != SCRUB.getLandAccess(gameState, dropsite))
						SCRUB.returnResources(gameState, ent);
				}
			}
		}
	}
	else if (subrole === SCRUB.Worker.SUBROLE_FISHER)
	{
		if (ent.isIdle())
			this.startFishing(gameState);
...
SCRUB.Worker.prototype.retryWorking = function(gameState, subrole)
{
	switch (subrole)
	{
	case SCRUB.Worker.SUBROLE_GATHERER:
		return this.startGathering(gameState);
	case SCRUB.Worker.SUBROLE_HUNTER:
		return this.startHunting(gameState);
	case SCRUB.Worker.SUBROLE_FISHER:
		return this.startFishing(gameState);
	case SCRUB.Worker.SUBROLE_BUILDER:
...
SCRUB.Worker.prototype.startBuilding = function(gameState)
{
	let target = gameState.getEntityById(this.ent.getMetadata(PlayerID, "target-foundation"));
	if (!target || target.foundationProgress() === undefined && target.needsRepair() == false)
		return false;
	if (SCRUB.getLandAccess(gameState, target) != this.entAccess)
		return false;
	this.ent.repair(target, target.hasClass("House"));  // autocontinue=true for houses
	return true;
};

SCRUB.Worker.prototype.startGathering = function(gameState)
{
	// First look for possible treasure if any
	if (SCRUB.gatherTreasure(gameState, this.ent))
		return true;
	...
	const findSupply = function(worker, supplies) {
		...
		for (let i = 0; i < supplies.length; ++i)
		{
			...
			if (SCRUB.IsSupplyFull(gameState, supplies[i].ent))
				continue;
			...
			if (supplies[i].ent.resourceSupplyType().specific != "wood" && nbGatherers > 0 &&
	...
	if (this.baseAccess == this.entAccess)
	{
		...
		// --> for wood, try to gather from fields if any, otherwise build one if any
		if (resource == "wood")
		{
	...
	if (resource == "wood")	// --> for wood, try to gather from fields if any, otherwise build one if any
	{
		for (const base of gameState.ai.HQ.baseManagers())
		{
			if (base.ID == this.baseID)
	...
	let shouldBuild = this.ent.isBuilder() && foundations.some(function(foundation) {
		if (!foundation || SCRUB.getLandAccess(gameState, foundation) != this.entAccess)
			return false;
		...
		if (structure.resourceDropsiteTypes() && structure.resourceDropsiteTypes().indexOf(resource) != -1)
		{
			...
			this.ent.setMetadata(PlayerID, "subrole", SCRUB.Worker.SUBROLE_BUILDER);
	...
	shouldBuild = this.ent.isBuilder() && foundations.some(function(foundation) {
		...
		if (structure.resourceDropsiteTypes() && structure.resourceDropsiteTypes().indexOf(resource) != -1)
		{
			let foundationAccess = SCRUB.getLandAccess(gameState, foundation);
	...
	// except for wood when farms can be used
	let allowDistant = true;
	if (resource == "wood")
	{
		if (gameState.ai.HQ.turnCache.allowDistantWood === undefined)
			gameState.ai.HQ.turnCache.allowDistantWood =
				!gameState.ai.HQ.canBuild(gameState, "structures/{civ}/field");
		allowDistant = gameState.ai.HQ.turnCache.allowDistantWood;
	}
	if (allowDistant)
	{
		if (this.baseAccess == this.entAccess)
	...
	this.ent.setMetadata(PlayerID, "subrole", SCRUB.Worker.SUBROLE_IDLE);
	return false;
};
 ...
 */
SCRUB.Worker.prototype.startHunting = function(gameState, position)
{
	// First look for possible treasure if any
	if (!position && SCRUB.gatherTreasure(gameState, this.ent))
		return true;
	...
	let isFastMoving = SCRUB.isFastMoving(this.ent);
	let isRanged = this.ent.hasClass("Ranged");
	let entPosition = position ? position : this.ent.position();
	let woodDropsites = gameState.playerData.hasSharedDropsites ?
	                    gameState.getAnyDropsites("wood") : gameState.getOwnDropsites("wood");

	let hasWoodDropsiteWithinDistance = function(supplyPosition, supplyAccess, distSquare)
	{
		for (let dropsite of woodDropsites.values())
		{
			if (!dropsite.position())
			...
			if (owner != PlayerID && (!dropsite.isSharedDropsite() || !gameState.isPlayerMutualAlly(owner)))
				continue;
			if (supplyAccess != SCRUB.getLandAccess(gameState, dropsite))
				continue;
	...
	for (let supply of resources.values())
	{
		...
		if (SCRUB.IsSupplyFull(gameState, supply))
			continue;
		// Check if available resource is worth one additionnal gatherer (except for farms).
		...
		let supplyAccess = SCRUB.getLandAccess(gameState, supply);
		...
		if (!hasWoodDropsiteWithinDistance(supply.position(), supplyAccess, distanceSquare))
			continue;
...
SCRUB.Worker.prototype.startFishing = function(gameState)
{
	...
	let fisherSea = SCRUB.getSeaAccess(gameState, this.ent);
	let fishDropsites = (gameState.playerData.hasSharedDropsites ? gameState.getAnyDropsites("wood") : gameState.getOwnDropsites("wood")).
	...
	let nearestDropsiteDist = function(supply) {
		...
		for (let dropsite of fishDropsites)
		{
			...
			if (owner != PlayerID && (!dropsite.isSharedDropsite() || !gameState.isPlayerMutualAlly(owner)))
				continue;
			if (fisherSea != SCRUB.getSeaAccess(gameState, dropsite))
				continue;
	...
	if (this.ent.getMetadata(PlayerID, "subrole") === SCRUB.Worker.SUBROLE_FISHER)
		this.ent.setMetadata(PlayerID, "subrole", SCRUB.Worker.SUBROLE_IDLE);
	return false;
};

SCRUB.Worker.prototype.gatherNearestField = function(gameState, baseID)
{
	...
	{
		if (SCRUB.IsSupplyFull(gameState, field))
			continue;
 ...
 */
SCRUB.Worker.prototype.buildAnyField = function(gameState, baseID)
{
 ...
 */
SCRUB.Worker.prototype.moveToGatherer = function(gameState, ent, forced)
{
	...
	if (!forced && gameState.ai.elapsedTime < (ent.getMetadata(PlayerID, "nextMoveToGatherer") || 5))
		return;
	const gatherers = this.base.workersBySubrole(gameState, SCRUB.Worker.SUBROLE_GATHERER);
	let dist = Math.min();
	let destination;
	...
	for (let gatherer of gatherers.values())
	{
		...
		if (!gatherer.position() || gatherer.getMetadata(PlayerID, "transport") !== undefined ||
		    SCRUB.getLandAccess(gameState, gatherer) != access || gatherer.isIdle())
			continue;
 ...
 */
SCRUB.Worker.prototype.isInaccessibleSupply = function(gameState)
{
