...
SCRUB.TransportPlan = function(gameState, units, startIndex, endIndex, endPos, ship)
{
	...
	this.state = SCRUB.TransportPlan.BOARDING;
	this.boardingPos = {};
 ...
 */
SCRUB.TransportPlan.BOARDING = "boarding";
/**
 * We're moving ships and eventually unload units.
 */
SCRUB.TransportPlan.SAILING = "sailing";

SCRUB.TransportPlan.prototype.init = function(gameState)
{
...
SCRUB.TransportPlan.prototype.countFreeSlots = function()
{
	let slots = 0;
...
SCRUB.TransportPlan.prototype.countFreeSlotsOnShip = function(ship)
{
...
SCRUB.TransportPlan.prototype.assignUnitToShip = function(gameState, ent)
{
...
SCRUB.TransportPlan.prototype.assignShip = function(gameState)
{
	let pos;
...
SCRUB.TransportPlan.prototype.addUnit = function(unit, endPos)
{
...
SCRUB.TransportPlan.prototype.removeUnit = function(gameState, unit)
{
...
SCRUB.TransportPlan.prototype.releaseShip = function(ship)
{
	if (ship.getMetadata(PlayerID, "transporter") != this.ID)
	{
		API3.warn(" SCRUB: try removing a transporter ship with " + ship.getMetadata(PlayerID, "transporter") +
	...
	if (ship.getMetadata(PlayerID, "role") === SCRUB.Worker.ROLE_SWITCH_TO_TRADER)
		ship.setMetadata(PlayerID, "role", SCRUB.Worker.ROLE_TRADER);
};

SCRUB.TransportPlan.prototype.releaseAll = function()
{
...
SCRUB.TransportPlan.prototype.cancelTransport = function(gameState)
{
 ...
 */
SCRUB.TransportPlan.prototype.update = function(gameState)
{
	if (this.state === SCRUB.TransportPlan.BOARDING)
		this.onBoarding(gameState);
	else if (this.state === SCRUB.TransportPlan.SAILING)
		this.onSailing(gameState);

	return this.units.length;
};

SCRUB.TransportPlan.prototype.onBoarding = function(gameState)
{
	let ready = true;
	...
	this.state = SCRUB.TransportPlan.SAILING;
	this.nTry = {};
	this.unloaded = [];
	this.recovered = [];
};
...
SCRUB.TransportPlan.prototype.isOnBoard = function(ent)
{
	for (let ship of this.transportShips.values())
	{
...
SCRUB.TransportPlan.prototype.getBoardingPos = function(gameState, ship, landIndex, seaIndex, destination, avoidEnnemy)
{
...
SCRUB.TransportPlan.prototype.onSailing = function(gameState)
{
	...
	for (let entId of this.unloaded)
	{
		...
		else if (!ent.position())  // unloading failed
		{
			...
			if (ship)
			{
				...
				{
					API3.warn("SCRUB transportPlan problem: unit not on ship without position ???");
					this.resetUnit(gameState, ent);
					ent.destroy();
				}
			}
			...
			{
				API3.warn("SCRUB transportPlan problem: unit on ship, but no ship ???");
				this.resetUnit(gameState, ent);
				ent.destroy();
			}
		}
		else if (SCRUB.getLandAccess(gameState, ent) != this.endIndex)
		{
...
SCRUB.TransportPlan.prototype.resetUnit = function(gameState, ent)
{
	ent.setMetadata(PlayerID, "transport", undefined);
	ent.setMetadata(PlayerID, "onBoard", undefined);
	ent.setMetadata(PlayerID, "endPos", undefined);
...
SCRUB.TransportPlan.prototype.Serialize = function()
{
	return {
		"ID": this.ID,
...
SCRUB.TransportPlan.prototype.Deserialize = function(data)
{
	for (let key in data)
		this[key] = data[key];
