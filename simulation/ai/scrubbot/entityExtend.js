...
SCRUB.isSiegeUnit = function(ent)
{
...
SCRUB.isFastMoving = function(ent)
{
...
SCRUB.getMaxStrength = function(ent, debugLevel, DamageTypeImportance, againstClass)
{
	let strength = 0;
	...
	for (let type of attackTypes)
	{
		...
		for (let str in attackStrength)
		{
			...
			if (DamageTypeImportance[str])
				...
				API3.warn("SCRUB: " + str + " unknown attackStrength in getMaxStrength (please add " + str + "  to config.js).");
		}
		...
		for (let str in attackTimes)
		{
			...
			case "prepare":
				...
				break;
			default:
				API3.warn("SCRUB: " + str + " unknown attackTimes in getMaxStrength");
			}
		}
	}
	...
	if (resistanceStrength.Damage)
		for (let str in resistanceStrength.Damage)
		{
			...
			if (DamageTypeImportance[str])
				...
				API3.warn("SCRUB: " + str + " unknown resistanceStrength in getMaxStrength (please add " + str + "  to config.js).");
		}
...
SCRUB.getLandAccess = function(gameState, ent)
{
	if (ent.hasClass("Unit"))
	{
		...
		if (!pos)
		{
			let holder = SCRUB.getHolder(gameState, ent);
			if (holder)
				return SCRUB.getLandAccess(gameState, holder);

			API3.warn("SCRUB error: entity without position, but not garrisoned");
...
SCRUB.getSeaAccess = function(gameState, ent)
{
...
SCRUB.setSeaAccess = function(gameState, ent)
{
	SCRUB.getSeaAccess(gameState, ent);
};

/** Decide if we should try to capture (returns true) or destroy (return false) */
SCRUB.allowCapture = function(gameState, ent, target)
{
	...
	{
		capture = ent.captureStrength() * SCRUB.getAttackBonus(ent, target, "Capture");
	...
	else
	{
		...
		{
			capturable.strength += ent.captureStrength() * SCRUB.getAttackBonus(ent, target, "Capture");
...
SCRUB.getAttackBonus = function(ent, target, type)
{
	let attackBonus = 1;
...
SCRUB.returnResources = function(gameState, ent)
{
	...
	for (let dropsite of dropsiteCollection.values())
	{
		...
		if (owner !== PlayerID && (!dropsite.isSharedDropsite() || !gameState.isPlayerMutualAlly(owner)))
			continue;
		if (SCRUB.getLandAccess(gameState, dropsite) != access)
			continue;
...
SCRUB.IsSupplyFull = function(gameState, ent)
{
 ...
 */
SCRUB.getBestBase = function(gameState, ent, onlyConstructedBase = false, exclude = false)
{
	let pos = ent.position();
	let accessIndex;
	if (!pos)
	{
		...
		pos = holder.position();
		accessIndex = SCRUB.getLandAccess(gameState, holder);
	}
	else
		accessIndex = SCRUB.getLandAccess(gameState, ent);
...
SCRUB.getHolder = function(gameState, ent)
{
...
SCRUB.getBuiltEntity = function(gameState, ent)
{
 ...
 */
SCRUB.isNotWorthBuilding = function(gameState, ent)
{
 ...
 */
SCRUB.isLineInsideEnemyTerritory = function(gameState, pos1, pos2, step=70)
{
...
SCRUB.gatherTreasure = function(gameState, ent, water = false)
{
	...
	let treasureFound;
	let distmin = Math.min();
	let access = water ? SCRUB.getSeaAccess(gameState, ent) : SCRUB.getLandAccess(gameState, ent);
	for (let treasure of gameState.ai.HQ.treasures.values())
	{
		...
		if (lastGathered && gameState.ai.elapsedTime - lastGathered < 20)
			continue;
		if (!water && access != SCRUB.getLandAccess(gameState, treasure))
			continue;
		if (water && access != SCRUB.getSeaAccess(gameState, treasure))
			continue;
