...
SCRUB.GarrisonManager = function(Config)
{
	this.Config = Config;
...
SCRUB.GarrisonManager.TYPE_FORCE = "force";
SCRUB.GarrisonManager.TYPE_TRADE = "trade";
SCRUB.GarrisonManager.TYPE_PROTECTION = "protection";
SCRUB.GarrisonManager.TYPE_DECAY = "decay";
SCRUB.GarrisonManager.TYPE_EMERGENCY = "emergency";

SCRUB.GarrisonManager.prototype.update = function(gameState, events)
{
		...
		for (let j = 0; j < list.length; ++j)
		{
			...
			else
			{
				...
				else
				{
					...
					{
						API3.warn("SCRUB garrison error: unit " + ent.id() + " (" + ent.genericName() +
	...
	for (let [id, gmin] of this.decayingStructures.entries())
	{
		...
		if (!ent || ent.owner() !== PlayerID)
			...
			gameState.ai.HQ.defenseManager.garrisonUnitsInside(gameState, ent, { "min": gmin, "type": SCRUB.GarrisonManager.TYPE_DECAY });
	}
};

/** TODO should add the units garrisoned inside garrisoned units */
SCRUB.GarrisonManager.prototype.numberOfGarrisonedUnits = function(holder)
{
...
/** TODO should add the units garrisoned inside garrisoned units */
SCRUB.GarrisonManager.prototype.numberOfGarrisonedSlots = function(holder)
{
	if (!this.holders.has(holder.id()))
...
SCRUB.GarrisonManager.prototype.allowMelee = function(holder)
{
	if (!this.holders.has(holder.id()))
		return undefined;
...
SCRUB.GarrisonManager.prototype.garrison = function(gameState, ent, holder, type)
{
	...
	ent.setMetadata(PlayerID, "subrole", SCRUB.Worker.SUBROLE_GARRISONING);
 ...
 */
SCRUB.GarrisonManager.prototype.leaveGarrison = function(ent)
{
...
SCRUB.GarrisonManager.prototype.cancelGarrison = function(ent)
{
	ent.stopMoving();
	this.leaveGarrison(ent);
...
SCRUB.GarrisonManager.prototype.keepGarrisoned = function(ent, holder, around)
{
	switch (ent.getMetadata(PlayerID, "garrisonType"))
	{
	case SCRUB.GarrisonManager.TYPE_FORCE:           // force the ungarrisoning
		return false;
	case SCRUB.GarrisonManager.TYPE_TRADE:		// trader garrisoned in ship
		return true;
	case SCRUB.GarrisonManager.TYPE_PROTECTION:	// hurt unit for healing or infantry for defense
		...
		if (ent.attackTypes() && ent.attackTypes().indexOf("Melee") !== -1)
			return false;
		if (around.unit)
			return ent.hasClass("Support") || SCRUB.isSiegeUnit(ent);	// only ranged siege here and below as melee siege already released above
		if (SCRUB.isSiegeUnit(ent))
			return around.meleeSiege;
		return holder.buffHeal() && ent.needsHeal();
	case SCRUB.GarrisonManager.TYPE_DECAY:
		return ent.captureStrength() && this.decayingStructures.has(holder.id());
	case SCRUB.GarrisonManager.TYPE_EMERGENCY: // f.e. hero in regicide mode
		if (holder.buffHeal() && ent.isHealable() && ent.healthLevel() < this.Config.garrisonHealthLevel.high)
			return true;
	...
	default:
		...
		ent.setMetadata(PlayerID, "garrisonType", SCRUB.GarrisonManager.TYPE_PROTECTION);
		return true;
	}
};

/** Add this holder in the list managed by the garrisonManager */
SCRUB.GarrisonManager.prototype.registerHolder = function(gameState, holder)
{
 ...
 */
SCRUB.GarrisonManager.prototype.addDecayingStructure = function(gameState, entId, justCaptured)
{
...
SCRUB.GarrisonManager.prototype.removeDecayingStructure = function(entId)
{
	if (!this.decayingStructures.has(entId))
		return;
...
SCRUB.GarrisonManager.prototype.Serialize = function()
{
	return { "holders": this.holders, "decayingStructures": this.decayingStructures };
};

SCRUB.GarrisonManager.prototype.Deserialize = function(data)
{
	for (let key in data)
		this[key] = data[key];
};
