 ...
 * Manage the research
 */
SCRUB.ResearchManager = function(Config)
{
	this.Config = Config;
};
 ...
 */
SCRUB.ResearchManager.prototype.checkPhase = function(gameState, queues)
{
	...
	if (SCRUBRequirements && gameState.hasResearchers(nextPhaseName, true))
	{
		...
		queues.majorTech.addPlan(new SCRUB.ResearchPlan(gameState, nextPhaseName, true));
	}
};

SCRUB.ResearchManager.prototype.researchPopulationBonus = function(gameState, queues)
{
	...
	for (let tech of techs)
	{
		...
		if (tech[1]._template.modifications[0].value !== "Population/Bonus")
			continue;
		queues.minorTech.addPlan(new SCRUB.ResearchPlan(gameState, tech[0]));
		break;
	}
};

SCRUB.ResearchManager.prototype.researchTradeBonus = function(gameState, queues)
{
	if (queues.minorTech.hasQueuedUnits())
		return;
...
SCRUB.ResearchManager.prototype.researchWantedTechs = function(gameState, techs)
{
	let phase1 = gameState.currentPhase() === 1;
	let available = phase1 ? gameState.ai.queueManager.getAvailableResources(gameState) : null;
	const numWorkers = phase1 ? gameState.getOwnEntitiesByRole(SCRUB.Worker.ROLE_WORKER, true).length : 0;
	for (let tech of techs)
	{
...
SCRUB.ResearchManager.prototype.researchPreferredTechs = function(gameState, techs)
{
	let phase2 = gameState.currentPhase() === 2;
	let available = phase2 ? gameState.ai.queueManager.getAvailableResources(gameState) : null;
	const numWorkers = phase2 ? gameState.getOwnEntitiesByRole(SCRUB.Worker.ROLE_WORKER, true).length : 0;
	for (let tech of techs)
	{
		if (!tech[1]._template.modifications)
			continue;
...
SCRUB.ResearchManager.prototype.update = function(gameState, queues)
{
	...
	if (techName)
	{
		if (techName.increasePriority)
		{
			...
			let plan = new SCRUB.ResearchPlan(gameState, techName.name);
			plan.queueToReset = "minorTech";
			queues.minorTech.addPlan(plan);
		}
		else
			queues.minorTech.addPlan(new SCRUB.ResearchPlan(gameState, techName.name));
		return;
	}
	...
	queues.minorTech.addPlan(new SCRUB.ResearchPlan(gameState, pickRandom(techs)[0]));
};

SCRUB.ResearchManager.prototype.CostSum = function(cost)
{
	let costSum = 0;
	for (let res in cost)
		costSum += cost[res];
...
SCRUB.ResearchManager.prototype.Serialize = function()
{
	return {};
};

SCRUB.ResearchManager.prototype.Deserialize = function(data)
{
};
