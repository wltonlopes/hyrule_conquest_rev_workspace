 ...
 */
SCRUB.NavalManager = function(Config)
{
	this.Config = Config;
...
SCRUB.NavalManager.prototype.init = function(gameState, deserializing)
{
	// docks
	this.docks = gameState.getOwnStructures().filter(API3.Filters.byClasses(["Dock", "Shipyard"]));
	this.docks.registerUpdates();

	this.ships = gameState.getOwnUnits().filter(API3.Filters.and(API3.Filters.byClass("Ship"), API3.Filters.not(API3.Filters.byMetadata(PlayerID, "role", SCRUB.Worker.ROLE_TRADER))));
	...
	for (let ship of this.ships.values())
		SCRUB.setSeaAccess(gameState, ship);
	for (let dock of this.docks.values())
		SCRUB.setSeaAccess(gameState, dock);
};

SCRUB.NavalManager.prototype.updateFishingBoats = function(sea, num)
{
	if (this.wantedFishShips[sea])
		this.wantedFishShips[sea] = num;
};

SCRUB.NavalManager.prototype.resetFishingBoats = function(gameState, sea)
{
	if (sea !== undefined)
...
SCRUB.NavalManager.prototype.getFishSea = function(gameState, fish)
{
...
SCRUB.NavalManager.prototype.canFishSafely = function(gameState, fish)
{
...
SCRUB.NavalManager.prototype.getUnconnectedSeas = function(gameState, region)
{
	let seas = gameState.ai.accessibility.regionLinks[region].slice();
	this.docks.forEach(dock => {
		if (!dock.hasClass("Dock") || SCRUB.getLandAccess(gameState, dock) != region)
			return;
		let i = seas.indexOf(SCRUB.getSeaAccess(gameState, dock));
		if (i != -1)
			seas.splice(i--, 1);
	});
	return seas;
...
SCRUB.NavalManager.prototype.checkEvents = function(gameState, queues, events)
{
	...
	for (let evt of events.Destroy)
	{
		...
		if (plan.state === SCRUB.TransportPlan.BOARDING)
		{
		...
		else if (plan.state === SCRUB.TransportPlan.SAILING)
		{
...
SCRUB.NavalManager.prototype.getPlan = function(ID)
{
...
SCRUB.NavalManager.prototype.addPlan = function(plan)
{
 ...
 */
SCRUB.NavalManager.prototype.requireTransport = function(gameState, ent, startIndex, endIndex, endPos)
{
	if (!ent.canGarrison())
		return false;
	...
	if (ent.getMetadata(PlayerID, "transport") !== undefined)
	{
		if (this.Config.debug > 0)
			API3.warn("SCRUB naval manager error: unit " + ent.id() + " has already required a transport");
		return false;
	}
	...
	for (let plan of this.transportPlans)
	{
		if (plan.startIndex != startIndex || plan.endIndex != endIndex || plan.state !== SCRUB.TransportPlan.BOARDING)
			continue;
		// Limit the number of siege units per transport to avoid problems when ungarrisoning
		if (SCRUB.isSiegeUnit(ent) && plan.units.filter(unit => SCRUB.isSiegeUnit(unit)).length > 3)
			continue;
		plans.push(plan);
	}
	...
	let plan = new SCRUB.TransportPlan(gameState, [ent], startIndex, endIndex, endPos);
	if (plan.failed)
	{
		if (this.Config.debug > 1)
...
SCRUB.NavalManager.prototype.splitTransport = function(gameState, plan)
{
	if (this.Config.debug > 1)
		API3.warn(">>>> split of transport plan started <<<<");
	let newplan = new SCRUB.TransportPlan(gameState, [], plan.startIndex, plan.endIndex, plan.endPos);
	if (newplan.failed)
	{
		if (this.Config.debug > 1)
 ...
 */
SCRUB.NavalManager.prototype.createTransportIfNeeded = function(gameState, fromPos, toPos, toAccess)
{
	...
	for (let ship of this.ships.values())
	{
		...
		let plan = new SCRUB.TransportPlan(gameState, units, fromAccess, toAccess, toPos, ship);
		if (plan.failed)
			continue;
		plan.init(gameState);
		this.transportPlans.push(plan);
...
SCRUB.NavalManager.prototype.setMinimalTransportShips = function(gameState, sea, number)
{
	if (!sea)
		return;
...
SCRUB.NavalManager.prototype.checkLevels = function(gameState, queues)
{
...
SCRUB.NavalManager.prototype.maintainFleet = function(gameState, queues)
{
	if (queues.ships.hasQueuedUnits())
		return;
	...
	for (let sea = 0; sea < this.seaShips.length; ++sea)
	{
		...
		if (this.seaTransportShips[sea].length < this.wantedTransportShips[sea])
		{
			...
			if (template)
			{
				queues.ships.addPlan(new SCRUB.TrainingPlan(gameState, template, { "sea": sea }, 1, 1));
				continue;
			}
		}
		...
		if (this.seaFishShips[sea].length < this.wantedFishShips[sea])
		{
			...
			if (template)
			{
				queues.ships.addPlan(new SCRUB.TrainingPlan(gameState, template, { "base": 0, "role": SCRUB.Worker.ROLE_WORKER, "sea": sea }, 1, 1));
				continue;
			}
		}
	}
...
SCRUB.NavalManager.prototype.assignShipsToPlans = function(gameState)
{
	for (let plan of this.transportPlans)
...
SCRUB.NavalManager.prototype.isShipBoarding = function(ship)
{
	if (!ship.position())
		return false;
 ...
 */
SCRUB.NavalManager.prototype.moveApart = function(gameState)
{
	let blockedShips = [];
	let blockedIds = [];
	...
	for (let ship of this.ships.values())
	{
		...
		else if (ship.isIdle())
		{
			...
			if (SCRUB.gatherTreasure(gameState, ship, true))
				continue;
			...
			{
				if (SCRUB.getSeaAccess(gameState, dock) != sea)
					continue;
	...
	for (let ship of gameState.ai.HQ.tradeManager.traders.filter(API3.Filters.byClass("Ship")).values())
	{
		...
		if (role === undefined || role !== SCRUB.Worker.ROLE_TRADER)	// already accounted before
			continue;
...
SCRUB.NavalManager.prototype.buildNavalStructures = function(gameState, queues)
{
			...
			gameState.ai.HQ.canBuild(gameState, "structures/{civ}/dock"))
		{
			...
			for (const base of gameState.ai.HQ.baseManagers())
			{
				...
				for (let sea of remaining)
				{
					...
					queues.dock.addPlan(new SCRUB.ConstructionPlan(gameState, "structures/{civ}/dock", { "land": wantedLand, "sea": sea }));
					dockStarted = true;
					break;
				}
			}
	...
	queues.militaryBuilding.addPlan(new SCRUB.ConstructionPlan(gameState, template, { "land": wantedLand, "sea": sea }));
};

/** goal can be either attack (choose ship with best arrowCount) or transport (choose ship with best capacity) */
SCRUB.NavalManager.prototype.getBestShip = function(gameState, sea, goal)
{
...
SCRUB.NavalManager.prototype.update = function(gameState, queues, events)
{
...
SCRUB.NavalManager.prototype.Serialize = function()
{
	let properties = {
...
SCRUB.NavalManager.prototype.Deserialize = function(gameState, data)
{
	...
	for (let i in data.transports)
	{
		...
		let plan = new SCRUB.TransportPlan(gameState, [], dataPlan.startIndex, dataPlan.endIndex, dataPlan.endPos);
