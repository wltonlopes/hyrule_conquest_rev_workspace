...
SCRUB.TERRITORY_PLAYER_MASK = 0x1F;
SCRUB.TERRITORY_BLINKING_MASK = 0x40;

SCRUB.createObstructionMap = function(gameState, accessIndex, template)
{
	...
	{
		let tilePlayer = territoryMap.data[k] & SCRUB.TERRITORY_PLAYER_MASK;
		let isConnected = (territoryMap.data[k] & SCRUB.TERRITORY_BLINKING_MASK) == 0;
...
SCRUB.createTerritoryMap = function(gameState)
{
	let map = gameState.ai.territoryMap;

	let ret = new API3.Map(gameState.sharedScript, "territory", map.data);
	ret.getOwner = function(p) { return this.point(p) & SCRUB.TERRITORY_PLAYER_MASK; };
	ret.getOwnerIndex = function(p) { return this.map[p] & SCRUB.TERRITORY_PLAYER_MASK; };
	ret.isBlinking = function(p) { return (this.point(p) & SCRUB.TERRITORY_BLINKING_MASK) != 0; };
	return ret;
};
...
SCRUB.outside_Mask = 1;
SCRUB.border_Mask = 2;
SCRUB.fullBorder_Mask = SCRUB.outside_Mask | SCRUB.border_Mask;
SCRUB.narrowFrontier_Mask = 4;
SCRUB.largeFrontier_Mask = 8;
SCRUB.fullFrontier_Mask = SCRUB.narrowFrontier_Mask | SCRUB.largeFrontier_Mask;

SCRUB.createBorderMap = function(gameState)
{
...
SCRUB.debugMap = function(gameState, map)
{
	let width = map.width;
