...
SCRUB.DIFFICULTY_SANDBOX = 0;
SCRUB.DIFFICULTY_VERY_EASY = 1;
SCRUB.DIFFICULTY_EASY = 2;
SCRUB.DIFFICULTY_MEDIUM = 3;
SCRUB.DIFFICULTY_HARD = 4;
SCRUB.DIFFICULTY_VERY_HARD = 5;

SCRUB.Config = function(difficulty = SCRUB.DIFFICULTY_MEDIUM, behavior)
{
	...
	this.Military = {
		"towerLapseTime": 60,	// Time to wait between building 2 towers
		"fortressLapseTime": 3000,	// Time to wait between building 2 fortresses
		"popForBarracks1": 20,
		"popForBarracks2": 60,
		"popForForge": 30,
		"numSentryTowers": 8
	...
	this.Economy = {
		...
		"supportRatio": 0.6,	// fraction of support workers among the workforce
		"provisionFields": 3
	};

	// Note: attack settings are set directly in attackPlan.js
	// defense
	this.Defense =
	{
		"defenseRatio": { "ally": 1.2, "neutral": 1.8, "own": 3 },	// ratio of defenders/attackers.
	...
	this.priorities =
	{
		"villager": 60,      // should be slightly lower than the citizen soldier one to not get all the food
		"citizenSoldier": 90,
		"trader": 50,
		"healer": 10,
		"ships": 70,
		...
		"militaryBuilding": 250,
		"defenseBuilding": 250,
		"civilCentre": 950,
		"majorTech": 700,
		"minorTech": 250,
		"wonder": 1000,
		"emergency": 2000    // used only in emergency situations, should be the highest one
	};
	...
	// See SCRUB.QueueManager.prototype.wantedGatherRates()
	this.queues =
	{
		"firstTurn": {
			"wood": 40,
...
SCRUB.Config.prototype.setConfig = function(gameState)
{
	if (this.difficulty > SCRUB.DIFFICULTY_SANDBOX)
	{
		...
		let personalityList = {
			"random": { "min": 0, "max": 1 },
			"defensive": { "min": 0.8, "max": 1.20 },
			"balanced": { "min": 0.50, "max": 1 },
			"aggressive": { "min": 0.50, "max": 0.80 }
		};
	...
	// SCRUB usually uses the continuous values of personality.aggressive and personality.defensive
	...
	if (this.difficulty < SCRUB.DIFFICULTY_EASY)
	{
		this.popScaling = 0.5;
	...
	else if (this.difficulty < SCRUB.DIFFICULTY_MEDIUM)
	{
		this.popScaling = 0.7;
		this.Economy.supportRatio = 0.4;
		this.Economy.provisionFields = 3;
		this.Military.numSentryTowers = this.personality.defensive > this.personalityCut.strong ? 1 : 0;
	}
	else
	{
		if (this.difficulty == SCRUB.DIFFICULTY_MEDIUM)
	...
	if (this.difficulty < SCRUB.DIFFICULTY_EASY)
		this.Economy.targetNumWorkers = Math.max(1, Math.min(50, maxPop));
	else if (this.difficulty < SCRUB.DIFFICULTY_MEDIUM)
		this.Economy.targetNumWorkers = Math.max(1, Math.min(70, Math.floor(maxPop/2)));
	else
		this.Economy.targetNumWorkers = Math.max(1, Math.min(140, Math.floor(maxPop/3)));
	...
	if (this.debug < 2)
		return;
	API3.warn(" >>>  SCRUB bot: personality = " + uneval(this.personality));
};

SCRUB.Config.prototype.Cheat = function(gameState)
{
...
SCRUB.Config.prototype.Serialize = function()
{
	var data = {};
	for (let key in this)
...
SCRUB.Config.prototype.Deserialize = function(data)
{
	for (let key in data)
		this[key] = data[key];
};
