...
SCRUB.ConstructionPlan = function(gameState, type, metadata, position)
{
	if (!SCRUB.QueuePlan.call(this, gameState, type, metadata))
		return false;
...
SCRUB.ConstructionPlan.prototype = Object.create(SCRUB.QueuePlan.prototype);

SCRUB.ConstructionPlan.prototype.canStart = function(gameState)
{
...
SCRUB.ConstructionPlan.prototype.start = function(gameState)
{
	...
	if (!builder)
	{
		API3.warn("SCRUB error: builder not found when starting construction.");
		Engine.ProfileStop();
		return;
	}
...
SCRUB.ConstructionPlan.prototype.findGoodPosition = function(gameState)
{
	...
	else	// No position was specified so try and find a sensible place to build
	{
		...
		if (!HQ.requireHouses || !template.hasClass("House"))
		{
			gameState.getOwnStructures().forEach(function(ent) {
				...
				let struct = SCRUB.getBuiltEntity(gameState, ent);
		...
		if (template.hasClass("Farmstead"))
		{
			for (let j = 0; j < placement.map.length; ++j)
			{
				...
				if (HQ.borderMap.map[j] & SCRUB.fullBorder_Mask)
	...
	if (this.metadata && this.metadata.base !== undefined)
	{
		...
		for (let j = 0; j < placement.map.length; ++j)
		{
			...
			{
				if (favorBorder && HQ.borderMap.map[j] & SCRUB.border_Mask)
					placement.set(j, placement.map[j] + 50);
				else if (disfavorBorder && !(HQ.borderMap.map[j] & SCRUB.fullBorder_Mask))
 ...
 */
SCRUB.ConstructionPlan.prototype.findDockPosition = function(gameState)
{
	let template = this.template;
	...
	for (let j = 0; j < territoryMap.length; ++j)
	{
		...
		if (oversea)
		{
			...
			{
				if (SCRUB.getSeaAccess(gameState, dock) != navalPassMap[i])
					continue;
		...
		if (gameState.ai.HQ.borderMap.map[j] & SCRUB.fullBorder_Mask)
			score += 20;
 ...
 */
SCRUB.ConstructionPlan.prototype.buildOverseaDock = function(gameState, template)
{
	...
	for (let i = 0; i < gameState.ai.accessibility.regionSize.length; ++i)
	{
		...
		for (let dock of docks.values())
		{
			if (SCRUB.getLandAccess(gameState, dock) != i)
				continue;
			keep = false;
			break;
		}
		...
		for (let cc of ccEnts.values())
		{
			let ccAccess = SCRUB.getLandAccess(gameState, cc);
			if (ccAccess != i)
			{
	...
	if (!gameState.ai.HQ.navalMap)
		API3.warn("SCRUB.findOverseaLand on a non-naval map??? we should never go there ");
...
SCRUB.ConstructionPlan.prototype.getDockAngle = function(gameState, x, z, size)
{
 ...
 */
SCRUB.ConstructionPlan.prototype.checkDockPlacement = function(gameState, x, z, halfDepth, halfWidth, angle)
{
 ...
 */
SCRUB.ConstructionPlan.prototype.around = [[ 1.0, 0.0], [ 0.87, 0.50], [ 0.50, 0.87], [ 0.0, 1.0], [-0.50, 0.87], [-0.87, 0.50],
	        [-1.0, 0.0], [-0.87, -0.50], [-0.50, -0.87], [ 0.0, -1.0], [ 0.50, -0.87], [ 0.87, -0.50]];

SCRUB.ConstructionPlan.prototype.isDockLocation = function(gameState, j, dimension, wantedLand, wantedSea)
{
	let width = gameState.ai.HQ.territoryMap.width;
	let cellSize = gameState.ai.HQ.territoryMap.cellSize;
 ...
 */
SCRUB.ConstructionPlan.prototype.getFrontierProximity = function(gameState, j)
{
	...
	for (let a of this.around)
	{
		for (let i = 1; i < 5; ++i)
		{
			...
			if (jz < 0 || jz >= width)
				continue;
			if (borderMap.map[jx+width*jz] & SCRUB.outside_Mask)
				continue;
 ...
 */
SCRUB.ConstructionPlan.prototype.getResourcesAround = function(gameState, types, i, radius)
{
...
SCRUB.ConstructionPlan.prototype.isGo = function(gameState)
{
...
SCRUB.ConstructionPlan.prototype.onStart = function(gameState)
{
...
SCRUB.ConstructionPlan.prototype.Serialize = function()
{
	return {
...
SCRUB.ConstructionPlan.prototype.Deserialize = function(gameState, data)
{
	for (let key in data)
		this[key] = data[key];
