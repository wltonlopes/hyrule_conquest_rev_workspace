...
SCRUB.BuildManager.prototype.init = function(gameState)
{
...
SCRUB.BuildManager.prototype.incrementBuilderCounters = function(civ, ent, increment)
{
	for (let buildable of ent.buildableEntities(civ))
	{
		if (this.builderCounters.has(buildable))
		{
			...
			if (count < 0)
			{
				API3.warn(" SCRUB error in incrementBuilderCounters for " + buildable + " with count < 0");
				continue;
			}
			...
			API3.warn(" SCRUB error in incrementBuilderCounters for " + buildable + " not yet set");
	}
};

/** Update the builders counters */
SCRUB.BuildManager.prototype.checkEvents = function(gameState, events)
{
 ...
 */
SCRUB.BuildManager.prototype.findStructuresByFilter = function(gameState, filter)
{
	const result = [];
 ...
 */
SCRUB.BuildManager.prototype.findStructureWithClass = function(gameState, classes)
{
	return this.findStructuresByFilter(gameState, API3.Filters.byClasses(classes))[0];
};

SCRUB.BuildManager.prototype.hasBuilder = function(template)
{
	let numBuilders = this.builderCounters.get(template);
	return numBuilders && numBuilders > 0;
};

SCRUB.BuildManager.prototype.isUnbuildable = function(gameState, template)
{
	return this.unbuildables.has(template) && this.unbuildables.get(template).time > gameState.ai.elapsedTime;
};

SCRUB.BuildManager.prototype.setBuildable = function(template)
{
...
SCRUB.BuildManager.prototype.setUnbuildable = function(gameState, template, time = 90, reason = "room")
{
	if (!this.unbuildables.has(template))
...
SCRUB.BuildManager.prototype.numberMissingRoom = function(gameState)
{
	let num = 0;
...
SCRUB.BuildManager.prototype.resetMissingRoom = function(gameState)
{
...
SCRUB.BuildManager.prototype.Serialize = function()
{
	return {
...
SCRUB.BuildManager.prototype.Deserialize = function(data)
{
	for (let key in data)
		this[key] = data[key];
};
