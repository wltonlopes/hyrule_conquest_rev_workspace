...
SCRUB.HQ.prototype.gameAnalysis = function(gameState)
{
	...
	this.canExpand = this.Config.difficulty != SCRUB.DIFFICULTY_SANDBOX;
 ...
 */
SCRUB.HQ.prototype.assignStartingEntities = function(gameState)
{
 ...
 */
SCRUB.HQ.prototype.regionAnalysis = function(gameState)
{
 ...
 */
SCRUB.HQ.prototype.structureAnalysis = function(gameState)
{
 ...
 */
SCRUB.HQ.prototype.buildFirstBase = function(gameState)
{
	...
	if (!total.canAfford(new API3.Resources(template.cost())))
	{
		...
		for (let ent of gameState.getOwnUnits().values())
		{
			...
			if (ent.isIdle())
				SCRUB.gatherTreasure(gameState, ent);
	...
	if (goal == "dock")
	{
		...
		gameState.ai.queues.dock.addPlan(new SCRUB.ConstructionPlan(gameState, "structures/{civ}/dock", { "sea": sea, "proximity": startingPoint[imax].pos }));
	}
	else
		gameState.ai.queues.civilCentre.addPlan(new SCRUB.ConstructionPlan(gameState, "structures/{civ}/civil_centre", { "base": -1, "resource": "wood", "proximity": startingPoint[imax].pos }));
};
 ...
 */
SCRUB.HQ.prototype.dispatchUnits = function(gameState)
{
	...
	if (allycc.length)
	{
		...
		units.filter(API3.Filters.byClasses(["Infantry+Ranged"])).forEach(ent => {
			...
			{
				if (!cc.position() || SCRUB.getLandAccess(gameState, cc) != access)
					continue;
				--num;
				--num1;
 ...
 */
SCRUB.HQ.prototype.configFirstBase = function(gameState)
{
	if (!this.hasPotentialBase())
		return;
	...
	if (!gameState.getOwnEntitiesByClass("DropsiteWood", true).hasEntities())
	{
		...
		if (newDP.quality > 40 && this.canBuild(gameState, newDP.templateName))
		{
			...
			gameState.ai.queues.dropsites.addPlan(new SCRUB.ConstructionPlan(gameState, newDP.templateName, { "base": this.baseManagers()[0].ID }, newDP.pos));
		}
	}
	// and build immediately a field if needed
	if (this.needField)
	{
		const template = gameState.applyCiv("structures/{civ}/field");
		if (!gameState.getOwnEntitiesByClass("Field", true).hasEntities() && this.canBuild(gameState, template))
			gameState.ai.queues.field.addPlan(new SCRUB.ConstructionPlan(gameState, template, { "base": this.baseManagers()[0].ID }));
	}
};
