 ...
 */
SCRUB.DiplomacyManager = function(Config)
{
	this.Config = Config;
 ...
 */
SCRUB.DiplomacyManager.prototype.init = function(gameState)
{
 ...
 */
SCRUB.DiplomacyManager.prototype.tributes = function(gameState)
{
	...
	for (let i = 1; i < gameState.sharedScript.playersData.length; ++i)
	{
			...
			else if (this.Config.chat && availableResources[res] === 0 && allyResources[res] > totalResources[res] + 600)
			{
				...
				for (let k = 0; k < mostNeeded.length; ++k)
				{
					if (mostNeeded[k].type == res && mostNeeded[k].wanted > 0)
					{
						...
						SCRUB.chatRequestTribute(gameState, res);
		...
		if (this.Config.chat)
			SCRUB.chatSentTribute(gameState, i);
		Engine.PostCommand(PlayerID, { "type": "tribute", "player": i, "amounts": tribute });
	}
};

SCRUB.DiplomacyManager.prototype.checkEvents = function(gameState, events)
{
	...
	for (let evt of events.DiplomacyChanged)
	{
		...
		else if (gameState.sharedScript.playersData[evt.player].isAlly[PlayerID] && gameState.isPlayerEnemy(evt.player))
		{
			...
			SCRUB.chatAnswerRequestDiplomacy(gameState, evt.player, "ally", response);
		}
 ...
 */
SCRUB.DiplomacyManager.prototype.lastManStandingCheck = function(gameState)
{
	...
	if (playerToTurnAgainst)
	{
		...
		{
			if (request.status === "waitingForTribute")
				SCRUB.chatAnswerRequestDiplomacy(gameState, player, request.requestType, "decline");
 ...
 */
SCRUB.DiplomacyManager.prototype.handleDiplomacyRequest = function(gameState, player, requestType)
{
	...
	SCRUB.chatAnswerRequestDiplomacy(gameState, player, requestType, response, requiredTribute);
};

SCRUB.DiplomacyManager.prototype.changePlayerDiplomacy = function(gameState, player, newDiplomaticStance)
{
	...
	if (this.Config.chat)
		SCRUB.chatNewDiplomacy(gameState, player, newDiplomaticStance);
};

SCRUB.DiplomacyManager.prototype.checkRequestedTributes = function(gameState)
{
	for (let [player, data] of this.receivedDiplomacyRequests)
		if (data.status === "waitingForTribute" && gameState.ai.elapsedTime > data.warnTime)
		{
			if (data.sentWarning)
			{
				...
				SCRUB.chatAnswerRequestDiplomacy(gameState, player, data.requestType, "decline");
			}
			else
			{
				data.sentWarning = true;
				data.warnTime = gameState.ai.elapsedTime + 60;
				SCRUB.chatAnswerRequestDiplomacy(gameState, player, data.requestType, "waitingForTribute", {
					"wanted": data.wanted,
					"type": data.type
				});
			}
 ...
 */
SCRUB.DiplomacyManager.prototype.sendDiplomacyRequest = function(gameState)
{
	let player;
	let max = 0;
	for (let i = 1; i < gameState.sharedScript.playersData.length; ++i)
	...
	SCRUB.chatNewRequestDiplomacy(gameState, player, requestType, "sendRequest");
};

SCRUB.DiplomacyManager.prototype.checkSentDiplomacyRequests = function(gameState)
{
	for (let [player, data] of this.sentDiplomacyRequests)
		...
		{
			SCRUB.chatNewRequestDiplomacy(gameState, player, data.requestType, "requestExpired");
			this.sentDiplomacyRequests.delete(player);
		}
};

SCRUB.DiplomacyManager.prototype.update = function(gameState, events)
{
...
SCRUB.DiplomacyManager.prototype.Serialize = function()
{
	return {
...
SCRUB.DiplomacyManager.prototype.Deserialize = function(data)
{
	for (let key in data)
		this[key] = data[key];
};
