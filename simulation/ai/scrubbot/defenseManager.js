SCRUB.DefenseManager = function(Config)
{
...
SCRUB.DefenseManager.prototype.update = function(gameState, events)
{
...
SCRUB.DefenseManager.prototype.makeIntoArmy = function(gameState, entityID, type = "default")
{
	if (type == "default")
	{
	...
	this.armies.push(new SCRUB.DefenseArmy(gameState, [entityID], type));
};

SCRUB.DefenseManager.prototype.getArmy = function(partOfArmy)
{
	return this.armies.find(army => army.ID == partOfArmy);
};

SCRUB.DefenseManager.prototype.isDangerous = function(gameState, entity)
{
	if (!entity.position())
		return false;
...
SCRUB.DefenseManager.prototype.checkEnemyUnits = function(gameState)
{
...
SCRUB.DefenseManager.prototype.checkEnemyArmies = function(gameState)
{
...
SCRUB.DefenseManager.prototype.assignDefenders = function(gameState)
{
	if (!this.armies.length)
		return;
	...
	for (let army of this.armies)
	{
		...
		for (let entId of army.foeEntities)
		{
			...
			if (!ent || !ent.position())
				continue;
			armyAccess = SCRUB.getLandAccess(gameState, ent);
			break;
		}
		if (!armyAccess)
			API3.warn(" SCRUB error: attacking army " + army.ID + " without access");
		army.recalculatePosition(gameState);
	...
	gameState.getOwnUnits().forEach(function(ent) {
		...
		if (ent.getMetadata(PlayerID, "plan") !== undefined && ent.getMetadata(PlayerID, "plan") != -1)
		{
			...
			if (subrole &&
				(subrole === SCRUB.Worker.SUBROLE_COMPLETING || subrole === SCRUB.Worker.SUBROLE_WALKING || subrole === SCRUB.Worker.SUBROLE_ATTACKING))
				return;
		}
	...
	for (let ipass = 0; ipass < 2; ++ipass)
	{
		...
		for (let i = 0; i < potentialDefenders.length; ++i)
		{
			...
			let aMin;
			let distMin;
			let access = ipass == 0 ? SCRUB.getLandAccess(gameState, ent) : undefined;
			for (let a = 0; a < armiesNeeding.length; ++a)
			{
				...
				if (!armiesNeeding[a].army.foeEntities.some(eEnt => {
					...
					return ent.canAttackTarget(eEntID, SCRUB.allowCapture(gameState, ent, eEntID));
					}))
					continue;
			...
			armiesNeeding[aMin].need -= SCRUB.getMaxStrength(ent, this.Config.debug, this.Config.DamageTypeImportance);
...
SCRUB.DefenseManager.prototype.abortArmy = function(gameState, army)
{
	army.clear(gameState);
	for (let i = 0; i < this.armies.length; ++i)
	{
 ...
 */
SCRUB.DefenseManager.prototype.checkEvents = function(gameState, events)
{
		...
		if (target.hasClass("Unit") && attacker.hasClass("Unit"))
		{
			...
			if (currentTarget)
			{
				...
				if (unitAIStateOrder === "REPAIR" && currentTarget.hasDefensiveFire())
					continue;
				if (unitAIStateOrder === "COMBAT" && !SCRUB.isSiegeUnit(currentTarget) &&
				    gameState.ai.HQ.capturableTargets.has(orderData[0].target))
				{
					...
					for (let entId of capturableTarget.ents)
					{
						if (allAttacked[entId])
							continue;
						let ent = gameState.getEntityById(entId);
						if (!ent || !ent.position() || !ent.canAttackTarget(attacker, SCRUB.allowCapture(gameState, ent, attacker)))
							continue;
					...
					if (minEnt)
					{
						...
						minEnt.attack(attacker.id(), SCRUB.allowCapture(gameState, minEnt, attacker));
					}
				}
			}
			let allowCapture = SCRUB.allowCapture(gameState, target, attacker);
...
SCRUB.DefenseManager.prototype.garrisonUnitsInside = function(gameState, target, data)
{
	...
	let access = SCRUB.getLandAccess(gameState, target);
	let garrisonManager = gameState.ai.HQ.garrisonManager;
	let garrisonArrowClasses = target.getGarrisonArrowClasses();
	const typeGarrison = data.type || SCRUB.GarrisonManager.TYPE_PROTECTION;
	...
	if (allowMelee === undefined)
	{
		...
		if (data.attacker)
			allowMelee = data.attacker.hasClass("Structure") ? data.attacker.attackRange("Ranged") : !SCRUB.isSiegeUnit(data.attacker);
		else
			allowMelee = true;
	}
	let units = gameState.getOwnUnits().filter(ent => {
		...
		if (!ent.hasClasses(garrisonArrowClasses))
			return false;
		if (typeGarrison !== SCRUB.GarrisonManager.TYPE_DECAY && !allowMelee && ent.attackTypes().indexOf("Melee") != -1)
			return false;
		if (ent.getMetadata(PlayerID, "transport") !== undefined)
			return false;
		...
		if (ent.getMetadata(PlayerID, "plan") !== undefined && ent.getMetadata(PlayerID, "plan") >= 0)
		{
			...
			if (typeGarrison !== SCRUB.GarrisonManager.TYPE_DECAY && subrole &&
				(subrole === SCRUB.Worker.SUBROLE_COMPLETING || subrole === SCRUB.Worker.SUBROLE_WALKING || subrole === SCRUB.Worker.SUBROLE_ATTACKING))
				return false;
		}
		if (SCRUB.getLandAccess(gameState, ent) != access)
			return false;
		return true;
...
SCRUB.DefenseManager.prototype.garrisonSiegeUnit = function(gameState, unit)
{
	let distmin = Math.min();
	let nearest;
	let unitAccess = SCRUB.getLandAccess(gameState, unit);
	let garrisonManager = gameState.ai.HQ.garrisonManager;
	for (let ent of gameState.getAllyStructures().values())
	{
		...
		if (ent.hitpoints() < ent.garrisonEjectHealth() * ent.maxHitpoints())
			continue;
		if (SCRUB.getLandAccess(gameState, ent) != unitAccess)
			continue;
	...
	if (nearest)
		garrisonManager.garrison(gameState, unit, nearest, SCRUB.GarrisonManager.TYPE_PROTECTION);
 ...
 */
SCRUB.DefenseManager.prototype.garrisonAttackedUnit = function(gameState, unit, emergency = false)
{
	let distmin = Math.min();
	let nearest;
	let unitAccess = SCRUB.getLandAccess(gameState, unit);
	...
	garrisonManager.garrison(gameState, unit, nearest, nearest.buffHeal() ? SCRUB.GarrisonManager.TYPE_PROTECTION : SCRUB.GarrisonManager.TYPE_EMERGENCY);
	return true;
};
 ...
 */
SCRUB.DefenseManager.prototype.GetCooperationLevel = function(ally)
{
 ...
 */
SCRUB.DefenseManager.prototype.switchToAttack = function(gameState, army)
{
	if (!army)
		return;
	for (let targetId of this.targetList)
	{
		...
		{
			let ent = gameState.getEntityById(entId);
			if (!ent || !ent.position() || SCRUB.getLandAccess(gameState, ent) != targetAccess)
				continue;
...
SCRUB.DefenseManager.prototype.Serialize = function()
{
	let properties = {
...
SCRUB.DefenseManager.prototype.Deserialize = function(gameState, data)
{
	...
	{
		let army = new SCRUB.DefenseArmy(gameState, []);
