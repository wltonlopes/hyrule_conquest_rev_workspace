 ...
 * Attack Manager
 */
SCRUB.AttackManager = function(Config)
{
	this.Config = Config;
	...
	this.upcomingAttacks = {
		[SCRUB.AttackPlan.TYPE_RUSH]: [],
		[SCRUB.AttackPlan.TYPE_RAID]: [],
		[SCRUB.AttackPlan.TYPE_DEFAULT]: []
	};
	this.startedAttacks = {
		[SCRUB.AttackPlan.TYPE_RUSH]: [],
		[SCRUB.AttackPlan.TYPE_RAID]: [],
...
SCRUB.AttackManager.prototype.checkEvents = function(gameState, events)
{
	...
	for (let evt of events.AttackRequest)
	{
		...
		for (let attackType in this.upcomingAttacks)
		{
			...
			{
				if (attack.state === SCRUB.AttackPlan.STATE_COMPLETING)
				{
	...
	if (targetPlayer !== undefined)
		SCRUB.chatAnswerRequestAttack(gameState, targetPlayer, answer, other);
 ...
 */
SCRUB.AttackManager.prototype.assignBombers = function(gameState)
{
	...
	for (let ent of bombers.values())
	{
		...
		if (ent.getMetadata(PlayerID, "plan") !== undefined && ent.getMetadata(PlayerID, "plan") != -1)
		{
			...
			if (subrole && (subrole === SCRUB.Worker.SUBROLE_COMPLETING || subrole === SCRUB.Worker.SUBROLE_WALKING || subrole === SCRUB.Worker.SUBROLE_ATTACKING))
				continue;
		}
		...
		{
			if (!ent.canAttackTarget(struct, SCRUB.allowCapture(gameState, ent, struct)))
				continue;
 ...
 */
SCRUB.AttackManager.prototype.update = function(gameState, queues, events)
{
	...
	const unexecutedAttacks = {
		[SCRUB.AttackPlan.TYPE_RUSH]: 0,
		[SCRUB.AttackPlan.TYPE_RAID]: 0,
		[SCRUB.AttackPlan.TYPE_DEFAULT]: 0
	};
	for (let attackType in this.upcomingAttacks)
	{
		for (let i = 0; i < this.upcomingAttacks[attackType].length; ++i)
		{
			...
			if (attack.isStarted())
				API3.warn("SCRUB problem in attackManager: attack in preparation has already started ???");

			let updateStep = attack.updatePreparation(gameState);
			// now we're gonna check if the preparation time is over
			if (updateStep === SCRUB.AttackPlan.PREPARATION_KEEP_GOING || attack.isPaused())
			{
				// just chillin'
				if (attack.state === SCRUB.AttackPlan.STATE_UNEXECUTED)
					++unexecutedAttacks[attackType];
			}
			else if (updateStep === SCRUB.AttackPlan.PREPARATION_FAILED)
			{
			...
			else if (updateStep === SCRUB.AttackPlan.PREPARATION_START)
			{
				if (attack.StartAttack(gameState))
				{
					...
					if (this.Config.chat)
						SCRUB.chatLaunchAttack(gameState, attack.targetPlayer, attack.getType());
	...
	{
		if (unexecutedAttacks[SCRUB.AttackPlan.TYPE_RUSH] === 0)
		{
			// we have a barracks and we want to rush, rush.
			let data = { "targetSize": this.rushSize[this.rushNumber] };
			const attackPlan = new SCRUB.AttackPlan(gameState, this.Config, this.totalNumber, SCRUB.AttackPlan.TYPE_RUSH, data);
			if (!attackPlan.failed)
			{
				...
				this.upcomingAttacks[SCRUB.AttackPlan.TYPE_RUSH].push(attackPlan);
			}
			this.rushNumber++;
		}
	}
	else if (unexecutedAttacks[SCRUB.AttackPlan.TYPE_DEFAULT] == 0 && unexecutedAttacks[SCRUB.AttackPlan.TYPE_RAID] == 0 &&
		this.startedAttacks[SCRUB.AttackPlan.TYPE_DEFAULT].length + this.startedAttacks[SCRUB.AttackPlan.TYPE_RAID].length <
			Math.min(2, 1 + Math.round(gameState.getPopulationMax() / 150)) &&
		(this.startedAttacks[SCRUB.AttackPlan.TYPE_DEFAULT].length + this.startedAttacks[SCRUB.AttackPlan.TYPE_RAID].length == 0 ||
		gameState.getPopulationMax() - gameState.getPopulation() > 15))
	{
		...
		{
			const type = this.attackNumber < 2 || this.startedAttacks[SCRUB.AttackPlan.TYPE_RAID].length > 0 ? SCRUB.AttackPlan.TYPE_DEFAULT : SCRUB.AttackPlan.TYPE_RAID;
			let attackPlan = new SCRUB.AttackPlan(gameState, this.Config, this.totalNumber, type);
			if (attackPlan.failed)
	...
	if (unexecutedAttacks[SCRUB.AttackPlan.TYPE_RAID] === 0 && gameState.ai.HQ.defenseManager.targetList.length)
	{
		let target;
	...
	if (this.Config.difficulty > SCRUB.DIFFICULTY_VERY_EASY && gameState.ai.playedTurn % 5 == 0)
		this.assignBombers(gameState);
};

SCRUB.AttackManager.prototype.getPlan = function(planName)
{
	for (let attackType in this.upcomingAttacks)
	{
		for (let attack of this.upcomingAttacks[attackType])
...
SCRUB.AttackManager.prototype.pausePlan = function(planName)
{
...
SCRUB.AttackManager.prototype.unpausePlan = function(planName)
{
	let attack = this.getPlan(planName);
	if (attack)
...
SCRUB.AttackManager.prototype.pauseAllPlans = function()
{
	for (let attackType in this.upcomingAttacks)
		for (let attack of this.upcomingAttacks[attackType])
			attack.setPaused(true);
...
SCRUB.AttackManager.prototype.unpauseAllPlans = function()
{
	for (let attackType in this.upcomingAttacks)
		for (let attack of this.upcomingAttacks[attackType])
			attack.setPaused(false);
...
SCRUB.AttackManager.prototype.getAttackInPreparation = function(type)
{
 ...
 * for future attacks.
 */
SCRUB.AttackManager.prototype.getEnemyPlayer = function(gameState, attack)
{
	let enemyPlayer;
	...
	if (attack.type === SCRUB.AttackPlan.TYPE_RUSH)
	{
	...
	if (attack.type !== SCRUB.AttackPlan.TYPE_RAID)
	{
		...
		for (let ourcc of ccEnts.values())
		{
			...
			let access = SCRUB.getLandAccess(gameState, ourcc);
			for (let enemycc of ccEnts.values())
			{
				...
				if (!gameState.isPlayerEnemy(enemycc.owner()))
					continue;
				if (access !== SCRUB.getLandAccess(gameState, enemycc))
					continue;
	...
	// then let's target our weakest enemy (basically counting enemies units)
 ...
 */
SCRUB.AttackManager.prototype.getWonderEnemyPlayer = function(gameState, attack)
{
	let enemyPlayer;
	let enemyWonder;
	let moreAdvanced;
 ...
 */
SCRUB.AttackManager.prototype.getRelicEnemyPlayer = function(gameState, attack)
{
	let enemyPlayer;
...
SCRUB.AttackManager.prototype.cancelAttacksAgainstPlayer = function(gameState, player)
{
	for (let attackType in this.upcomingAttacks)
		for (let attack of this.upcomingAttacks[attackType])
...
SCRUB.AttackManager.prototype.raidTargetEntity = function(gameState, ent)
{
	let data = { "target": ent };
	const attackPlan = new SCRUB.AttackPlan(gameState, this.Config, this.totalNumber, SCRUB.AttackPlan.TYPE_RAID, data);
	if (attackPlan.failed)
		return null;
	if (this.Config.debug > 1)
	...
	this.totalNumber++;
	attackPlan.init(gameState);
	this.upcomingAttacks[SCRUB.AttackPlan.TYPE_RAID].push(attackPlan);
	return attackPlan;
};
 ...
 */
SCRUB.AttackManager.prototype.numAttackingUnitsAround = function(pos, dist)
{
	let num = 0;
	for (let attackType in this.startedAttacks)
		for (let attack of this.startedAttacks[attackType])
 ...
 */
SCRUB.AttackManager.prototype.switchDefenseToAttack = function(gameState, target, data)
{
	...
	const attackType = SCRUB.AttackPlan.TYPE_DEFAULT;
	let attackPlan = new SCRUB.AttackPlan(gameState, this.Config, this.totalNumber, attackType, attackData);
	if (attackPlan.failed)
		return false;
	this.totalNumber++;
	attackPlan.init(gameState);
	...
	for (let army of gameState.ai.HQ.defenseManager.armies)
	{
		...
		while (army.ownEntities.length > 0)
		{
			...
			let accessOk = unit.getMetadata(PlayerID, "transport") !== undefined ||
			               unit.position() && SCRUB.getLandAccess(gameState, unit) == targetAccess;
			if (unit && accessOk && attackPlan.isAvailableUnit(gameState, unit))
			{
				unit.setMetadata(PlayerID, "plan", attackPlan.name);
				unit.setMetadata(PlayerID, "role", SCRUB.Worker.ROLE_ATTACK);
	...
	attackPlan.state = SCRUB.AttackPlan.STATE_ARRIVED;
	return true;
};

SCRUB.AttackManager.prototype.Serialize = function()
{
	let properties = {
...
SCRUB.AttackManager.prototype.Deserialize = function(gameState, data)
{
	...
	for (let key in data.upcomingAttacks)
	{
		...
		{
			let attack = new SCRUB.AttackPlan(gameState, this.Config, dataAttack.properties.name);
