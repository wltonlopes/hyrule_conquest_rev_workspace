...
GORO.DIFFICULTY_SANDBOX = 0;
GORO.DIFFICULTY_VERY_EASY = 1;
GORO.DIFFICULTY_EASY = 2;
GORO.DIFFICULTY_MEDIUM = 3;
GORO.DIFFICULTY_HARD = 4;
GORO.DIFFICULTY_VERY_HARD = 5;

GORO.Config = function(difficulty = GORO.DIFFICULTY_MEDIUM, behavior)
{
	...
	// See GORO.QueueManager.prototype.wantedGatherRates()
	this.queues =
	{
		"firstTurn": {
			"wood": 40,
...
GORO.Config.prototype.setConfig = function(gameState)
{
	if (this.difficulty > GORO.DIFFICULTY_SANDBOX)
	{
	...
	// GORO usually uses the continuous values of personality.aggressive and personality.defensive
	...
	if (this.difficulty < GORO.DIFFICULTY_EASY)
	{
		this.popScaling = 0.5;
	...
	else if (this.difficulty < GORO.DIFFICULTY_MEDIUM)
	{
		this.popScaling = 0.7;
	...
	{
		if (this.difficulty == GORO.DIFFICULTY_MEDIUM)
	...
	if (this.debug < 2)
		return;
	API3.warn(" >>>  GORO bot: personality = " + uneval(this.personality));
};

GORO.Config.prototype.Cheat = function(gameState)
{
...
GORO.Config.prototype.Serialize = function()
{
	var data = {};
	for (let key in this)
...
GORO.Config.prototype.Deserialize = function(data)
{
	for (let key in data)
		this[key] = data[key];
};
