...
GORO.isFastMoving = function(ent)
{
...
GORO.getMaxStrength = function(ent, debugLevel, DamageTypeImportance, againstClass)
{
	let strength = 0;
	...
	for (let type of attackTypes)
	{
		...
		for (let str in attackStrength)
		{
			...
			if (DamageTypeImportance[str])
				...
				API3.warn("GORO: " + str + " unknown attackStrength in getMaxStrength (please add " + str + "  to config.js).");
		}
		...
		for (let str in attackTimes)
		{
			...
			case "prepare":
				...
				break;
			default:
				API3.warn("GORO: " + str + " unknown attackTimes in getMaxStrength");
			}
		}
	}
	...
	if (resistanceStrength.Damage)
		for (let str in resistanceStrength.Damage)
		{
			...
			if (DamageTypeImportance[str])
				...
				API3.warn("GORO: " + str + " unknown resistanceStrength in getMaxStrength (please add " + str + "  to config.js).");
		}
...
GORO.getLandAccess = function(gameState, ent)
{
	if (ent.hasClass("Unit"))
	{
		...
		if (!pos)
		{
			let holder = GORO.getHolder(gameState, ent);
			if (holder)
				return GORO.getLandAccess(gameState, holder);

			API3.warn("GORO error: entity without position, but not garrisoned");
			GORO.dumpEntity(ent);
			return undefined;
		}
...
GORO.getSeaAccess = function(gameState, ent)
{
...
GORO.setSeaAccess = function(gameState, ent)
{
	GORO.getSeaAccess(gameState, ent);
};

/** Decide if we should try to capture (returns true) or destroy (return false) */
GORO.allowCapture = function(gameState, ent, target)
{
	...
	{
		capture = ent.captureStrength() * GORO.getAttackBonus(ent, target, "Capture");
	...
	else
	{
		...
		{
			capturable.strength += ent.captureStrength() * GORO.getAttackBonus(ent, target, "Capture");
...
GORO.getAttackBonus = function(ent, target, type)
{
	let attackBonus = 1;
...
GORO.returnResources = function(gameState, ent)
{
	...
	for (let dropsite of dropsiteCollection.values())
	{
		...
		if (owner !== PlayerID && (!dropsite.isSharedDropsite() || !gameState.isPlayerMutualAlly(owner)))
			continue;
		if (GORO.getLandAccess(gameState, dropsite) != access)
			continue;
...
GORO.IsSupplyFull = function(gameState, ent)
{
 ...
 */
GORO.getBestBase = function(gameState, ent, onlyConstructedBase = false, exclude = false)
{
	let pos = ent.position();
	let accessIndex;
	if (!pos)
	{
		...
		pos = holder.position();
		accessIndex = GORO.getLandAccess(gameState, holder);
	}
	else
		accessIndex = GORO.getLandAccess(gameState, ent);
...
GORO.getHolder = function(gameState, ent)
{
...
GORO.getBuiltEntity = function(gameState, ent)
{
 ...
 */
GORO.isNotWorthBuilding = function(gameState, ent)
{
 ...
 */
GORO.isLineInsideEnemyTerritory = function(gameState, pos1, pos2, step=70)
{
...
GORO.gatherTreasure = function(gameState, ent, water = false)
{
	...
	let treasureFound;
	let distmin = Math.min();
	let access = water ? GORO.getSeaAccess(gameState, ent) : GORO.getLandAccess(gameState, ent);
	for (let treasure of gameState.ai.HQ.treasures.values())
	{
		...
		if (lastGathered && gameState.ai.elapsedTime - lastGathered < 20)
			continue;
		if (!water && access != GORO.getLandAccess(gameState, treasure))
			continue;
		if (water && access != GORO.getSeaAccess(gameState, treasure))
			continue;
