 ...
 */
GORO.NavalManager = function(Config)
{
	this.Config = Config;
...
GORO.NavalManager.prototype.init = function(gameState, deserializing)
{
	// docks
	this.docks = gameState.getOwnStructures().filter(API3.Filters.byClasses(["Dock", "Shipyard"]));
	this.docks.registerUpdates();

	this.ships = gameState.getOwnUnits().filter(API3.Filters.and(API3.Filters.byClass("Ship"), API3.Filters.not(API3.Filters.byMetadata(PlayerID, "role", GORO.Worker.ROLE_TRADER))));
	...
	for (let ship of this.ships.values())
		GORO.setSeaAccess(gameState, ship);
	for (let dock of this.docks.values())
		GORO.setSeaAccess(gameState, dock);
};

GORO.NavalManager.prototype.updateFishingBoats = function(sea, num)
{
	if (this.wantedFishShips[sea])
		this.wantedFishShips[sea] = num;
};

GORO.NavalManager.prototype.resetFishingBoats = function(gameState, sea)
{
	if (sea !== undefined)
...
GORO.NavalManager.prototype.getFishSea = function(gameState, fish)
{
...
GORO.NavalManager.prototype.canFishSafely = function(gameState, fish)
{
...
GORO.NavalManager.prototype.getUnconnectedSeas = function(gameState, region)
{
	let seas = gameState.ai.accessibility.regionLinks[region].slice();
	this.docks.forEach(dock => {
		if (!dock.hasClass("Dock") || GORO.getLandAccess(gameState, dock) != region)
			return;
		let i = seas.indexOf(GORO.getSeaAccess(gameState, dock));
		if (i != -1)
			seas.splice(i--, 1);
	});
	return seas;
...
GORO.NavalManager.prototype.checkEvents = function(gameState, queues, events)
{
	...
	for (let evt of events.Destroy)
	{
		...
		if (plan.state === GORO.TransportPlan.BOARDING)
		{
		...
		else if (plan.state === GORO.TransportPlan.SAILING)
		{
...
GORO.NavalManager.prototype.getPlan = function(ID)
{
...
GORO.NavalManager.prototype.addPlan = function(plan)
{
 ...
 */
GORO.NavalManager.prototype.requireTransport = function(gameState, ent, startIndex, endIndex, endPos)
{
	if (!ent.canGarrison())
		return false;
	...
	if (ent.getMetadata(PlayerID, "transport") !== undefined)
	{
		if (this.Config.debug > 0)
			API3.warn("GORO naval manager error: unit " + ent.id() + " has already required a transport");
		return false;
	}
	...
	for (let plan of this.transportPlans)
	{
		if (plan.startIndex != startIndex || plan.endIndex != endIndex || plan.state !== GORO.TransportPlan.BOARDING)
			continue;
		// Limit the number of siege units per transport to avoid problems when ungarrisoning
		if (GORO.isSiegeUnit(ent) && plan.units.filter(unit => GORO.isSiegeUnit(unit)).length > 3)
			continue;
		plans.push(plan);
	}
	...
	let plan = new GORO.TransportPlan(gameState, [ent], startIndex, endIndex, endPos);
	if (plan.failed)
	{
		if (this.Config.debug > 1)
...
GORO.NavalManager.prototype.splitTransport = function(gameState, plan)
{
	if (this.Config.debug > 1)
		API3.warn(">>>> split of transport plan started <<<<");
	let newplan = new GORO.TransportPlan(gameState, [], plan.startIndex, plan.endIndex, plan.endPos);
	if (newplan.failed)
	{
		if (this.Config.debug > 1)
 ...
 */
GORO.NavalManager.prototype.createTransportIfNeeded = function(gameState, fromPos, toPos, toAccess)
{
	...
	for (let ship of this.ships.values())
	{
		...
		let plan = new GORO.TransportPlan(gameState, units, fromAccess, toAccess, toPos, ship);
		if (plan.failed)
			continue;
		plan.init(gameState);
		this.transportPlans.push(plan);
...
GORO.NavalManager.prototype.setMinimalTransportShips = function(gameState, sea, number)
{
	if (!sea)
		return;
...
GORO.NavalManager.prototype.checkLevels = function(gameState, queues)
{
...
GORO.NavalManager.prototype.maintainFleet = function(gameState, queues)
{
	if (queues.ships.hasQueuedUnits())
		return;
	...
	for (let sea = 0; sea < this.seaShips.length; ++sea)
	{
		...
		if (this.seaTransportShips[sea].length < this.wantedTransportShips[sea])
		{
			...
			if (template)
			{
				queues.ships.addPlan(new GORO.TrainingPlan(gameState, template, { "sea": sea }, 1, 1));
				continue;
			}
		}
		...
		if (this.seaFishShips[sea].length < this.wantedFishShips[sea])
		{
			...
			if (template)
			{
				queues.ships.addPlan(new GORO.TrainingPlan(gameState, template, { "base": 0, "role": GORO.Worker.ROLE_WORKER, "sea": sea }, 1, 1));
				continue;
			}
		}
	}
...
GORO.NavalManager.prototype.assignShipsToPlans = function(gameState)
{
	for (let plan of this.transportPlans)
...
GORO.NavalManager.prototype.isShipBoarding = function(ship)
{
	if (!ship.position())
		return false;
 ...
 */
GORO.NavalManager.prototype.moveApart = function(gameState)
{
	let blockedShips = [];
	let blockedIds = [];
	...
	for (let ship of this.ships.values())
	{
		...
		else if (ship.isIdle())
		{
			...
			if (GORO.gatherTreasure(gameState, ship, true))
				continue;
			...
			{
				if (GORO.getSeaAccess(gameState, dock) != sea)
					continue;
	...
	for (let ship of gameState.ai.HQ.tradeManager.traders.filter(API3.Filters.byClass("Ship")).values())
	{
		...
		if (role === undefined || role !== GORO.Worker.ROLE_TRADER)	// already accounted before
			continue;
...
GORO.NavalManager.prototype.buildNavalStructures = function(gameState, queues)
{
			...
			gameState.ai.HQ.canBuild(gameState, "structures/{civ}/dock"))
		{
			...
			for (const base of gameState.ai.HQ.baseManagers())
			{
				...
				for (let sea of remaining)
				{
					...
					queues.dock.addPlan(new GORO.ConstructionPlan(gameState, "structures/{civ}/dock", { "land": wantedLand, "sea": sea }));
					dockStarted = true;
					break;
				}
			}
	...
	queues.militaryBuilding.addPlan(new GORO.ConstructionPlan(gameState, template, { "land": wantedLand, "sea": sea }));
};

/** goal can be either attack (choose ship with best arrowCount) or transport (choose ship with best capacity) */
GORO.NavalManager.prototype.getBestShip = function(gameState, sea, goal)
{
...
GORO.NavalManager.prototype.update = function(gameState, queues, events)
{
...
GORO.NavalManager.prototype.Serialize = function()
{
	let properties = {
...
GORO.NavalManager.prototype.Deserialize = function(gameState, data)
{
	...
	for (let i in data.transports)
	{
		...
		let plan = new GORO.TransportPlan(gameState, [], dataPlan.startIndex, dataPlan.endIndex, dataPlan.endPos);
