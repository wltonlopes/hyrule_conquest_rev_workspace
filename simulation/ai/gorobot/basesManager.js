...
GORO.BasesManager = function(Config)
{
	this.Config = Config;
...
GORO.BasesManager.prototype.init = function(gameState)
{
	...
	this.noBase = new GORO.BaseManager(gameState, this);
	this.noBase.init(gameState, GORO.BaseManager.STATE_WITH_ANCHOR);
	...
	for (const cc of gameState.getOwnStructures().filter(API3.Filters.byClass("CivCentre")).values())
		if (cc.foundationProgress() === undefined)
			this.createBase(gameState, cc, GORO.BaseManager.STATE_WITH_ANCHOR);
		else
			this.createBase(gameState, cc, GORO.BaseManager.STATE_UNCONSTRUCTED);
};

/**
 * Initialization needed after deserialization (only called when deserialising).
 */
GORO.BasesManager.prototype.postinit = function(gameState)
{
 ...
 */
GORO.BasesManager.prototype.createBase = function(gameState, ent, type = GORO.BaseManager.STATE_WITH_ANCHOR)
{
	const access = GORO.getLandAccess(gameState, ent);
	let newbase;
	for (const base of this.baseManagers)
	{
		if (base.accessIndex != access)
			continue;
		if (type !== GORO.BaseManager.STATE_ANCHORLESS && base.anchor)
			continue;
		if (type !== GORO.BaseManager.STATE_ANCHORLESS)
		{
	...
	if (!newbase)
	{
		newbase = new GORO.BaseManager(gameState, this);
...
GORO.BasesManager.prototype.checkEvents = function(gameState, events)
{
	let addBase = false;
	...
	for (const evt of events.EntityRenamed)
	{
		...
		if (ent.getMetadata(PlayerID, "role") === GORO.Worker.ROLE_WORKER)
		{
	...
	for (const evt of events.Create)
	{
		...
		if (ent.getMetadata(PlayerID, "base") == -1)	// Standard base around a cc
		{
			...
			const newbase = this.createBase(gameState, ent, GORO.BaseManager.STATE_UNCONSTRUCTED);
		...
		{
			const newbase = this.createBase(gameState, ent, GORO.BaseManager.STATE_ANCHORLESS);
			// Let's get a few units from other bases there to build this.
	...
	for (const evt of events.OwnershipChanged)
	{
		...
		{
			GORO.getBestBase(gameState, ent).assignEntity(gameState, ent);
			continue;
		}
		if (ent.hasClass("CivCentre"))   // build a new base around it
		{
			...
			if (ent.foundationProgress() !== undefined)
				newbase = this.createBase(gameState, ent, GORO.BaseManager.STATE_UNCONSTRUCTED);
			else
			{
				newbase = this.createBase(gameState, ent, GORO.BaseManager.STATE_CAPTURED);
				addBase = true;
			}
		...
		else
		{
			...
			if (!ent.decaying() && ent.resourceDropsiteTypes())
				base = this.createBase(gameState, ent, GORO.BaseManager.STATE_ANCHORLESS);
			else
				base = GORO.getBestBase(gameState, ent) || this.noBase;
	...
	for (const evt of events.TrainingFinished)
	{
		for (const entId of evt.entities)
		{
			...
			if (ent.getMetadata(PlayerID, "role") === GORO.Worker.ROLE_WORKER)
			{
				...
				if (ent.getMetadata(PlayerID, "base") === undefined)
				{
					base = GORO.getBestBase(gameState, ent);
					base.assignEntity(gameState, ent);
				}
				else
			...
			else if (ent.resourceSupplyType() && ent.position())
			{
				...
				{
					if (!dropsite.position() || GORO.getLandAccess(gameState, dropsite) != access)
						continue;
 ...
 */
GORO.BasesManager.prototype.bulkPickWorkers = function(gameState, baseRef, number)
{
 ...
 */
GORO.BasesManager.prototype.getTotalResourceLevel = function(gameState, resources = Resources.GetCodes(), proximity = ["nearby", "medium"])
{
	const total = {};
 ...
 */
GORO.BasesManager.prototype.GetCurrentGatherRates = function(gameState)
{
...
GORO.BasesManager.prototype.AddTCGatherer = function(supplyID)
{
...
GORO.BasesManager.prototype.RemoveTCGatherer = function(supplyID)
{
...
GORO.BasesManager.prototype.GetTCGatherer = function(supplyID)
{
	if (this.turnCache.resourceGatherer && this.turnCache.resourceGatherer[supplyID])
...
GORO.BasesManager.prototype.AddTCResGatherer = function(resource)
{
...
GORO.BasesManager.prototype.GetTCResGatherer = function(resource)
{
	const check = "resourceGatherer-" + resource;
	if (this.turnCache[check])
 ...
 */
GORO.BasesManager.prototype.isResourceExhausted = function(resource)
{
 ...
 */
GORO.BasesManager.prototype.numActiveBases = function()
{
...
GORO.BasesManager.prototype.hasActiveBase = function()
{
	return !!this.numActiveBases();
};

GORO.BasesManager.prototype.numPotentialBases = function()
{
	if (!this.turnCache.base)
		this.updateBaseCache();
...
GORO.BasesManager.prototype.hasPotentialBase = function()
{
 ...
 */
GORO.BasesManager.prototype.updateBaseCache = function()
{
...
GORO.BasesManager.prototype.resetBaseCache = function()
{
	this.turnCache.base = undefined;
};

GORO.BasesManager.prototype.baselessBase = function()
{
	return this.noBase;
};
 ...
 */
GORO.BasesManager.prototype.getBaseByID = function(baseID)
{
 ...
 */
GORO.BasesManager.prototype.assignGatherers = function()
{
	for (const base of this.baseManagers)
 ...
 */
GORO.BasesManager.prototype.assignEntity = function(gameState, ent, territoryIndex)
{
	let bestbase;
	for (const base of this.baseManagers)
	{
	...
	if (!bestbase)	// entity outside our territory
	{
		if (ent.hasClass("Structure") && !ent.decaying() && ent.resourceDropsiteTypes())
			bestbase = this.createBase(gameState, ent, GORO.BaseManager.STATE_ANCHORLESS);
		else
			bestbase = GORO.getBestBase(gameState, ent) || this.noBase;
 ...
 */
GORO.BasesManager.prototype.addGatherRates = function(gameState, rates)
{
	for (const base of this.baseManagers)
 ...
 */
GORO.BasesManager.prototype.baseAtIndex = function(territoryIndex)
{
 ...
 * @param {number} territoryIndex
 */
GORO.BasesManager.prototype.removeBaseFromTerritoryIndex = function(territoryIndex)
{
 ...
 */
GORO.BasesManager.prototype.addTerritoryIndexToBase = function(gameState, territoryIndex, passabilityMap)
{
...
GORO.BasesManager.prototype.reassignTerritories = function(deletedBase, territoryMap)
{
	...
	for (let j = 0; j < territoryMap.length; ++j)
	{
		...
		{
			API3.warn("GORO reassignTerritories: should never happen");
 ...
 */
GORO.BasesManager.prototype.update = function(gameState, queues, events)
{
...
GORO.BasesManager.prototype.Serialize = function()
{
	const properties = {
...
GORO.BasesManager.prototype.Deserialize = function(gameState, data)
{
	...
	for (const basedata of data.baseManagers)
	{
		...
		const newbase = new GORO.BaseManager(gameState, this);
		newbase.Deserialize(gameState, basedata);
		newbase.init(gameState, GORO.BaseManager.STATE_WITH_ANCHOR);
		newbase.Deserialize(gameState, basedata);
		this.baseManagers.push(newbase);
	}
};
