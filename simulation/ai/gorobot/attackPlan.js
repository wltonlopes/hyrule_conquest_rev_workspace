 ...
 */
GORO.AttackPlan = function(gameState, Config, uniqueID, type = GORO.AttackPlan.TYPE_DEFAULT, data)
{
	this.Config = Config;
	this.name = uniqueID;
	this.type = type;
	this.state = GORO.AttackPlan.STATE_UNEXECUTED;
	...
	for (const base of gameState.ai.HQ.baseManagers())
	{
		if (!base.anchor || !base.anchor.position())
			continue;
		let access = GORO.getLandAccess(gameState, base.anchor);
		if (!rallyPoint)
		{
	...
	if (!rallyPoint)	// no base ?  take the position of any of our entities
	{
		for (let ent of gameState.getOwnEntities().values())
		{
			if (!ent.position())
				continue;
			let access = GORO.getLandAccess(gameState, ent);
	...
	if (gameState.ai.HQ.navalMap)
	{
		for (let structure of gameState.getEnemyStructures().values())
		{
			...
			if (!structure.position())
				continue;
			let access = GORO.getLandAccess(gameState, structure);
			...
			else if (!this.overseas)
			{
				...
				if (!sea)
				{
					if (this.target)
					{
						API3.warn("GORO: " + this.type + " " + this.name + " has an inaccessible target " +
	...
	if (type === GORO.AttackPlan.TYPE_RUSH)
	{
		priority = 150;
	...
	else if (type === GORO.AttackPlan.TYPE_RAID)
	{
		priority = 250;
	...
	if (this.Config.difficulty < GORO.DIFFICULTY_EASY)
	{
		priority *= 0.4;
		variation *= 0.2;
	}
	else if (this.Config.difficulty < GORO.DIFFICULTY_MEDIUM)
	{
		priority *= 0.8;
		variation *= 0.6;
	}
	...
	this.siegeState = GORO.AttackPlan.SIEGE_NOT_TESTED;
...
GORO.AttackPlan.PREPARATION_FAILED = 0;
GORO.AttackPlan.PREPARATION_KEEP_GOING = 1;
GORO.AttackPlan.PREPARATION_START = 2;

GORO.AttackPlan.SIEGE_NOT_TESTED = 0;
GORO.AttackPlan.SIEGE_NO_TRAINER = 1;

/**
 * Siege added in build orders
 */
GORO.AttackPlan.SIEGE_ADDED = 2;

GORO.AttackPlan.STATE_UNEXECUTED = "unexecuted";
GORO.AttackPlan.STATE_COMPLETING = "completing";
GORO.AttackPlan.STATE_ARRIVED = "arrived";

GORO.AttackPlan.TYPE_DEFAULT = "Attack";
GORO.AttackPlan.TYPE_RAID = "Raid";
GORO.AttackPlan.TYPE_RUSH = "Rush";

GORO.AttackPlan.prototype.init = function(gameState)
{
...
GORO.AttackPlan.prototype.getName = function()
{
	return this.name;
};

GORO.AttackPlan.prototype.getType = function()
{
	return this.type;
};

GORO.AttackPlan.prototype.isStarted = function()
{
	return this.state !== GORO.AttackPlan.STATE_UNEXECUTED && this.state !== GORO.AttackPlan.STATE_COMPLETING;
};

GORO.AttackPlan.prototype.isPaused = function()
{
	return this.paused;
};

GORO.AttackPlan.prototype.setPaused = function(boolValue)
{
 ...
 */
GORO.AttackPlan.prototype.canStart = function()
{
	if (!this.canBuildUnits)
		return true;
...
GORO.AttackPlan.prototype.mustStart = function()
{
	if (this.isPaused())
		return false;
	...
	if (MinReachedEverywhere)
		return this.type === GORO.AttackPlan.TYPE_RAID && this.target && this.target.foundationProgress() &&
		                                             this.target.foundationProgress() > 50;
	return false;
};

GORO.AttackPlan.prototype.forceStart = function()
{
	for (let unitCat in this.unitStat)
	{
		let Unit = this.unitStat[unitCat];
...
GORO.AttackPlan.prototype.emptyQueues = function()
{
	this.queue.empty();
...
GORO.AttackPlan.prototype.removeQueues = function(gameState)
{
...
GORO.AttackPlan.prototype.addBuildOrder = function(gameState, name, unitStats, resetQueue)
{
	if (!this.isStarted())
	{
...
GORO.AttackPlan.prototype.addSiegeUnits = function(gameState)
{
	if (this.siegeState === GORO.AttackPlan.SIEGE_ADDED || this.state !== GORO.AttackPlan.STATE_UNEXECUTED)
		return false;
	...
	{
		this.siegeState = GORO.AttackPlan.SIEGE_NO_TRAINER;
		return false;
	}
	...
	this.siegeState = GORO.AttackPlan.SIEGE_ADDED;
	let targetSize;
	if (this.Config.difficulty < GORO.DIFFICULTY_MEDIUM)
		targetSize = this.type === GORO.AttackPlan.TYPE_R_ATTACK ? Math.max(this.Config.difficulty, 1) : Math.max(this.Config.difficulty - 1, 0);
	else
		targetSize = this.type === GORO.AttackPlan.TYPE_RUSH ? this.Config.difficulty + 1 : this.Config.difficulty - 1;
	targetSize = Math.max(Math.round(this.Config.popScaling * targetSize), this.type === GORO.AttackPlan.TYPE_RUSH ? 1 : 0);
	if (!targetSize)
		return true;
	// no minsize as we don't want the plan to fail at the last minute though.
...
GORO.AttackPlan.prototype.updatePreparation = function(gameState)
{
	...
	if (this.state === GORO.AttackPlan.STATE_COMPLETING)
	{
		...
		else
		{
			...
			if (this.waitingForTransport())
				return GORO.AttackPlan.PREPARATION_KEEP_GOING;
			...
			if (gameState.ai.elapsedTime < this.maxCompletingTime && this.hasForceOrder())
				return GORO.AttackPlan.PREPARATION_KEEP_GOING;
			return GORO.AttackPlan.PREPARATION_START;
		}
	}
	...
	if (this.type !== GORO.AttackPlan.TYPE_RAID || !this.forced)    // Forced Raids have special purposes (as relic capture)
		this.assignUnits(gameState);
	if (this.type !== GORO.AttackPlan.TYPE_RAID && gameState.ai.HQ.attackManager.getAttackInPreparation(GORO.AttackPlan.TYPE_RAID) !== undefined)
	...
	if (gameState.getPopulationMax() - gameState.getPopulation() < 5)
	{
		...
		else	// Abort the plan so that its units will be reassigned to other plans.
		{
			if (this.Config.debug > 1)
			{
				...
				API3.warn(" attacks upcoming: raid " + am.upcomingAttacks[GORO.AttackPlan.TYPE_RAID].length +
					  " rush " + am.upcomingAttacks[GORO.AttackPlan.TYPE_RUSH].length +
					  " attack " + am.upcomingAttacks[GORO.AttackPlan.TYPE_DEFAULT].length);
				API3.warn(" attacks started: raid " + am.startedAttacks[GORO.AttackPlan.TYPE_RAID].length +
					  " rush " + am.startedAttacks[GORO.AttackPlan.TYPE_RUSH].length +
					  " attack " + am.startedAttacks[GORO.AttackPlan.TYPE_DEFAULT].length);
			}
			return GORO.AttackPlan.PREPARATION_FAILED;
		}
	}
	...
	else
	{
		if (this.canBuildUnits)
		{
			...
			if (this.siegeState === GORO.AttackPlan.SIEGE_NOT_TESTED ||
				this.siegeState === GORO.AttackPlan.SIEGE_NO_TRAINER && gameState.ai.playedTurn % 5 == 0)
			...
			if (!this.buildOrders.length)
				return GORO.AttackPlan.PREPARATION_FAILED;	// will abort the plan
		}
		return GORO.AttackPlan.PREPARATION_KEEP_GOING;
	}

	// if we're here, it means we must start
	this.state = GORO.AttackPlan.STATE_COMPLETING;
	...
	if (this.type === GORO.AttackPlan.TYPE_RAID)
		this.maxCompletingTime = this.forced ? 0 : gameState.ai.elapsedTime + 20;
	else
	{
		if (this.type === GORO.AttackPlan.TYPE_RUSH || this.forced)
	...
	for (let ent of this.unitCollection.values())
	{
		...
		ent.setMetadata(PlayerID, "role", GORO.Worker.ROLE_ATTACK);
		ent.setMetadata(PlayerID, "subrole", GORO.Worker.SUBROLE_COMPLETING);
		let queued = false;
		if (ent.resourceCarrying() && ent.resourceCarrying().length)
			queued = GORO.returnResources(gameState, ent);
		let index = GORO.getLandAccess(gameState, ent);
		if (index == rallyIndex)
...
GORO.AttackPlan.prototype.trainMoreUnits = function(gameState)
{
	...
	if (firstOrder[0] < firstOrder[3].targetSize)
	{
		...
		if (queue.length() <= 5)
		{
			...
			else
			{
				...
				data.role = gameState.getTemplate(template).hasClass("CitizenSoldier") ? GORO.Worker.ROLE_WORKER : GORO.Worker.ROLE_ATTACK;
				let trainingPlan = new GORO.TrainingPlan(gameState, template, data, max, max);
...
GORO.AttackPlan.prototype.assignUnits = function(gameState)
{
	let plan = this.name;
	let added = false;
	...
	let num = 0;
	const numbase = {};
	let keep = this.type !== GORO.AttackPlan.TYPE_RUSH ?
		6 + 4 * gameState.getNumPlayerEnemies() + 8 * this.Config.personality.defensive : 8;
	keep = Math.round(this.Config.popScaling * keep);
	for (const ent of gameState.getOwnEntitiesByRole(GORO.Worker.ROLE_WORKER, true).values())
	{
		...
		{
			API3.warn("GORO problem ent without base ");
			GORO.dumpEntity(ent);
			continue;
		}
		if (num++ < keep || numbase[baseID] < 5)
			continue;
		if (this.type !== GORO.AttackPlan.TYPE_RUSH && ent.getMetadata(PlayerID, "subrole") !== GORO.Worker.SUBROLE_IDLE)
			continue;
		ent.setMetadata(PlayerID, "plan", plan);
		this.unitCollection.updateEnt(ent);
		added = true;
...
GORO.AttackPlan.prototype.isAvailableUnit = function(gameState, ent)
{
	if (!ent.position())
		return false;
...
GORO.AttackPlan.prototype.reassignFastUnit = function(gameState)
{
	for (let ent of this.unitCollection.values())
	{
		...
		if (!ent.hasClasses(["FastMoving", "CitizenSoldier"]))
			continue;
		const raid = gameState.ai.HQ.attackManager.getAttackInPreparation(GORO.AttackPlan.TYPE_RAID);
...
GORO.AttackPlan.prototype.chooseTarget = function(gameState)
{
	...
	let targetIndex = GORO.getLandAccess(gameState, this.target);
 ...
 */
GORO.AttackPlan.prototype.getNearestTarget = function(gameState, position, sameLand)
{
	...
	else
	{
		...
		else if (this.type === GORO.AttackPlan.TYPE_RUSH || this.type === GORO.AttackPlan.TYPE_DEFAULT)
		{
	...
	for (let ent of targets.values())
	{
		...
		if (this.type !== GORO.AttackPlan.TYPE_RUSH && this.type !== GORO.AttackPlan.TYPE_RAID && ent.hasClass("Field"))
			dist += 100000;
		if (dist < minDist)
		{
			minDist = dist;
 ...
 */
GORO.AttackPlan.prototype.defaultTargetFinder = function(gameState, playerEnemy)
{
...
GORO.AttackPlan.prototype.isValidTarget = function(ent)
{
	if (!ent.position())
		return false;
	if (this.sameLand && GORO.getLandAccess(this.gameState, ent) != this.sameLand)
		return false;
...
GORO.AttackPlan.prototype.rushTargetFinder = function(gameState, playerEnemy)
{
	let targets = new API3.EntityCollection(gameState.sharedScript);
	let buildings;
	...
	if (!targets.hasEntities() && this.type === GORO.AttackPlan.TYPE_RUSH && playerEnemy)
...
GORO.AttackPlan.prototype.raidTargetFinder = function(gameState)
{
	let targets = new API3.EntityCollection(gameState.sharedScript);
 ...
 */
GORO.AttackPlan.prototype.checkTargetObstruction = function(gameState, target, position)
{
	if (GORO.getLandAccess(gameState, target) != gameState.ai.accessibility.getAccessValue(position))
		return target;
...
GORO.AttackPlan.prototype.getPathToTarget = function(gameState, fixedRallyPoint = false)
{
	let startAccess = gameState.ai.accessibility.getAccessValue(this.rallyPoint);
	let endAccess = GORO.getLandAccess(gameState, this.target);
...
GORO.AttackPlan.prototype.setRallyPoint = function(gameState)
{
 ...
 */
GORO.AttackPlan.prototype.StartAttack = function(gameState)
{
	if (this.Config.debug > 1)
	...
	for (let ent of this.unitCollection.values())
	{
		ent.setMetadata(PlayerID, "subrole", GORO.Worker.SUBROLE_WALKING);
	...
	let targetAccess = GORO.getLandAccess(gameState, this.target);
...
GORO.AttackPlan.prototype.update = function(gameState, events)
{
	...
	if (this.state === GORO.AttackPlan.STATE_ARRIVED)
	{
		...
		this.unitCollection.forEach(ent => {
			ent.stopMoving();
			ent.setMetadata(PlayerID, "subrole", GORO.Worker.SUBROLE_ATTACKING);
		});
		if (this.type === GORO.AttackPlan.TYPE_RUSH)   // try to find a better target for rush
		{
	...
	if (this.state == "")
	{
		...
		for (let evt of events.Attacked)
		{
			...
			{	// if our siege units are attacked, we'll send some units to deal with enemies.
				...
				{
					if (GORO.isSiegeUnit(ent))	// needed as mauryan elephants are not filtered out
						continue;

					let allowCapture = GORO.allowCapture(gameState, ent, attacker);
				...
				if (GORO.isSiegeUnit(attacker) && attacker.hasClass("Melee") && ourUnit.hasClass("Melee") && ourUnit.canAttackTarget(attacker, GORO.allowCapture(gameState, ourUnit, attacker)))
				{
					ourUnit.attack(attacker.id(), GORO.allowCapture(gameState, ourUnit, attacker));
			...
			else
			{
				...
				else if (GORO.isSiegeUnit(attacker))
				...
				else
				{
					...
					{
						let allowCapture = GORO.allowCapture(gameState, ent, attacker);
						if (GORO.isSiegeUnit(ent) || !ent.canAttackTarget(attacker, allowCapture))
							continue;
					...
					let allowCapture = GORO.allowCapture(gameState, ourUnit, attacker);
		...
		for (let ent of this.unitCollection.values())
		{
			...
			{
				if (GORO.isSiegeUnit(target) || target.hasClass("Hero"))
		...
		if (this.type === GORO.AttackPlan.TYPE_RUSH)
		...
		for (let check = 0; check < lgth; check++)
		{
			...
			let siegeUnit = GORO.isSiegeUnit(ent);
			if (ent.isIdle())
				needsUpdate = true;
			...
			if (this.isBlocked)
			{
				...
				range += 30;
			range *= range;
			let entAccess = GORO.getLandAccess(gameState, ent);
			// Checking for gates if we're a siege unit.
			if (siegeUnit)
			{
				let mStruct = enemyStructures.filter(enemy => {
					if (!enemy.position() || !ent.canAttackTarget(enemy, GORO.allowCapture(gameState, ent, enemy)))
						return false;
					if (API3.SquareVectorDistance(enemy.position(), ent.position()) > range)
						return false;
					if (enemy.foundationProgress() == 0)
						return false;
					if (GORO.getLandAccess(gameState, enemy) != entAccess)
						return false;
					return true;
				}).toEntityArray();
				if (mStruct.length)
				{
					...
					if (mStruct[0].hasClass("Gate"))
						ent.attack(mStruct[0].id(), GORO.allowCapture(gameState, ent, mStruct[0]));
					else
					{
						let rand = randIntExclusive(0, mStruct.length * 0.2);
						ent.attack(mStruct[rand].id(), GORO.allowCapture(gameState, ent, mStruct[rand]));
					}
				}
				else
				{
			...
			else
			{
				...
				if (mUnit.length)
				{
					...
					ent.attack(mUnit[rand].id(), GORO.allowCapture(gameState, ent, mUnit[rand]));
				}
					...
					else if (needsUpdate)  // really nothing   let's try to help our nearest unit
					{
						...
						this.unitCollection.forEach(unit => {
							...
							if (dist > distmin)
								return;
							distmin = dist;
							if (!ent.canAttackTarget(target, GORO.allowCapture(gameState, ent, target)))
								return;
							attacker = target;
						});
						if (attacker)
							ent.attack(attacker.id(), GORO.allowCapture(gameState, ent, attacker));
					}
				}
			}
		}
...
GORO.AttackPlan.prototype.UpdateTransporting = function(gameState, events)
{
	let done = true;
	for (let ent of this.unitCollection.values())
	{
	...
	if (done)
	{
		this.state = GORO.AttackPlan.STATE_ARRIVED;
		return;
	}
...
GORO.AttackPlan.prototype.UpdateWalking = function(gameState, events)
{
...
GORO.AttackPlan.prototype.UpdateTarget = function(gameState)
{
	...
	if (this.target && this.target.hasClass("Unit"))
	{
		...
		if (!this.targetPos)
		{
			let holder = GORO.getHolder(gameState, this.target);
			...
			for (let attackType in attackManager.startedAttacks)
			{
				for (let attack of attackManager.startedAttacks[attackType])
				{
					...
					if (!attack.target || !gameState.getEntityById(attack.target.id()) ||
						...
						continue;
					if (accessIndex != GORO.getLandAccess(gameState, attack.target))
						continue;
...
/** reset any units */
GORO.AttackPlan.prototype.Abort = function(gameState)
{
	...
	if (this.unitCollection.hasEntities())
	{
		...
		if (this.isStarted())
		{
			...
			for (const base of gameState.ai.HQ.baseManagers())
			{
				if (!base.anchor || !base.anchor.position())
					continue;
				if (GORO.getLandAccess(gameState, base.anchor) != access)
					continue;
		...
		for (let ent of this.unitCollection.values())
		{
			if (ent.getMetadata(PlayerID, "role") === GORO.Worker.ROLE_ATTACK)
				ent.stopMoving();
			if (rallyPoint)
...
GORO.AttackPlan.prototype.removeUnit = function(ent, update)
{
	if (ent.getMetadata(PlayerID, "role") === GORO.Worker.ROLE_ATTACK)
	{
		if (ent.hasClass("CitizenSoldier"))
			ent.setMetadata(PlayerID, "role", GORO.Worker.ROLE_WORKER);
		else
...
GORO.AttackPlan.prototype.checkEvents = function(gameState, events)
{
	for (let evt of events.EntityRenamed)
	{
		if (!this.target || this.target.id() != evt.entity)
			continue;
		if (this.type === GORO.AttackPlan.TYPE_RAID && !this.isStarted())
			this.target = undefined;
		else
	...
	if (!this.overseas || this.state !== GORO.AttackPlan.STATE_UNEXECUTED)
		return;
...
GORO.AttackPlan.prototype.waitingForTransport = function()
{
	for (let ent of this.unitCollection.values())
		if (ent.getMetadata(PlayerID, "transport") !== undefined)
			return true;
...
GORO.AttackPlan.prototype.hasSiegeUnits = function()
{
	for (let ent of this.unitCollection.values())
		if (GORO.isSiegeUnit(ent))
			return true;
	return false;
};

GORO.AttackPlan.prototype.hasForceOrder = function(data, value)
{
	for (let ent of this.unitCollection.values())
	{
 ...
 */
GORO.AttackPlan.prototype.getAttackAccess = function(gameState)
{
	for (let ent of this.unitCollection.filterNearest(this.position, 1).values())
		return GORO.getLandAccess(gameState, ent);

	return 0;
};

GORO.AttackPlan.prototype.debugAttack = function()
{
...
GORO.AttackPlan.prototype.Serialize = function()
{
	let properties = {
		"name": this.name,
		"type": this.type,
...
GORO.AttackPlan.prototype.Deserialize = function(gameState, data)
{
