GORO.DefenseManager = function(Config)
{
...
GORO.DefenseManager.prototype.update = function(gameState, events)
{
...
GORO.DefenseManager.prototype.makeIntoArmy = function(gameState, entityID, type = "default")
{
	if (type == "default")
	{
	...
	this.armies.push(new GORO.DefenseArmy(gameState, [entityID], type));
};

GORO.DefenseManager.prototype.getArmy = function(partOfArmy)
{
	return this.armies.find(army => army.ID == partOfArmy);
};

GORO.DefenseManager.prototype.isDangerous = function(gameState, entity)
{
	if (!entity.position())
		return false;
...
GORO.DefenseManager.prototype.checkEnemyUnits = function(gameState)
{
...
GORO.DefenseManager.prototype.checkEnemyArmies = function(gameState)
{
...
GORO.DefenseManager.prototype.assignDefenders = function(gameState)
{
	if (!this.armies.length)
		return;
	...
	for (let army of this.armies)
	{
		...
		for (let entId of army.foeEntities)
		{
			...
			if (!ent || !ent.position())
				continue;
			armyAccess = GORO.getLandAccess(gameState, ent);
			break;
		}
		if (!armyAccess)
			API3.warn(" GORO error: attacking army " + army.ID + " without access");
		army.recalculatePosition(gameState);
	...
	gameState.getOwnUnits().forEach(function(ent) {
		...
		if (ent.getMetadata(PlayerID, "plan") !== undefined && ent.getMetadata(PlayerID, "plan") != -1)
		{
			...
			if (subrole &&
				(subrole === GORO.Worker.SUBROLE_COMPLETING || subrole === GORO.Worker.SUBROLE_WALKING || subrole === GORO.Worker.SUBROLE_ATTACKING))
				return;
		}
	...
	for (let ipass = 0; ipass < 2; ++ipass)
	{
		...
		for (let i = 0; i < potentialDefenders.length; ++i)
		{
			...
			let aMin;
			let distMin;
			let access = ipass == 0 ? GORO.getLandAccess(gameState, ent) : undefined;
			for (let a = 0; a < armiesNeeding.length; ++a)
			{
				...
				if (!armiesNeeding[a].army.foeEntities.some(eEnt => {
					...
					return ent.canAttackTarget(eEntID, GORO.allowCapture(gameState, ent, eEntID));
					}))
					continue;
			...
			armiesNeeding[aMin].need -= GORO.getMaxStrength(ent, this.Config.debug, this.Config.DamageTypeImportance);
...
GORO.DefenseManager.prototype.abortArmy = function(gameState, army)
{
	army.clear(gameState);
	for (let i = 0; i < this.armies.length; ++i)
	{
 ...
 */
GORO.DefenseManager.prototype.checkEvents = function(gameState, events)
{
		...
		if (target.hasClass("Unit") && attacker.hasClass("Unit"))
		{
			...
			if (currentTarget)
			{
				...
				if (unitAIStateOrder === "REPAIR" && currentTarget.hasDefensiveFire())
					continue;
				if (unitAIStateOrder === "COMBAT" && !GORO.isSiegeUnit(currentTarget) &&
				    gameState.ai.HQ.capturableTargets.has(orderData[0].target))
				{
					...
					for (let entId of capturableTarget.ents)
					{
						if (allAttacked[entId])
							continue;
						let ent = gameState.getEntityById(entId);
						if (!ent || !ent.position() || !ent.canAttackTarget(attacker, GORO.allowCapture(gameState, ent, attacker)))
							continue;
					...
					if (minEnt)
					{
						...
						minEnt.attack(attacker.id(), GORO.allowCapture(gameState, minEnt, attacker));
					}
				}
			}
			let allowCapture = GORO.allowCapture(gameState, target, attacker);
...
GORO.DefenseManager.prototype.garrisonUnitsInside = function(gameState, target, data)
{
	...
	let access = GORO.getLandAccess(gameState, target);
	let garrisonManager = gameState.ai.HQ.garrisonManager;
	let garrisonArrowClasses = target.getGarrisonArrowClasses();
	const typeGarrison = data.type || GORO.GarrisonManager.TYPE_PROTECTION;
	...
	if (allowMelee === undefined)
	{
		...
		if (data.attacker)
			allowMelee = data.attacker.hasClass("Structure") ? data.attacker.attackRange("Ranged") : !GORO.isSiegeUnit(data.attacker);
		else
			allowMelee = true;
	}
	let units = gameState.getOwnUnits().filter(ent => {
		...
		if (!ent.hasClasses(garrisonArrowClasses))
			return false;
		if (typeGarrison !== GORO.GarrisonManager.TYPE_DECAY && !allowMelee && ent.attackTypes().indexOf("Melee") != -1)
			return false;
		if (ent.getMetadata(PlayerID, "transport") !== undefined)
			return false;
		...
		if (ent.getMetadata(PlayerID, "plan") !== undefined && ent.getMetadata(PlayerID, "plan") >= 0)
		{
			...
			if (typeGarrison !== GORO.GarrisonManager.TYPE_DECAY && subrole &&
				(subrole === GORO.Worker.SUBROLE_COMPLETING || subrole === GORO.Worker.SUBROLE_WALKING || subrole === GORO.Worker.SUBROLE_ATTACKING))
				return false;
		}
		if (GORO.getLandAccess(gameState, ent) != access)
			return false;
		return true;
...
GORO.DefenseManager.prototype.garrisonSiegeUnit = function(gameState, unit)
{
	let distmin = Math.min();
	let nearest;
	let unitAccess = GORO.getLandAccess(gameState, unit);
	let garrisonManager = gameState.ai.HQ.garrisonManager;
	for (let ent of gameState.getAllyStructures().values())
	{
		...
		if (ent.hitpoints() < ent.garrisonEjectHealth() * ent.maxHitpoints())
			continue;
		if (GORO.getLandAccess(gameState, ent) != unitAccess)
			continue;
	...
	if (nearest)
		garrisonManager.garrison(gameState, unit, nearest, GORO.GarrisonManager.TYPE_PROTECTION);
 ...
 */
GORO.DefenseManager.prototype.garrisonAttackedUnit = function(gameState, unit, emergency = false)
{
	let distmin = Math.min();
	let nearest;
	let unitAccess = GORO.getLandAccess(gameState, unit);
	...
	garrisonManager.garrison(gameState, unit, nearest, nearest.buffHeal() ? GORO.GarrisonManager.TYPE_PROTECTION : GORO.GarrisonManager.TYPE_EMERGENCY);
	return true;
};
 ...
 */
GORO.DefenseManager.prototype.GetCooperationLevel = function(ally)
{
 ...
 */
GORO.DefenseManager.prototype.switchToAttack = function(gameState, army)
{
	if (!army)
		return;
	for (let targetId of this.targetList)
	{
		...
		{
			let ent = gameState.getEntityById(entId);
			if (!ent || !ent.position() || GORO.getLandAccess(gameState, ent) != targetAccess)
				continue;
...
GORO.DefenseManager.prototype.Serialize = function()
{
	let properties = {
...
GORO.DefenseManager.prototype.Deserialize = function(gameState, data)
{
	...
	{
		let army = new GORO.DefenseArmy(gameState, []);
