 ...
 * Manage the trade
 */
GORO.TradeManager = function(Config)
{
	this.Config = Config;
...
GORO.TradeManager.prototype.init = function(gameState)
{
	this.traders = gameState.getOwnUnits().filter(API3.Filters.byMetadata(PlayerID, "role", GORO.Worker.ROLE_TRADER));
	this.traders.registerUpdates();
	this.minimalGain = gameState.ai.HQ.navalMap ? 3 : 5;
};

GORO.TradeManager.prototype.hasTradeRoute = function()
{
	return this.tradeRoute !== undefined;
};

GORO.TradeManager.prototype.assignTrader = function(ent)
{
	ent.setMetadata(PlayerID, "role", GORO.Worker.ROLE_TRADER);
	this.traders.updateEnt(ent);
};

GORO.TradeManager.prototype.trainMoreTraders = function(gameState, queues)
{
	...
	gameState.getOwnTrainingFacilities().forEach(function(ent) {
		...
		{
			if (!item.metadata || !item.metadata.role || item.metadata.role !== GORO.Worker.ROLE_TRADER)
				continue;
	...
	let template;
	const metadata = { "role": GORO.Worker.ROLE_TRADER };
	if (this.tradeRoute.sea)
	{
		...
		gameState.ai.HQ.navalManager.seaTransportShips[this.tradeRoute.sea].forEach(function(ship) {
			if (already || !ship.hasClass("Trader"))
				return;
			if (ship.getMetadata(PlayerID, "role") === GORO.Worker.ROLE_SWITCH_TO_TRADER)
			{
				already = true;
				return;
			}
		...
		if (shipToSwitch)
		{
			if (shipToSwitch.getMetadata(PlayerID, "transporter") === undefined)
				shipToSwitch.setMetadata(PlayerID, "role", GORO.Worker.ROLE_TRADER);
			else
				shipToSwitch.setMetadata(PlayerID, "role", GORO.Worker.ROLE_SWITCH_TO_TRADER);
			return;
		}
	...
	if (!gameState.getTemplate(template))
	{
		if (this.Config.debug > 0)
			API3.warn("GORO error: trying to train " + template + " for civ " +
			          gameState.getPlayerCiv() + " but no template found.");
		return;
	}
	queues.trader.addPlan(new GORO.TrainingPlan(gameState, template, metadata, 1, 1));
};

GORO.TradeManager.prototype.updateTrader = function(gameState, ent)
{
	if (ent.hasClass("Ship") && gameState.ai.playedTurn % 5 == 0 &&
	    !ent.unitAIState().startsWith("INDIVIDUAL.COLLECTTREASURE") &&
	    GORO.gatherTreasure(gameState, ent, true))
		return;
	...
	let access = ent.hasClass("Ship") ? GORO.getSeaAccess(gameState, ent) : GORO.getLandAccess(gameState, ent);
...
GORO.TradeManager.prototype.setTradingGoods = function(gameState)
{
 ...
 */
GORO.TradeManager.prototype.performBarter = function(gameState)
{
...
GORO.TradeManager.prototype.checkEvents = function(gameState, events)
{
...
GORO.TradeManager.prototype.activateProspection = function(gameState)
{
 ...
 */
GORO.TradeManager.prototype.checkRoutes = function(gameState, accessIndex)
{
	...
	for (let m1 of market1.values())
	{
		if (!m1.position())
			continue;
		let access1 = GORO.getLandAccess(gameState, m1);
		let sea1 = m1.hasClass("Naval") ? GORO.getSeaAccess(gameState, m1) : undefined;
		for (let m2 of market2.values())
		{
			...
			if (!m2.position())
				continue;
			let access2 = GORO.getLandAccess(gameState, m2);
			let sea2 = m2.hasClass("Naval") ? GORO.getSeaAccess(gameState, m2) : undefined;
			let land = access1 == access2 ? access1 : undefined;
			let sea = sea1 && sea1 == sea2 ? sea1 : undefined;
			if (!land && !sea)
				continue;
			if (land && GORO.isLineInsideEnemyTerritory(gameState, m1.position(), m2.position()))
				continue;
			let gainMultiplier;
	...
	if (this.Config.chat)
	{
		...
		{	// Warn an ally that we have a trade route with him
			GORO.chatNewTradeRoute(gameState, owner);
...
GORO.TradeManager.prototype.checkTrader = function(gameState, ent)
{
	    ...
	    possibleRoute.target.id() != presentRoute.source && possibleRoute.target.id() != presentRoute.target)
	{
		...
		{
			let closestBase = GORO.getBestBase(gameState, ent, true);
...
GORO.TradeManager.prototype.prospectForNewMarket = function(gameState, queues)
{
...
GORO.TradeManager.prototype.isNewMarketWorth = function(expectedGain)
{
	if (!Resources.GetTradableCodes().length)
		return false;
...
GORO.TradeManager.prototype.update = function(gameState, events, queues)
{
	if (gameState.ai.HQ.canBarter && Resources.GetBarterableCodes().length)
		this.performBarter(gameState);

	if (this.Config.difficulty <= GORO.DIFFICULTY_VERY_EASY)
		return;
...
GORO.TradeManager.prototype.routeEntToId = function(route)
{
	if (!route)
		return undefined;
...
GORO.TradeManager.prototype.routeIdToEnt = function(gameState, route)
{
	if (!route)
		return undefined;
...
GORO.TradeManager.prototype.Serialize = function()
{
	return {
...
GORO.TradeManager.prototype.Deserialize = function(gameState, data)
{
	for (let key in data)
	{
