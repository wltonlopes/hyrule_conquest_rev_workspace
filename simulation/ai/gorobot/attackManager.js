 ...
 * Attack Manager
 */
GORO.AttackManager = function(Config)
{
	this.Config = Config;
	...
	this.upcomingAttacks = {
		[GORO.AttackPlan.TYPE_RUSH]: [],
		[GORO.AttackPlan.TYPE_RAID]: [],
		[GORO.AttackPlan.TYPE_DEFAULT]: []
	};
	this.startedAttacks = {
		[GORO.AttackPlan.TYPE_RUSH]: [],
		[GORO.AttackPlan.TYPE_RAID]: [],
...
GORO.AttackManager.prototype.checkEvents = function(gameState, events)
{
	...
	for (let evt of events.AttackRequest)
	{
		...
		for (let attackType in this.upcomingAttacks)
		{
			...
			{
				if (attack.state === GORO.AttackPlan.STATE_COMPLETING)
				{
	...
	if (targetPlayer !== undefined)
		GORO.chatAnswerRequestAttack(gameState, targetPlayer, answer, other);
 ...
 */
GORO.AttackManager.prototype.assignBombers = function(gameState)
{
	...
	for (let ent of bombers.values())
	{
		...
		if (ent.getMetadata(PlayerID, "plan") !== undefined && ent.getMetadata(PlayerID, "plan") != -1)
		{
			...
			if (subrole && (subrole === GORO.Worker.SUBROLE_COMPLETING || subrole === GORO.Worker.SUBROLE_WALKING || subrole === GORO.Worker.SUBROLE_ATTACKING))
				continue;
		}
		...
		{
			if (!ent.canAttackTarget(struct, GORO.allowCapture(gameState, ent, struct)))
				continue;
 ...
 */
GORO.AttackManager.prototype.update = function(gameState, queues, events)
{
	...
	const unexecutedAttacks = {
		[GORO.AttackPlan.TYPE_RUSH]: 0,
		[GORO.AttackPlan.TYPE_RAID]: 0,
		[GORO.AttackPlan.TYPE_DEFAULT]: 0
	};
	for (let attackType in this.upcomingAttacks)
	{
		for (let i = 0; i < this.upcomingAttacks[attackType].length; ++i)
		{
			...
			if (attack.isStarted())
				API3.warn("GORO problem in attackManager: attack in preparation has already started ???");

			let updateStep = attack.updatePreparation(gameState);
			// now we're gonna check if the preparation time is over
			if (updateStep === GORO.AttackPlan.PREPARATION_KEEP_GOING || attack.isPaused())
			{
				// just chillin'
				if (attack.state === GORO.AttackPlan.STATE_UNEXECUTED)
					++unexecutedAttacks[attackType];
			}
			else if (updateStep === GORO.AttackPlan.PREPARATION_FAILED)
			{
			...
			else if (updateStep === GORO.AttackPlan.PREPARATION_START)
			{
				if (attack.StartAttack(gameState))
				{
					...
					if (this.Config.chat)
						GORO.chatLaunchAttack(gameState, attack.targetPlayer, attack.getType());
	...
	{
		if (unexecutedAttacks[GORO.AttackPlan.TYPE_RUSH] === 0)
		{
			// we have a barracks and we want to rush, rush.
			let data = { "targetSize": this.rushSize[this.rushNumber] };
			const attackPlan = new GORO.AttackPlan(gameState, this.Config, this.totalNumber, GORO.AttackPlan.TYPE_RUSH, data);
			if (!attackPlan.failed)
			{
				...
				this.upcomingAttacks[GORO.AttackPlan.TYPE_RUSH].push(attackPlan);
			}
			this.rushNumber++;
		}
	}
	else if (unexecutedAttacks[GORO.AttackPlan.TYPE_DEFAULT] == 0 && unexecutedAttacks[GORO.AttackPlan.TYPE_RAID] == 0 &&
		this.startedAttacks[GORO.AttackPlan.TYPE_DEFAULT].length + this.startedAttacks[GORO.AttackPlan.TYPE_RAID].length <
			Math.min(2, 1 + Math.round(gameState.getPopulationMax() / 150)) &&
		(this.startedAttacks[GORO.AttackPlan.TYPE_DEFAULT].length + this.startedAttacks[GORO.AttackPlan.TYPE_RAID].length == 0 ||
		gameState.getPopulationMax() - gameState.getPopulation() > 15))
	{
		...
		{
			const type = this.attackNumber < 2 || this.startedAttacks[GORO.AttackPlan.TYPE_RAID].length > 0 ? GORO.AttackPlan.TYPE_DEFAULT : GORO.AttackPlan.TYPE_RAID;
			let attackPlan = new GORO.AttackPlan(gameState, this.Config, this.totalNumber, type);
			if (attackPlan.failed)
	...
	if (unexecutedAttacks[GORO.AttackPlan.TYPE_RAID] === 0 && gameState.ai.HQ.defenseManager.targetList.length)
	{
		let target;
	...
	if (this.Config.difficulty > GORO.DIFFICULTY_VERY_EASY && gameState.ai.playedTurn % 5 == 0)
		this.assignBombers(gameState);
};

GORO.AttackManager.prototype.getPlan = function(planName)
{
	for (let attackType in this.upcomingAttacks)
	{
		for (let attack of this.upcomingAttacks[attackType])
...
GORO.AttackManager.prototype.pausePlan = function(planName)
{
...
GORO.AttackManager.prototype.unpausePlan = function(planName)
{
	let attack = this.getPlan(planName);
	if (attack)
...
GORO.AttackManager.prototype.pauseAllPlans = function()
{
	for (let attackType in this.upcomingAttacks)
		for (let attack of this.upcomingAttacks[attackType])
			attack.setPaused(true);
...
GORO.AttackManager.prototype.unpauseAllPlans = function()
{
	for (let attackType in this.upcomingAttacks)
		for (let attack of this.upcomingAttacks[attackType])
			attack.setPaused(false);
...
GORO.AttackManager.prototype.getAttackInPreparation = function(type)
{
 ...
 * for future attacks.
 */
GORO.AttackManager.prototype.getEnemyPlayer = function(gameState, attack)
{
	let enemyPlayer;
	...
	if (attack.type === GORO.AttackPlan.TYPE_RUSH)
	{
	...
	if (attack.type !== GORO.AttackPlan.TYPE_RAID)
	{
		...
		for (let ourcc of ccEnts.values())
		{
			...
			let access = GORO.getLandAccess(gameState, ourcc);
			for (let enemycc of ccEnts.values())
			{
				...
				if (!gameState.isPlayerEnemy(enemycc.owner()))
					continue;
				if (access !== GORO.getLandAccess(gameState, enemycc))
					continue;
 ...
 */
GORO.AttackManager.prototype.getWonderEnemyPlayer = function(gameState, attack)
{
	let enemyPlayer;
	let enemyWonder;
	let moreAdvanced;
 ...
 */
GORO.AttackManager.prototype.getRelicEnemyPlayer = function(gameState, attack)
{
	let enemyPlayer;
...
GORO.AttackManager.prototype.cancelAttacksAgainstPlayer = function(gameState, player)
{
	for (let attackType in this.upcomingAttacks)
		for (let attack of this.upcomingAttacks[attackType])
...
GORO.AttackManager.prototype.raidTargetEntity = function(gameState, ent)
{
	let data = { "target": ent };
	const attackPlan = new GORO.AttackPlan(gameState, this.Config, this.totalNumber, GORO.AttackPlan.TYPE_RAID, data);
	if (attackPlan.failed)
		return null;
	if (this.Config.debug > 1)
	...
	this.totalNumber++;
	attackPlan.init(gameState);
	this.upcomingAttacks[GORO.AttackPlan.TYPE_RAID].push(attackPlan);
	return attackPlan;
};
 ...
 */
GORO.AttackManager.prototype.numAttackingUnitsAround = function(pos, dist)
{
	let num = 0;
	for (let attackType in this.startedAttacks)
		for (let attack of this.startedAttacks[attackType])
 ...
 */
GORO.AttackManager.prototype.switchDefenseToAttack = function(gameState, target, data)
{
	...
	const attackType = GORO.AttackPlan.TYPE_DEFAULT;
	let attackPlan = new GORO.AttackPlan(gameState, this.Config, this.totalNumber, attackType, attackData);
	if (attackPlan.failed)
		return false;
	this.totalNumber++;
	attackPlan.init(gameState);
	...
	for (let army of gameState.ai.HQ.defenseManager.armies)
	{
		...
		while (army.ownEntities.length > 0)
		{
			...
			let accessOk = unit.getMetadata(PlayerID, "transport") !== undefined ||
			               unit.position() && GORO.getLandAccess(gameState, unit) == targetAccess;
			if (unit && accessOk && attackPlan.isAvailableUnit(gameState, unit))
			{
				unit.setMetadata(PlayerID, "plan", attackPlan.name);
				unit.setMetadata(PlayerID, "role", GORO.Worker.ROLE_ATTACK);
	...
	attackPlan.state = GORO.AttackPlan.STATE_ARRIVED;
	return true;
};

GORO.AttackManager.prototype.Serialize = function()
{
	let properties = {
...
GORO.AttackManager.prototype.Deserialize = function(gameState, data)
{
	...
	for (let key in data.upcomingAttacks)
	{
		...
		{
			let attack = new GORO.AttackPlan(gameState, this.Config, dataAttack.properties.name);
