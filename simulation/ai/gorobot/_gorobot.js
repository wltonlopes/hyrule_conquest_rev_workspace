...
GORO.GoroBot = function(settings)
{
	...
	this.Config = new GORO.Config(settings.difficulty, settings.behavior);

	this.savedEvents = {};
};

GORO.GoroBot.prototype = Object.create(API3.BaseAI.prototype);

GORO.GoroBot.prototype.CustomInit = function(gameState)
{
	...
	else
	{
		...
		for (let i in this.Config.priorities)
			this.queues[i] = new GORO.Queue();

		this.queueManager = new GORO.QueueManager(this.Config, this.queues);
...
GORO.GoroBot.prototype.OnUpdate = function(sharedScript)
{
	...
	{
		Engine.ProfileStart("GoroBot bot (player " + this.player +")");
...
GORO.GoroBot.prototype.Serialize = function()
{
	let savedEvents = {};
	for (let key in this.savedEvents)
	{
...
GORO.GoroBot.prototype.Deserialize = function(data, sharedScript)
{
