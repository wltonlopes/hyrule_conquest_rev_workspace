...
GORO.GarrisonManager = function(Config)
{
	this.Config = Config;
...
GORO.GarrisonManager.TYPE_FORCE = "force";
GORO.GarrisonManager.TYPE_TRADE = "trade";
GORO.GarrisonManager.TYPE_PROTECTION = "protection";
GORO.GarrisonManager.TYPE_DECAY = "decay";
GORO.GarrisonManager.TYPE_EMERGENCY = "emergency";

GORO.GarrisonManager.prototype.update = function(gameState, events)
{
		...
		for (let j = 0; j < list.length; ++j)
		{
			...
			else
			{
				...
				else
				{
					...
					{
						API3.warn("GORO garrison error: unit " + ent.id() + " (" + ent.genericName() +
	...
	for (let [id, gmin] of this.decayingStructures.entries())
	{
		...
		if (!ent || ent.owner() !== PlayerID)
			...
			gameState.ai.HQ.defenseManager.garrisonUnitsInside(gameState, ent, { "min": gmin, "type": GORO.GarrisonManager.TYPE_DECAY });
	}
};

/** TODO should add the units garrisoned inside garrisoned units */
GORO.GarrisonManager.prototype.numberOfGarrisonedUnits = function(holder)
{
...
/** TODO should add the units garrisoned inside garrisoned units */
GORO.GarrisonManager.prototype.numberOfGarrisonedSlots = function(holder)
{
	if (!this.holders.has(holder.id()))
...
GORO.GarrisonManager.prototype.allowMelee = function(holder)
{
	if (!this.holders.has(holder.id()))
		return undefined;
...
GORO.GarrisonManager.prototype.garrison = function(gameState, ent, holder, type)
{
	...
	ent.setMetadata(PlayerID, "subrole", GORO.Worker.SUBROLE_GARRISONING);
 ...
 */
GORO.GarrisonManager.prototype.leaveGarrison = function(ent)
{
...
GORO.GarrisonManager.prototype.cancelGarrison = function(ent)
{
	ent.stopMoving();
	this.leaveGarrison(ent);
...
GORO.GarrisonManager.prototype.keepGarrisoned = function(ent, holder, around)
{
	switch (ent.getMetadata(PlayerID, "garrisonType"))
	{
	case GORO.GarrisonManager.TYPE_FORCE:           // force the ungarrisoning
		return false;
	case GORO.GarrisonManager.TYPE_TRADE:		// trader garrisoned in ship
		return true;
	case GORO.GarrisonManager.TYPE_PROTECTION:	// hurt unit for healing or infantry for defense
		...
		if (ent.attackTypes() && ent.attackTypes().indexOf("Melee") !== -1)
			return false;
		if (around.unit)
			return ent.hasClass("Support") || GORO.isSiegeUnit(ent);	// only ranged siege here and below as melee siege already released above
		if (GORO.isSiegeUnit(ent))
			return around.meleeSiege;
		return holder.buffHeal() && ent.needsHeal();
	case GORO.GarrisonManager.TYPE_DECAY:
		return ent.captureStrength() && this.decayingStructures.has(holder.id());
	case GORO.GarrisonManager.TYPE_EMERGENCY: // f.e. hero in regicide mode
		if (holder.buffHeal() && ent.isHealable() && ent.healthLevel() < this.Config.garrisonHealthLevel.high)
			return true;
	...
	default:
		...
		ent.setMetadata(PlayerID, "garrisonType", GORO.GarrisonManager.TYPE_PROTECTION);
		return true;
	}
};

/** Add this holder in the list managed by the garrisonManager */
GORO.GarrisonManager.prototype.registerHolder = function(gameState, holder)
{
 ...
 */
GORO.GarrisonManager.prototype.addDecayingStructure = function(gameState, entId, justCaptured)
{
...
GORO.GarrisonManager.prototype.removeDecayingStructure = function(entId)
{
	if (!this.decayingStructures.has(entId))
		return;
...
GORO.GarrisonManager.prototype.Serialize = function()
{
	return { "holders": this.holders, "decayingStructures": this.decayingStructures };
};

GORO.GarrisonManager.prototype.Deserialize = function(data)
{
	for (let key in data)
		this[key] = data[key];
};
