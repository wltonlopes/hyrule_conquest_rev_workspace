 ...
 * Manage the research
 */
GORO.ResearchManager = function(Config)
{
	this.Config = Config;
};
 ...
 */
GORO.ResearchManager.prototype.checkPhase = function(gameState, queues)
{
	...
	if (GORORequirements && gameState.hasResearchers(nextPhaseName, true))
	{
		...
		queues.majorTech.addPlan(new GORO.ResearchPlan(gameState, nextPhaseName, true));
	}
};

GORO.ResearchManager.prototype.researchPopulationBonus = function(gameState, queues)
{
	...
	for (let tech of techs)
	{
		...
		if (tech[1]._template.modifications[0].value !== "Population/Bonus")
			continue;
		queues.minorTech.addPlan(new GORO.ResearchPlan(gameState, tech[0]));
		break;
	}
};

GORO.ResearchManager.prototype.researchTradeBonus = function(gameState, queues)
{
	if (queues.minorTech.hasQueuedUnits())
		return;
...
GORO.ResearchManager.prototype.researchWantedTechs = function(gameState, techs)
{
	let phase1 = gameState.currentPhase() === 1;
	let available = phase1 ? gameState.ai.queueManager.getAvailableResources(gameState) : null;
	const numWorkers = phase1 ? gameState.getOwnEntitiesByRole(GORO.Worker.ROLE_WORKER, true).length : 0;
	for (let tech of techs)
	{
...
GORO.ResearchManager.prototype.researchPreferredTechs = function(gameState, techs)
{
	let phase2 = gameState.currentPhase() === 2;
	let available = phase2 ? gameState.ai.queueManager.getAvailableResources(gameState) : null;
	const numWorkers = phase2 ? gameState.getOwnEntitiesByRole(GORO.Worker.ROLE_WORKER, true).length : 0;
	for (let tech of techs)
	{
		if (!tech[1]._template.modifications)
			continue;
...
GORO.ResearchManager.prototype.update = function(gameState, queues)
{
	...
	if (techName)
	{
		if (techName.increasePriority)
		{
			...
			let plan = new GORO.ResearchPlan(gameState, techName.name);
			plan.queueToReset = "minorTech";
			queues.minorTech.addPlan(plan);
		}
		else
			queues.minorTech.addPlan(new GORO.ResearchPlan(gameState, techName.name));
		return;
	}
	...
	queues.minorTech.addPlan(new GORO.ResearchPlan(gameState, pickRandom(techs)[0]));
};

GORO.ResearchManager.prototype.CostSum = function(cost)
{
	let costSum = 0;
	for (let res in cost)
		costSum += cost[res];
...
GORO.ResearchManager.prototype.Serialize = function()
{
	return {};
};

GORO.ResearchManager.prototype.Deserialize = function(data)
{
};
