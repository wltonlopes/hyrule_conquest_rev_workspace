...
GORO.TransportPlan = function(gameState, units, startIndex, endIndex, endPos, ship)
{
	...
	this.state = GORO.TransportPlan.BOARDING;
	this.boardingPos = {};
 ...
 */
GORO.TransportPlan.BOARDING = "boarding";
/**
 * We're moving ships and eventually unload units.
 */
GORO.TransportPlan.SAILING = "sailing";

GORO.TransportPlan.prototype.init = function(gameState)
{
...
GORO.TransportPlan.prototype.countFreeSlots = function()
{
	let slots = 0;
...
GORO.TransportPlan.prototype.countFreeSlotsOnShip = function(ship)
{
...
GORO.TransportPlan.prototype.assignUnitToShip = function(gameState, ent)
{
...
GORO.TransportPlan.prototype.assignShip = function(gameState)
{
	let pos;
...
GORO.TransportPlan.prototype.addUnit = function(unit, endPos)
{
...
GORO.TransportPlan.prototype.removeUnit = function(gameState, unit)
{
...
GORO.TransportPlan.prototype.releaseShip = function(ship)
{
	if (ship.getMetadata(PlayerID, "transporter") != this.ID)
	{
		API3.warn(" GORO: try removing a transporter ship with " + ship.getMetadata(PlayerID, "transporter") +
	...
	if (ship.getMetadata(PlayerID, "role") === GORO.Worker.ROLE_SWITCH_TO_TRADER)
		ship.setMetadata(PlayerID, "role", GORO.Worker.ROLE_TRADER);
};

GORO.TransportPlan.prototype.releaseAll = function()
{
...
GORO.TransportPlan.prototype.cancelTransport = function(gameState)
{
 ...
 */
GORO.TransportPlan.prototype.update = function(gameState)
{
	if (this.state === GORO.TransportPlan.BOARDING)
		this.onBoarding(gameState);
	else if (this.state === GORO.TransportPlan.SAILING)
		this.onSailing(gameState);

	return this.units.length;
};

GORO.TransportPlan.prototype.onBoarding = function(gameState)
{
	let ready = true;
	...
	this.state = GORO.TransportPlan.SAILING;
	this.nTry = {};
	this.unloaded = [];
	this.recovered = [];
};
...
GORO.TransportPlan.prototype.isOnBoard = function(ent)
{
	for (let ship of this.transportShips.values())
	{
...
GORO.TransportPlan.prototype.getBoardingPos = function(gameState, ship, landIndex, seaIndex, destination, avoidEnnemy)
{
...
GORO.TransportPlan.prototype.onSailing = function(gameState)
{
	...
	for (let entId of this.unloaded)
	{
		...
		else if (!ent.position())  // unloading failed
		{
			...
			if (ship)
			{
				...
				{
					API3.warn("GORO transportPlan problem: unit not on ship without position ???");
					this.resetUnit(gameState, ent);
					ent.destroy();
				}
			}
			...
			{
				API3.warn("GORO transportPlan problem: unit on ship, but no ship ???");
				this.resetUnit(gameState, ent);
				ent.destroy();
			}
		}
		else if (GORO.getLandAccess(gameState, ent) != this.endIndex)
		{
...
GORO.TransportPlan.prototype.resetUnit = function(gameState, ent)
{
	ent.setMetadata(PlayerID, "transport", undefined);
	ent.setMetadata(PlayerID, "onBoard", undefined);
	ent.setMetadata(PlayerID, "endPos", undefined);
...
GORO.TransportPlan.prototype.Serialize = function()
{
	return {
		"ID": this.ID,
...
GORO.TransportPlan.prototype.Deserialize = function(data)
{
	for (let key in data)
		this[key] = data[key];
