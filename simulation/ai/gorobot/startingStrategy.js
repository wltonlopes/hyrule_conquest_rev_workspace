...
GORO.HQ.prototype.gameAnalysis = function(gameState)
{
	...
	this.canExpand = this.Config.difficulty != GORO.DIFFICULTY_SANDBOX;
 ...
 */
GORO.HQ.prototype.assignStartingEntities = function(gameState)
{
 ...
 */
GORO.HQ.prototype.regionAnalysis = function(gameState)
{
 ...
 */
GORO.HQ.prototype.structureAnalysis = function(gameState)
{
 ...
 */
GORO.HQ.prototype.buildFirstBase = function(gameState)
{
	...
	if (!total.canAfford(new API3.Resources(template.cost())))
	{
		...
		for (let ent of gameState.getOwnUnits().values())
		{
			...
			if (ent.isIdle())
				GORO.gatherTreasure(gameState, ent);
	...
	if (goal == "dock")
	{
		...
		gameState.ai.queues.dock.addPlan(new GORO.ConstructionPlan(gameState, "structures/{civ}/dock", { "sea": sea, "proximity": startingPoint[imax].pos }));
	}
	else
		gameState.ai.queues.civilCentre.addPlan(new GORO.ConstructionPlan(gameState, "structures/{civ}/civil_centre", { "base": -1, "resource": "wood", "proximity": startingPoint[imax].pos }));
};
 ...
 */
GORO.HQ.prototype.dispatchUnits = function(gameState)
{
	...
	if (allycc.length)
	{
		...
		units.filter(API3.Filters.byClasses(["Infantry+Ranged"])).forEach(ent => {
			...
			{
				if (!cc.position() || GORO.getLandAccess(gameState, cc) != access)
					continue;
				--num;
				--num1;
 ...
 */
GORO.HQ.prototype.configFirstBase = function(gameState)
{
	if (!this.hasPotentialBase())
		return;
	...
	if (!gameState.getOwnEntitiesByClass("DropsiteWood", true).hasEntities())
	{
		...
		if (newDP.quality > 40 && this.canBuild(gameState, newDP.templateName))
		{
			...
			gameState.ai.queues.dropsites.addPlan(new GORO.ConstructionPlan(gameState, newDP.templateName, { "base": this.baseManagers()[0].ID }, newDP.pos));
		}
	}
	...
	if (this.needField)
	{
		...
		if (!gameState.getOwnEntitiesByClass("Field", true).hasEntities() && this.canBuild(gameState, template))
			gameState.ai.queues.field.addPlan(new GORO.ConstructionPlan(gameState, template, { "base": this.baseManagers()[0].ID }));
	}
};
