...
GORO.Queue.prototype.empty = function()
{
	this.plans = [];
};

GORO.Queue.prototype.addPlan = function(newPlan)
{
	if (!newPlan)
		return;
...
GORO.Queue.prototype.check= function(gameState)
{
...
GORO.Queue.prototype.getNext = function()
{
...
GORO.Queue.prototype.startNext = function(gameState)
{
	if (this.plans.length > 0)
	{
 ...
 */
GORO.Queue.prototype.maxAccountWanted = function(gameState, fraction)
{
...
GORO.Queue.prototype.queueCost = function()
{
	let cost = new API3.Resources();
	for (let plan of this.plans)
...
GORO.Queue.prototype.length = function()
{
	return this.plans.length;
};

GORO.Queue.prototype.hasQueuedUnits = function()
{
	return this.plans.length > 0;
};

GORO.Queue.prototype.countQueuedUnits = function()
{
	let count = 0;
	for (let plan of this.plans)
		count += plan.number;
...
GORO.Queue.prototype.hasQueuedUnitsWithClass = function(classe)
{
	return this.plans.some(plan => plan.template && plan.template.hasClass(classe));
};

GORO.Queue.prototype.countQueuedUnitsWithClass = function(classe)
{
	let count = 0;
	for (let plan of this.plans)
...
GORO.Queue.prototype.countQueuedUnitsWithMetadata = function(data, value)
{
	let count = 0;
	for (let plan of this.plans)
...
GORO.Queue.prototype.Serialize = function()
{
	let plans = [];
	for (let plan of this.plans)
...
GORO.Queue.prototype.Deserialize = function(gameState, data)
{
	...
	for (let dataPlan of data.plans)
	{
		...
		if (dataPlan.category == "unit")
			plan = new GORO.TrainingPlan(gameState, dataPlan.type);
		else if (dataPlan.category == "building")
			plan = new GORO.ConstructionPlan(gameState, dataPlan.type);
		else if (dataPlan.category == "technology")
			plan = new GORO.ResearchPlan(gameState, dataPlan.type);
		else
		{
			API3.warn("GORO deserialization error: plan unknown " + uneval(dataPlan));
			continue;
		}
