...
GORO.ConstructionPlan = function(gameState, type, metadata, position)
{
	if (!GORO.QueuePlan.call(this, gameState, type, metadata))
		return false;
...
GORO.ConstructionPlan.prototype = Object.create(GORO.QueuePlan.prototype);

GORO.ConstructionPlan.prototype.canStart = function(gameState)
{
...
GORO.ConstructionPlan.prototype.start = function(gameState)
{
	...
	if (!builder)
	{
		API3.warn("GORO error: builder not found when starting construction.");
		Engine.ProfileStop();
		return;
	}
...
GORO.ConstructionPlan.prototype.findGoodPosition = function(gameState)
{
	...
	else	// No position was specified so try and find a sensible place to build
	{
		...
		if (!HQ.requireHouses || !template.hasClass("House"))
		{
			gameState.getOwnStructures().forEach(function(ent) {
				...
				let struct = GORO.getBuiltEntity(gameState, ent);
		...
		if (template.hasClass("Farmstead"))
		{
			for (let j = 0; j < placement.map.length; ++j)
			{
				...
				if (HQ.borderMap.map[j] & GORO.fullBorder_Mask)
	...
	if (this.metadata && this.metadata.base !== undefined)
	{
		...
		for (let j = 0; j < placement.map.length; ++j)
		{
			...
			{
				if (favorBorder && HQ.borderMap.map[j] & GORO.border_Mask)
					placement.set(j, placement.map[j] + 50);
				else if (disfavorBorder && !(HQ.borderMap.map[j] & GORO.fullBorder_Mask))
 ...
 */
GORO.ConstructionPlan.prototype.findDockPosition = function(gameState)
{
	let template = this.template;
	...
	for (let j = 0; j < territoryMap.length; ++j)
	{
		...
		if (oversea)
		{
			...
			{
				if (GORO.getSeaAccess(gameState, dock) != navalPassMap[i])
					continue;
		...
		if (gameState.ai.HQ.borderMap.map[j] & GORO.fullBorder_Mask)
			score += 20;
 ...
 */
GORO.ConstructionPlan.prototype.buildOverseaDock = function(gameState, template)
{
	...
	for (let i = 0; i < gameState.ai.accessibility.regionSize.length; ++i)
	{
		...
		for (let dock of docks.values())
		{
			if (GORO.getLandAccess(gameState, dock) != i)
				continue;
			keep = false;
			break;
		}
		...
		for (let cc of ccEnts.values())
		{
			let ccAccess = GORO.getLandAccess(gameState, cc);
			if (ccAccess != i)
			{
	...
	if (!gameState.ai.HQ.navalMap)
		API3.warn("GORO.findOverseaLand on a non-naval map??? we should never go there ");
...
GORO.ConstructionPlan.prototype.getDockAngle = function(gameState, x, z, size)
{
 ...
 */
GORO.ConstructionPlan.prototype.checkDockPlacement = function(gameState, x, z, halfDepth, halfWidth, angle)
{
 ...
 */
GORO.ConstructionPlan.prototype.around = [[ 1.0, 0.0], [ 0.87, 0.50], [ 0.50, 0.87], [ 0.0, 1.0], [-0.50, 0.87], [-0.87, 0.50],
	        [-1.0, 0.0], [-0.87, -0.50], [-0.50, -0.87], [ 0.0, -1.0], [ 0.50, -0.87], [ 0.87, -0.50]];

GORO.ConstructionPlan.prototype.isDockLocation = function(gameState, j, dimension, wantedLand, wantedSea)
{
	let width = gameState.ai.HQ.territoryMap.width;
	let cellSize = gameState.ai.HQ.territoryMap.cellSize;
 ...
 */
GORO.ConstructionPlan.prototype.getFrontierProximity = function(gameState, j)
{
	...
	for (let a of this.around)
	{
		for (let i = 1; i < 5; ++i)
		{
			...
			if (jz < 0 || jz >= width)
				continue;
			if (borderMap.map[jx+width*jz] & GORO.outside_Mask)
				continue;
 ...
 */
GORO.ConstructionPlan.prototype.getResourcesAround = function(gameState, types, i, radius)
{
...
GORO.ConstructionPlan.prototype.isGo = function(gameState)
{
...
GORO.ConstructionPlan.prototype.onStart = function(gameState)
{
...
GORO.ConstructionPlan.prototype.Serialize = function()
{
	return {
...
GORO.ConstructionPlan.prototype.Deserialize = function(gameState, data)
{
	for (let key in data)
		this[key] = data[key];
