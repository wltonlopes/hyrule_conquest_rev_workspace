...
GORO.VictoryManager = function(Config)
{
	this.Config = Config;
 ...
 */
GORO.VictoryManager.prototype.init = function(gameState)
{
 ...
 */
GORO.VictoryManager.prototype.checkEvents = function(gameState, events)
{
	if (gameState.getVictoryConditions().has("wonder"))
	{
	...
	if (gameState.getVictoryConditions().has("regicide"))
	{
		...
		for (let evt of events.TrainingFinished)
			for (let entId of evt.entities)
			{
				...
				if (ent && ent.isOwn(PlayerID) && ent.getMetadata(PlayerID, "role") === GORO.Worker.ROLE_CRITICAL_ENT_HEALER)
	...
	for (let evt of events.UnGarrison)
	{
		...
		if ((ent.getMetadata(PlayerID, "role") === GORO.Worker.ROLE_CRITICAL_ENT_HEALER ||
		     ent.getMetadata(PlayerID, "role") === GORO.Worker.ROLE_CRITICAL_ENT_GUARD) && !this.guardEnts.get(evt.entity))
		{
...
GORO.VictoryManager.prototype.removeCriticalEnt = function(gameState, criticalEntId)
{
 ...
 */
GORO.VictoryManager.prototype.manageCriticalEntHealers = function(gameState, queues)
{
	...
	for (let data of this.criticalEnts.values())
	{
		...
		queues.healer.addPlan(new GORO.TrainingPlan(gameState, template, { "role": GORO.Worker.ROLE_CRITICAL_ENT_HEALER, "base": 0 }, 1, 1));
		return;
	}
};
 ...
 */
GORO.VictoryManager.prototype.manageCriticalEntGuards = function(gameState)
{
	let numWorkers = gameState.getOwnEntitiesByRole(GORO.Worker.ROLE_WORKER, true).length;
	if (numWorkers < 20)
	{
		for (let data of this.criticalEnts.values())
		{
			for (let guardId of data.guards.keys())
			{
				...
				if (!guardEnt || !guardEnt.hasClass("CitizenSoldier") ||
				    guardEnt.getMetadata(PlayerID, "role") !== GORO.Worker.ROLE_CRITICAL_ENT_GUARD)
					continue;
...
GORO.VictoryManager.prototype.tryAssignMilitaryGuard = function(gameState, guardEnt, criticalEnt, checkForSameAccess)
{
	if (guardEnt.getMetadata(PlayerID, "plan") !== undefined ||
	    ...
	    GORO.getLandAccess(gameState, criticalEnt) != GORO.getLandAccess(gameState, guardEnt)))
		return false;
	...
	guardEnt.setMetadata(PlayerID, "role", GORO.Worker.ROLE_CRITICAL_ENT_GUARD);
	return true;
};

GORO.VictoryManager.prototype.pickCriticalEntRetreatLocation = function(gameState, criticalEnt, emergency)
{
	...
	let accessIndex = GORO.getLandAccess(gameState, criticalEnt);
	let bestBase = GORO.getBestBase(gameState, criticalEnt, true);
 ...
 */
GORO.VictoryManager.prototype.assignGuardToCriticalEnt = function(gameState, guardEnt, criticalEntId)
{
	...
	let guardEntAccess = GORO.getLandAccess(gameState, guardEnt);
	let criticalEntAccess = GORO.getLandAccess(gameState, criticalEnt);
	if (guardEntAccess == criticalEntAccess)
	{
		let queued = GORO.returnResources(gameState, guardEnt);
		guardEnt.guard(criticalEnt, queued);
		const guardRole = guardEnt.getMetadata(PlayerID, "role") === GORO.Worker.ROLE_CRITICAL_ENT_HEALER ? "healer" : "guard";
...
GORO.VictoryManager.prototype.resetCaptureGaiaRelic = function(gameState)
{
...
GORO.VictoryManager.prototype.update = function(gameState, events, queues)
{
 ...
 */
GORO.VictoryManager.prototype.captureGaiaRelic = function(gameState, relic)
{
	...
	for (let plan of plans)
	{
		...
		for (let ent of attack.unitCollection.values())
			capture += ent.captureStrength() * GORO.getAttackBonus(ent, relic, "Capture");
	}
	// No need to make a new attack if already enough units
	if (capture > sumCapturePoints / 50)
		return;
	let relicPosition = relic.position();
	let access = GORO.getLandAccess(gameState, relic);
	let units = gameState.getOwnUnits().filter(ent => {
		...
		{
			let attack = gameState.ai.HQ.attackManager.getPlan(plan);
			if (attack && (attack.state !== GORO.AttackPlan.STATE_UNEXECUTED || attack.type === GORO.AttackPlan.TYPE_RAID))
				return false;
		}
...
GORO.VictoryManager.prototype.abortCaptureGaiaRelic = function(gameState, relicId)
{
...
GORO.VictoryManager.prototype.Serialize = function()
{
	return {
...
GORO.VictoryManager.prototype.Deserialize = function(data)
{
	for (let key in data)
		this[key] = data[key];
};
