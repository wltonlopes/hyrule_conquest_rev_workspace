 ...
 */
GORO.DefenseArmy = function(gameState, foeEntities, type)
{
 ...
 */
GORO.DefenseArmy.prototype.addFoe = function(gameState, enemyId, force)
{
 ...
 */
GORO.DefenseArmy.prototype.removeFoe = function(gameState, enemyId, enemyEntity)
{
 ...
 */
GORO.DefenseArmy.prototype.addOwn = function(gameState, id, force)
{
	...
	if (subrole === undefined || subrole !== GORO.Worker.SUBROLE_DEFENDER)
		ent.setMetadata(PlayerID, "formerSubrole", subrole);
	ent.setMetadata(PlayerID, "subrole", GORO.Worker.SUBROLE_DEFENDER);
	return true;
};

GORO.DefenseArmy.prototype.removeOwn = function(gameState, id, Entity)
{
	...
	if (ent.getMetadata(PlayerID, "transport") !== undefined)
	{
		...
		if (plan && plan.state === GORO.TransportPlan.BOARDING && ent.position())
 ...
 */
GORO.DefenseArmy.prototype.clear = function(gameState)
{
	...
	else
	{
		...
		for (let struct of gameState.getAllyStructures().values())
		{
			...
			if (!pos || !gameState.isPlayerMutualAlly(gameState.ai.HQ.territoryMap.getOwner(pos)))
				continue;
			if (GORO.getLandAccess(gameState, struct) !== armyAccess)
				continue;
...
GORO.DefenseArmy.prototype.assignUnit = function(gameState, entID)
{
	...
	for (let id of this.foeEntities)
	{
		...
		if (!ent.canAttackTarget(eEnt, GORO.allowCapture(gameState, ent, eEnt)))
			continue;
		...
		if (this.assignedAgainst[id].length > 10 ||
			this.assignedAgainst[id].length > 8 && !eEnt.hasClass("Hero") && !GORO.isSiegeUnit(eEnt))
			continue;
	...
	let ownIndex = GORO.getLandAccess(gameState, ent);
	...
	if (ownIndex == foeIndex || ent.hasClass("Ship"))
	{
		...
		ent.attack(idFoe, GORO.allowCapture(gameState, ent, foeEnt), queued);
	}
	else
...
GORO.DefenseArmy.prototype.getType = function()
{
	return this.type;
};

GORO.DefenseArmy.prototype.getState = function()
{
 ...
 */
GORO.DefenseArmy.prototype.merge = function(gameState, otherArmy)
{
...
GORO.DefenseArmy.prototype.needsDefenders = function(gameState)
{
	let defenseRatio;
...
GORO.DefenseArmy.prototype.recalculatePosition = function(gameState, force)
{
...
GORO.DefenseArmy.prototype.recalculateStrengths = function(gameState)
{
	this.ownStrength = 0;
	this.foeStrength = 0;
...
GORO.DefenseArmy.prototype.evaluateStrength = function(ent, isOwn, remove)
{
	if (!ent)
		return;
	...
	if (ent.hasClass("Structure"))
	{
		...
		entStrength = GORO.getMaxStrength(ent, this.Config.debug, this.Config.DamageTypeImportance);
...
GORO.DefenseArmy.prototype.checkEvents = function(gameState, events)
{
...
GORO.DefenseArmy.prototype.update = function(gameState)
{
	for (let entId of this.ownEntities)
	{
		let ent = gameState.getEntityById(entId);
		...
		else if (orderData.length && orderData[0].target && orderData[0].attackType && orderData[0].attackType === "Capture")
		{
			...
			if (target && !GORO.allowCapture(gameState, ent, target))
...
GORO.DefenseArmy.prototype.Serialize = function()
{
	return {
		"ID": this.ID,
		"type": this.type,
...
GORO.DefenseArmy.prototype.Deserialize = function(data)
{
	for (let key in data)
		this[key] = data[key];
};
