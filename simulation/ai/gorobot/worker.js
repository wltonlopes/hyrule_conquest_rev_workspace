...
GORO.Worker.ROLE_ATTACK = "attack";
GORO.Worker.ROLE_TRADER = "trader";
GORO.Worker.ROLE_SWITCH_TO_TRADER = "switchToTrader";
GORO.Worker.ROLE_WORKER = "worker";
GORO.Worker.ROLE_CRITICAL_ENT_GUARD = "criticalEntGuard";
GORO.Worker.ROLE_CRITICAL_ENT_HEALER = "criticalEntHealer";

GORO.Worker.SUBROLE_DEFENDER = "defender";
GORO.Worker.SUBROLE_IDLE = "idle";
GORO.Worker.SUBROLE_BUILDER = "builder";
GORO.Worker.SUBROLE_COMPLETING = "completing";
GORO.Worker.SUBROLE_WALKING = "walking";
GORO.Worker.SUBROLE_ATTACKING = "attacking";
GORO.Worker.SUBROLE_GATHERER = "gatherer";
GORO.Worker.SUBROLE_HUNTER = "hunter";
GORO.Worker.SUBROLE_FISHER = "fisher";
GORO.Worker.SUBROLE_GARRISONING = "garrisoning";

GORO.Worker.prototype.update = function(gameState, ent)
{
	...
	if (ent.getMetadata(PlayerID, "transport") !== undefined)
	{
		...
		if (subrole === GORO.Worker.SUBROLE_BUILDER && ent.getMetadata(PlayerID, "target-foundation") !== undefined)
		{
			...
			if (!target && plan && plan.state === GORO.TransportPlan.BOARDING && ent.position())
				plan.removeUnit(gameState, ent);
		}
		// and gatherer if there are no more dropsite accessible in the base the ent is going to
		if (subrole === GORO.Worker.SUBROLE_GATHERER || subrole === GORO.Worker.SUBROLE_HUNTER)
		{
			let plan = gameState.ai.HQ.navalManager.getPlan(ent.getMetadata(PlayerID, "transport"));
			if (plan.state === GORO.TransportPlan.BOARDING && ent.position())
			{
				let hasDropsite = false;
				let gatherType = ent.getMetadata(PlayerID, "gather-type") || "stone";
				for (let structure of gameState.getOwnStructures().values())
				{
					if (GORO.getLandAccess(gameState, structure) != plan.endIndex)
						continue;
					let resourceDropsiteTypes = GORO.getBuiltEntity(gameState, structure).resourceDropsiteTypes();
				...
				if (!hasDropsite)
				{
					...
					{
						if (!unit.position() || GORO.getLandAccess(gameState, unit) != plan.endIndex)
							continue;
	...
	this.entAccess = GORO.getLandAccess(gameState, ent);
	...
	if ((subrole === GORO.Worker.SUBROLE_HUNTER || subrole === GORO.Worker.SUBROLE_GATHERER) &&
	     ...
	     unitAIState == "INDIVIDUAL.COMBAT.APPROACHING"))
	{
		...
		if (unitAIState == "INDIVIDUAL.COMBAT.APPROACHING" && ent.unitAIOrderData().length)
		{
			...
			if (orderData && orderData.target)
			{
				...
				if (target && target.resourceSupplyType() && target.resourceSupplyType().generic == "stone")
				{
					...
					{
						let distanceSquare = GORO.isFastMoving(ent) ? 90000 : 30000;
						let targetAccess = GORO.getLandAccess(gameState, target);
						let stoneDropsites = gameState.playerData.hasSharedDropsites ?
						                    gameState.getAnyDropsites("stone") : gameState.getOwnDropsites("stone");
						let hasstoneDropsiteWithinDistance = false;
						for (let dropsite of stoneDropsites.values())
						{
							...
							if (owner != PlayerID && (!dropsite.isSharedDropsite() || !gameState.isPlayerMutualAlly(owner)))
								continue;
							if (targetAccess != GORO.getLandAccess(gameState, dropsite))
								continue;
							if (API3.SquareVectorDistance(target.position(), dropsite.position()) < distanceSquare)
							{
								hasstoneDropsiteWithinDistance = true;
								break;
							}
						}
						if (!hasstoneDropsiteWithinDistance)
						{
							 if (this.retryWorking(gameState, subrole))
								return;
							ent.stopMoving();
	...
	{
		if (subrole === GORO.Worker.SUBROLE_FISHER)
			...
			!ent.getMetadata(PlayerID, "PartOfArmy"))
		{
			...
			{
				let target = gameState.getEntityById(orderData.target);
				if (target && (!target.position() || GORO.getLandAccess(gameState, target) != this.entAccess))
				{
					if (this.retryWorking(gameState, subrole))
						return;
					ent.stopMoving();
			...
			!ent.getMetadata(PlayerID, "PartOfArmy"))
		{
			...
			if (orderData && orderData.target && orderData.attackType && orderData.attackType == "Capture")
			{
				...
				if (target && target.owner() > 0 && !gameState.isPlayerAlly(target.owner()))
					ent.attack(orderData.target, GORO.allowCapture(gameState, ent, target));
			}
		}
		return;
	}
	...
	if (subrole === GORO.Worker.SUBROLE_GATHERER)
	{
		if (ent.isIdle())
		{
			...
			else if (!GORO.returnResources(gameState, ent))     // try to deposit resources
			{
		...
		else if (unitAIStateOrder == "GATHER")
		{
			...
			if (unitAIState == "INDIVIDUAL.GATHER.RETURNINGRESOURCE.APPROACHING")
			{
				if (gameState.ai.playedTurn % 10 == 0)
				{
					...
					if (dropsite && dropsite.position() && this.entAccess != GORO.getLandAccess(gameState, dropsite))
						GORO.returnResources(gameState, this.ent);
				}
	...
	else if (subrole === GORO.Worker.SUBROLE_BUILDER)
	{
		...
		{
			let goalAccess = GORO.getLandAccess(gameState, target);
			let queued = GORO.returnResources(gameState, ent);
	...
	else if (subrole === GORO.Worker.SUBROLE_HUNTER)
	{
		...
		else	// Perform some sanity checks
		{
			if (unitAIStateOrder == "GATHER")
			{
				...
				else if (unitAIState == "INDIVIDUAL.GATHER.RETURNINGRESOURCE.APPROACHING")
				{
					...
					let dropsite = gameState.getEntityById(ent.unitAIOrderData()[0].target);
					if (dropsite && dropsite.position() && this.entAccess != GORO.getLandAccess(gameState, dropsite))
						GORO.returnResources(gameState, ent);
				}
			}
		}
	}
	else if (subrole === GORO.Worker.SUBROLE_FISHER)
	{
		if (ent.isIdle())
			this.startFishing(gameState);
...
GORO.Worker.prototype.retryWorking = function(gameState, subrole)
{
	switch (subrole)
	{
	case GORO.Worker.SUBROLE_GATHERER:
		return this.startGathering(gameState);
	case GORO.Worker.SUBROLE_HUNTER:
		return this.startHunting(gameState);
	case GORO.Worker.SUBROLE_FISHER:
		return this.startFishing(gameState);
	case GORO.Worker.SUBROLE_BUILDER:
...
GORO.Worker.prototype.startBuilding = function(gameState)
{
	let target = gameState.getEntityById(this.ent.getMetadata(PlayerID, "target-foundation"));
	if (!target || target.foundationProgress() === undefined && target.needsRepair() == false)
		return false;
	if (GORO.getLandAccess(gameState, target) != this.entAccess)
		return false;
	this.ent.repair(target, target.hasClass("House"));  // autocontinue=true for houses
	return true;
};

GORO.Worker.prototype.startGathering = function(gameState)
{
	// First look for possible treasure if any
	if (GORO.gatherTreasure(gameState, this.ent))
		return true;
	...
	const findSupply = function(worker, supplies) {
		...
		for (let i = 0; i < supplies.length; ++i)
		{
			...
			if (GORO.IsSupplyFull(gameState, supplies[i].ent))
				continue;
			...
			if (supplies[i].ent.resourceSupplyType().specific != "stone" && nbGatherers > 0 &&
	...
	if (this.baseAccess == this.entAccess)
	{
		...
		// --> for stone, try to gather from fields if any, otherwise build one if any
		if (resource == "stone")
		{
	...
	if (resource == "stone")	// --> for stone, try to gather from fields if any, otherwise build one if any
	{
		for (const base of gameState.ai.HQ.baseManagers())
		{
			if (base.ID == this.baseID)
	...
	let shouldBuild = this.ent.isBuilder() && foundations.some(function(foundation) {
		if (!foundation || GORO.getLandAccess(gameState, foundation) != this.entAccess)
			return false;
		...
		if (structure.resourceDropsiteTypes() && structure.resourceDropsiteTypes().indexOf(resource) != -1)
		{
			...
			this.ent.setMetadata(PlayerID, "subrole", GORO.Worker.SUBROLE_BUILDER);
	...
	shouldBuild = this.ent.isBuilder() && foundations.some(function(foundation) {
		...
		if (structure.resourceDropsiteTypes() && structure.resourceDropsiteTypes().indexOf(resource) != -1)
		{
			let foundationAccess = GORO.getLandAccess(gameState, foundation);
	...
	// except for stone when farms can be used
	let allowDistant = true;
	if (resource == "stone")
	{
		if (gameState.ai.HQ.turnCache.allowDistantstone === undefined)
			gameState.ai.HQ.turnCache.allowDistantstone =
				!gameState.ai.HQ.canBuild(gameState, "structures/{civ}/field");
		allowDistant = gameState.ai.HQ.turnCache.allowDistantstone;
	}
	if (allowDistant)
	{
		if (this.baseAccess == this.entAccess)
	...
	this.ent.setMetadata(PlayerID, "subrole", GORO.Worker.SUBROLE_IDLE);
	return false;
};
 ...
 */
GORO.Worker.prototype.startHunting = function(gameState, position)
{
	// First look for possible treasure if any
	if (!position && GORO.gatherTreasure(gameState, this.ent))
		return true;
	...
	let isFastMoving = GORO.isFastMoving(this.ent);
	let isRanged = this.ent.hasClass("Ranged");
	let entPosition = position ? position : this.ent.position();
	let stoneDropsites = gameState.playerData.hasSharedDropsites ?
	                    gameState.getAnyDropsites("stone") : gameState.getOwnDropsites("stone");

	let hasstoneDropsiteWithinDistance = function(supplyPosition, supplyAccess, distSquare)
	{
		for (let dropsite of stoneDropsites.values())
		{
			if (!dropsite.position())
			...
			if (owner != PlayerID && (!dropsite.isSharedDropsite() || !gameState.isPlayerMutualAlly(owner)))
				continue;
			if (supplyAccess != GORO.getLandAccess(gameState, dropsite))
				continue;
	...
	for (let supply of resources.values())
	{
		...
		if (GORO.IsSupplyFull(gameState, supply))
			continue;
		// Check if available resource is worth one additionnal gatherer (except for farms).
		...
		let supplyAccess = GORO.getLandAccess(gameState, supply);
		...
		if (!hasstoneDropsiteWithinDistance(supply.position(), supplyAccess, distanceSquare))
			continue;
...
GORO.Worker.prototype.startFishing = function(gameState)
{
	...
	let fisherSea = GORO.getSeaAccess(gameState, this.ent);
	let fishDropsites = (gameState.playerData.hasSharedDropsites ? gameState.getAnyDropsites("stone") : gameState.getOwnDropsites("stone")).
	...
	let nearestDropsiteDist = function(supply) {
		...
		for (let dropsite of fishDropsites)
		{
			...
			if (owner != PlayerID && (!dropsite.isSharedDropsite() || !gameState.isPlayerMutualAlly(owner)))
				continue;
			if (fisherSea != GORO.getSeaAccess(gameState, dropsite))
				continue;
	...
	if (this.ent.getMetadata(PlayerID, "subrole") === GORO.Worker.SUBROLE_FISHER)
		this.ent.setMetadata(PlayerID, "subrole", GORO.Worker.SUBROLE_IDLE);
	return false;
};

GORO.Worker.prototype.gatherNearestField = function(gameState, baseID)
{
	...
	{
		if (GORO.IsSupplyFull(gameState, field))
			continue;
 ...
 */
GORO.Worker.prototype.buildAnyField = function(gameState, baseID)
{
 ...
 */
GORO.Worker.prototype.moveToGatherer = function(gameState, ent, forced)
{
	...
	if (!forced && gameState.ai.elapsedTime < (ent.getMetadata(PlayerID, "nextMoveToGatherer") || 5))
		return;
	const gatherers = this.base.workersBySubrole(gameState, GORO.Worker.SUBROLE_GATHERER);
	let dist = Math.min();
	let destination;
	...
	for (let gatherer of gatherers.values())
	{
		...
		if (!gatherer.position() || gatherer.getMetadata(PlayerID, "transport") !== undefined ||
		    GORO.getLandAccess(gameState, gatherer) != access || gatherer.isIdle())
			continue;
 ...
 */
GORO.Worker.prototype.isInaccessibleSupply = function(gameState)
{
