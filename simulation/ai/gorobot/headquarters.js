...
GORO.HQ = function(Config)
{
	...
	this.basesManager = new GORO.BasesManager(this.Config);
	this.attackManager = new GORO.AttackManager(this.Config);
	this.buildManager = new GORO.BuildManager();
	this.defenseManager = new GORO.DefenseManager(this.Config);
	this.tradeManager = new GORO.TradeManager(this.Config);
	this.navalManager = new GORO.NavalManager(this.Config);
	this.researchManager = new GORO.ResearchManager(this.Config);
	this.diplomacyManager = new GORO.DiplomacyManager(this.Config);
	this.garrisonManager = new GORO.GarrisonManager(this.Config);
	this.victoryManager = new GORO.VictoryManager(this.Config);
	this.emergencyManager = new GORO.EmergencyManager(this.Config);
...
GORO.HQ.prototype.init = function(gameState, queues)
{
	this.territoryMap = GORO.createTerritoryMap(gameState);
	...
	this.borderMap = GORO.createBorderMap(gameState);
 ...
 */
GORO.HQ.prototype.postinit = function(gameState)
{
 ...
 */
GORO.HQ.prototype.getSeaBetweenIndices = function(gameState, index1, index2)
{
...
GORO.HQ.prototype.checkEvents = function(gameState, events)
{
	...
	for (let evt of events.OwnershipChanged)   // capture events
	{
		...
		{
			ent.setMetadata(PlayerID, "role", GORO.Worker.ROLE_TRADER);
			ent.setMetadata(PlayerID, "route", undefined);
		}
		if (ent.hasClass("Worker"))
		{
			ent.setMetadata(PlayerID, "role", GORO.Worker.ROLE_WORKER);
			ent.setMetadata(PlayerID, "subrole", GORO.Worker.SUBROLE_IDLE);
		}
		if (ent.hasClass("Ship"))
			GORO.setSeaAccess(gameState, ent);
	...
	for (let evt of events.TrainingFinished)
	{
		for (let entId of evt.entities)
		{
			...
			if (plan !== undefined && plan >= 0)
			{
				...
				if (!attack || attack.state !== GORO.AttackPlan.STATE_UNEXECUTED)
					ent.setMetadata(PlayerID, "plan", -1);
			}
		}
	}
...
GORO.HQ.prototype.handleNewBase = function(gameState)
{
...
GORO.HQ.prototype.checkPhaseRequirements = function(gameState, queues)
{
	...
	for (let entityReq of requirements)
	{
		...
		case "Town":
			    ...
			    !queues.militaryBuilding.hasQueuedUnits())
			{
				...
				{
					plan = new GORO.ConstructionPlan(gameState, "structures/{civ}/market", { "phaseUp": true });
				...
				{
					plan = new GORO.ConstructionPlan(gameState, "structures/{civ}/temple", { "phaseUp": true });
					queue = "economicBuilding";
					break;
				}
				...
				{
					plan = new GORO.ConstructionPlan(gameState, "structures/{civ}/forge", { "phaseUp": true });
		...
		default:
			...
			if (!queues[queue].hasQueuedUnits())
			{
				...
				if (structure && this.canBuild(gameState, structure))
					plan = new GORO.ConstructionPlan(gameState, structure, { "phaseUp": true });
			}
		}
		...
		if (plan)
		{
			if (queue == "wonder")
			{
				gameState.ai.queueManager.changePriority("majorTech", 500, { "phaseUp": true });
...
GORO.HQ.prototype.OnPhaseUp = function(gameState, phase)
{
};

/** This code trains citizen workers, trying to keep close to a ratio of worker/soldiers */
GORO.HQ.prototype.trainMoreWorkers = function(gameState, queues)
{
	// default template
	...
	gameState.getOwnUnits().forEach(ent => {
		if (ent.getMetadata(PlayerID, "role") === GORO.Worker.ROLE_WORKER && ent.getMetadata(PlayerID, "plan") === undefined)
		{
			++numberOfWorkers;
	...
	gameState.getOwnTrainingFacilities().forEach(function(ent) {
		for (let item of ent.trainingQueue())
		{
			...
			if (item.metadata && item.metadata.role && item.metadata.role === GORO.Worker.ROLE_WORKER &&
	...
	if (this.attackManager.rushNumber < this.attackManager.maxRushes || this.attackManager.upcomingAttacks[GORO.AttackPlan.TYPE_RUSH].length)
		alpha = 0.7;
	...
	if (!template && templateDef)
		queues.villager.addPlan(new GORO.TrainingPlan(gameState, templateDef, { "role": GORO.Worker.ROLE_WORKER, "base": 0, "support": true }, size, size));
	else if (template)
		queues.citizenSoldier.addPlan(new GORO.TrainingPlan(gameState, template, { "role": GORO.Worker.ROLE_WORKER, "base": 0 }, size, size));
};

/** picks the best template based on parameters and classes */
GORO.HQ.prototype.findBestTrainableUnit = function(gameState, classes, requirements)
{
	let units;
	...
	units.sort((a, b) => {
		...
		for (let param of parameters)
		{
			if (param[0] == "strength")
			{
				aValue += GORO.getMaxStrength(a[1], gameState.ai.Config.debug, gameState.ai.Config.DamageTypeImportance) * param[1];
				bValue += GORO.getMaxStrength(b[1], gameState.ai.Config.debug, gameState.ai.Config.DamageTypeImportance) * param[1];
			}
			else if (param[0] == "siegeStrength")
			{
				aValue += GORO.getMaxStrength(a[1], gameState.ai.Config.debug, gameState.ai.Config.DamageTypeImportance, "Structure") * param[1];
				bValue += GORO.getMaxStrength(b[1], gameState.ai.Config.debug, gameState.ai.Config.DamageTypeImportance, "Structure") * param[1];
			}
 ...
 */
GORO.HQ.prototype.bulkPickWorkers = function(gameState, baseRef, number)
{
	return this.basesManager.bulkPickWorkers(gameState, baseRef, number);
};

GORO.HQ.prototype.getTotalResourceLevel = function(gameState, resources, proximity)
{
 ...
 */
GORO.HQ.prototype.GetCurrentGatherRates = function(gameState)
{
 ...
 */
GORO.HQ.prototype.GetWantedGatherRates = function(gameState)
{
 ...
 */
GORO.HQ.prototype.pickMostNeededResources = function(gameState, allowedResources = [])
{
 ...
 */
GORO.HQ.prototype.findEconomicCCLocation = function(gameState, template, resource, proximity, fromStrategic)
{
	...
	// obstruction map
	let obstructions = GORO.createObstructionMap(gameState, 0, template);
	let halfSize = 0;
	...
	for (let j = 0; j < this.territoryMap.length; ++j)
	{
		...
		else
		{
			...
			for (let cc of ccList)
			{
				...
				if (dist < minDist)
					minDist = dist;
				accessible = accessible || index == GORO.getLandAccess(gameState, cc.ent);
			}
			if (norm == 0)
				continue;
			...
			oversea = !accessible && dpList.some(dp => GORO.getLandAccess(gameState, dp.ent) == index);
			if (!oversea)
			{
		...
		if (this.borderMap.map[j] & GORO.fullBorder_Mask)	// disfavor the borders of the map
			norm *= 0.5;
 ...
 */
GORO.HQ.prototype.findStrategicCCLocation = function(gameState, template)
{
	...
	for (let j = 0; j < this.territoryMap.length; ++j)
	{
		...
		if (this.borderMap.map[j] & GORO.fullBorder_Mask)
			currentVal += 10000;
 ...
 */
GORO.HQ.prototype.findMarketLocation = function(gameState, template)
{
	...
	for (let j = 0; j < this.territoryMap.length; ++j)
	{
		...
		if (this.borderMap.map[j] & GORO.narrowFrontier_Mask)
			continue;
		...
		for (let market of markets)
		{
			...
			{
				if (GORO.getSeaAccess(gameState, market) != gameState.ai.accessibility.getAccessValue(pos, true))
					continue;
				gainMultiplier = traderTemplatesGains.navalGainMultiplier;
			}
			else if (GORO.getLandAccess(gameState, market) == index &&
				!GORO.isLineInsideEnemyTerritory(gameState, market.position(), pos))
 ...
 */
GORO.HQ.prototype.findDefensiveLocation = function(gameState, template)
{
	...
	for (let j = 0; j < this.territoryMap.length; ++j)
	{
		if (!wonderMode)
		{
			...
			if (!(this.borderMap.map[j] & GORO.fullFrontier_Mask))
				continue;
			if (this.borderMap.map[j] & GORO.largeFrontier_Mask && isTower)
				continue;
		}
...
GORO.HQ.prototype.buildTemple = function(gameState, queues)
{
	...
	if (this.canBuild(gameState, "structures/{civ}/temple_vesta"))
		...
		return;
	queues.economicBuilding.addPlan(new GORO.ConstructionPlan(gameState, templateName));
};

GORO.HQ.prototype.buildMarket = function(gameState, queues)
{
	if (gameState.getOwnEntitiesByClass("Market", true).hasEntities() ||
		!this.canBuild(gameState, "structures/{civ}/market"))
		return;
	...
	let plan = new GORO.ConstructionPlan(gameState, "structures/{civ}/market");
	plan.queueToReset = "economicBuilding";
	queues.economicBuilding.addPlan(plan);
};

/** Build a farmstead */
GORO.HQ.prototype.buildFarmstead = function(gameState, queues)
{
	...
	queues.economicBuilding.addPlan(new GORO.ConstructionPlan(gameState, "structures/{civ}/farmstead"));
};
 ...
 */
GORO.HQ.prototype.buildWonder = function(gameState, queues, force = false)
{
	...
	queues.wonder.addPlan(new GORO.ConstructionPlan(gameState, "structures/{civ}/wonder"));
};

/** Build a corral, and train animals there */
GORO.HQ.prototype.manageCorral = function(gameState, queues)
{
	                ...
	                nCorral < this.currentPhase && gameState.getPopulation() > 30 * nCorral)
	{
		...
		{
			queues.corral.addPlan(new GORO.ConstructionPlan(gameState, "structures/{civ}/corral"));
			return;
		}
		if (!nCorral)
			return;
	...
	for (let corral of gameState.getOwnEntitiesByClass("Corral", true).values())
	{
		...
		for (let trainable of trainables)
		{
			...
			if (count > nCorral)
				continue;
			queues.corral.addPlan(new GORO.TrainingPlan(gameState, trainable, { "trainer": corral.id() }));
			return;
		}
	}
};
 ...
 */
GORO.HQ.prototype.buildMoreHouses = function(gameState, queues)
{
	...
	{
		let plan = new GORO.ConstructionPlan(gameState, houseTemplateString);
...
GORO.HQ.prototype.checkBaseExpansion = function(gameState, queues)
{
...
GORO.HQ.prototype.buildNewBase = function(gameState, queues, resource)
{
	...
	queues.civilCentre.addPlan(new GORO.ConstructionPlan(gameState, template, { "base": -1, "resource": resource }));
	return true;
};

/** Deals with building fortresses and towers along our border with enemies. */
GORO.HQ.prototype.buildDefenses = function(gameState, queues)
{
	...
	if (!this.saveResources && (this.currentPhase > 2 || gameState.isResearching(gameState.getPhaseName(3))))
	{
		...
		if (this.canBuild(gameState, "structures/{civ}/fortress"))
		{
				...
				gameState.getOwnFoundationsByClass("Fortress").length < 2)
			{
				...
				let plan = new GORO.ConstructionPlan(gameState, "structures/{civ}/fortress");
	...
	if (this.Config.Military.numSentryTowers && this.currentPhase < 2 && this.canBuild(gameState, "structures/{civ}/sentry_tower"))
	{
		...
		if (numTowers < this.Config.Military.numSentryTowers && gameState.ai.elapsedTime > towerLapseTime + this.fortStartTime)
		{
			...
			queues.defenseBuilding.addPlan(new GORO.ConstructionPlan(gameState, "structures/{civ}/sentry_tower"));
		}
		return;
	}
		...
		gameState.getOwnFoundationsByClass("Tower").length < 3)
	{
		...
		let plan = new GORO.ConstructionPlan(gameState, "structures/{civ}/defense_tower");
		plan.queueToReset = "defenseBuilding";
		queues.defenseBuilding.addPlan(plan);
	}
};

GORO.HQ.prototype.buildForge = function(gameState, queues)
{
	...
	if (this.canBuild(gameState, "structures/{civ}/forge"))
		queues.militaryBuilding.addPlan(new GORO.ConstructionPlan(gameState, "structures/{civ}/forge"));
};
 ...
 */
GORO.HQ.prototype.constructTrainingBuildings = function(gameState, queues)
{
	    ...
	    this.phasing == 2 && gameState.getOwnStructures().filter(API3.Filters.byClass("Village")).length < 5)
	{
		...
		if (numBarracks + numRanges == 0)
		{
			...
			if (template)
			{
				...
				let plan = new GORO.ConstructionPlan(gameState, template, { "militaryBase": true });
		...
		{
			queues.militaryBuilding.addPlan(new GORO.ConstructionPlan(gameState, stableTemplate, { "militaryBase": true }));
			return;
		}
		if (numKennels == 0 && kennelTemplate)
		{
			queues.militaryBuilding.addPlan(new GORO.ConstructionPlan(gameState, kennelTemplate, { "militaryBase": true }));
			return;
		}
		if (numMercs == 0 && mercsTemplate)
		{
			queues.militaryBuilding.addPlan(new GORO.ConstructionPlan(gameState, mercsTemplate, { "militaryBase": true }));
			return;
		}
		...
		if (numBarracks + numRanges == 1 && this.getAccountedPopulation(gameState) > this.Config.Military.popForBarracks2)
		{
			...
			if (template)
			{
				queues.militaryBuilding.addPlan(new GORO.ConstructionPlan(gameState, template, { "militaryBase": true }));
				return;
			}
		}
	...
	{
		queues.militaryBuilding.addPlan(new GORO.ConstructionPlan(gameState, "structures/{civ}/elephant_stable", { "militaryBase": true }));
		return;
	}

	if (this.canBuild(gameState, "structures/{civ}/arsenal") && !gameState.getOwnEntitiesByClass("Arsenal", true).hasEntities())
	{
		queues.militaryBuilding.addPlan(new GORO.ConstructionPlan(gameState, "structures/{civ}/arsenal", { "militaryBase": true }));
		return;
	}
	...
	if (!nAdvanced || nAdvanced < this.bAdvanced.length && this.getAccountedPopulation(gameState) > 110)
	{
		for (let advanced of this.bAdvanced)
		{
			...
			if (template.hasDefensiveFire() || template.trainableEntities(civ))
				queues.militaryBuilding.addPlan(new GORO.ConstructionPlan(gameState, advanced, { "militaryBase": true }));
			else	// not a military building, but still use this queue
				queues.militaryBuilding.addPlan(new GORO.ConstructionPlan(gameState, advanced));
			return;
		}
	}
};
 ...
 */
GORO.HQ.prototype.findBestBaseForMilitary = function(gameState)
{
 ...
 */
GORO.HQ.prototype.trainEmergencyUnits = function(gameState, positions)
{
	...
	const metadata = { "role": GORO.Worker.ROLE_WORKER, "base": nearestAnchor.getMetadata(PlayerID, "base"), "plan": -1, "trainer": nearestAnchor.id() };
	if (autogarrison)
		metadata.garrisonType = GORO.GarrisonManager.TYPE_PROTECTION;
	gameState.ai.queues.emergency.addPlan(new GORO.TrainingPlan(gameState, templateFound[0], metadata, 1, 1));
	return true;
};

GORO.HQ.prototype.canBuild = function(gameState, structure)
{
...
GORO.HQ.prototype.updateTerritories = function(gameState)
{
	...
	for (let j = 0; j < this.territoryMap.length; ++j)
	{
		if (this.borderMap.map[j] & GORO.outside_Mask)
			continue;
		if (this.borderMap.map[j] & GORO.fullFrontier_Mask)
			this.borderMap.map[j] &= ~GORO.fullFrontier_Mask;	// reset the frontier
		...
		else
		{
			...
			for (let a of around)
			{
				...
				if (jz < 0 || jz >= width)
					continue;
				if (this.borderMap.map[jx+width*jz] & GORO.outside_Mask)
					continue;
				...
				{
					this.borderMap.map[j] |= GORO.narrowFrontier_Mask;
					break;
				}
			...
			if (onFrontier && !(this.borderMap.map[j] & GORO.narrowFrontier_Mask))
				this.borderMap.map[j] |= GORO.largeFrontier_Mask;
 ...
 */
GORO.HQ.prototype.getBaseByID = function(baseID)
{
 ...
 */
GORO.HQ.prototype.numActiveBases = function()
{
	return this.basesManager.numActiveBases();
};

GORO.HQ.prototype.hasActiveBase = function()
{
	return this.basesManager.hasActiveBase();
};

GORO.HQ.prototype.numPotentialBases = function()
{
	return this.basesManager.numPotentialBases();
};

GORO.HQ.prototype.hasPotentialBase = function()
{
	return this.basesManager.hasPotentialBase();
};

GORO.HQ.prototype.isDangerousLocation = function(gameState, pos, radius)
{
...
GORO.HQ.prototype.isNearInvadingArmy = function(pos)
{
...
GORO.HQ.prototype.isUnderEnemyFire = function(gameState, pos, radius = 0)
{
...
GORO.HQ.prototype.updateCaptureStrength = function(gameState)
{
	...
	for (let ent of gameState.getOwnUnits().values())
	{
		...
		if (!this.capturableTargets.has(targetId))
			this.capturableTargets.set(targetId, {
				"strength": ent.captureStrength() * GORO.getAttackBonus(ent, target, "Capture"),
				"ents": new Set([ent.id()])
			});
		else
		{
			let capturableTarget = this.capturableTargets.get(target.id());
			capturableTarget.strength += ent.captureStrength() * GORO.getAttackBonus(ent, target, "Capture");
	...
	for (let [targetId, capturableTarget] of this.capturableTargets)
	{
		...
		for (let entId of capturableTarget.ents)
		{
			...
			if (allowCapture === undefined)
				allowCapture = GORO.allowCapture(gameState, ent, target);
			let orderData = ent.unitAIOrderData();
 ...
 */
GORO.HQ.prototype.isDefendable = function(ent)
{
 ...
 */
GORO.HQ.prototype.getAccountedPopulation = function(gameState)
{
 ...
 */
GORO.HQ.prototype.getAccountedWorkers = function(gameState)
{
	if (this.turnCache.accountedWorkers == undefined)
	{
		let workers = gameState.getOwnEntitiesByRole(GORO.Worker.ROLE_WORKER, true).length;
		for (let ent of gameState.getOwnTrainingFacilities().values())
		{
			for (let item of ent.trainingQueue())
			{
...
GORO.HQ.prototype.baseManagers = function()
{
 ...
 */
GORO.HQ.prototype.baseAtIndex = function(territoryIndex)
{
 ...
 */
GORO.HQ.prototype.update = function(gameState, queues, events)
{
	...
	*/
	...
	if (gameState.ai.playedTurn % 3 == 0)
	{
		...
		if (this.Config.difficulty > GORO.DIFFICULTY_SANDBOX)
	...
	if (this.Config.difficulty > GORO.DIFFICULTY_SANDBOX && (this.hasActiveBase() || !this.canBuildUnits))
...
GORO.HQ.prototype.Serialize = function()
{
	let properties = {
		"phasing": this.phasing,
...
GORO.HQ.prototype.Deserialize = function(gameState, data)
{
