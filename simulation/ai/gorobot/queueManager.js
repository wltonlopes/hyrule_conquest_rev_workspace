...
GORO.QueueManager = function(Config, queues)
{
	this.Config = Config;
	this.queues = queues;
	this.priorities = {};
...
GORO.QueueManager.prototype.getAvailableResources = function(gameState)
{
...
GORO.QueueManager.prototype.getTotalAccountedResources = function()
{
...
GORO.QueueManager.prototype.currentNeeds = function(gameState)
{
...
// TODO: many things.
GORO.QueueManager.prototype.wantedGatherRates = function(gameState)
{
...
GORO.QueueManager.prototype.printQueues = function(gameState)
{
	let numWorkers = 0;
...
GORO.QueueManager.prototype.clear = function()
{
	for (let i in this.queues)
		this.queues[i].empty();
};
 ...
 */
GORO.QueueManager.prototype.setAccounts = function(gameState, cost, i)
{
 ...
 */
GORO.QueueManager.prototype.transferAccounts = function(cost, i, j)
{
	for (let res of Resources.GetCodes())
	{
 ...
 */
GORO.QueueManager.prototype.distributeResources = function(gameState)
{
	...
	for (let res of Resources.GetCodes())
	{
		...
		if (available < 0)
			API3.warn("GORO: problem with remaining " + res + " in queueManager " + available);
	}
};

GORO.QueueManager.prototype.switchResource = function(gameState, res)
{
...
GORO.QueueManager.prototype.startNextItems = function(gameState)
{
	for (let q of this.queueArrays)
	{
		let name = q[0];
...
GORO.QueueManager.prototype.update = function(gameState)
{
...
GORO.QueueManager.prototype.checkPausedQueues = function(gameState)
{
	const numWorkers = gameState.countOwnEntitiesAndQueuedWithRole(GORO.Worker.ROLE_WORKER);
...
GORO.QueueManager.prototype.canAfford = function(queue, cost)
{
...
GORO.QueueManager.prototype.pauseQueue = function(queue, scrapAccounts)
{
...
GORO.QueueManager.prototype.unpauseQueue = function(queue)
{
	if (this.queues[queue])
		this.queues[queue].paused = false;
};

GORO.QueueManager.prototype.pauseAll = function(scrapAccounts, but)
{
	for (let q in this.queues)
	{
		if (q == but)
...
GORO.QueueManager.prototype.unpauseAll = function(but)
{
	for (let q in this.queues)
		if (q != but)
...
GORO.QueueManager.prototype.addQueue = function(queueName, priority)
{
	if (this.queues[queueName] !== undefined)
		return;

	this.queues[queueName] = new GORO.Queue();
...
GORO.QueueManager.prototype.removeQueue = function(queueName)
{
	if (this.queues[queueName] === undefined)
		return;
...
GORO.QueueManager.prototype.getPriority = function(queueName)
{
	return this.priorities[queueName];
};

GORO.QueueManager.prototype.changePriority = function(queueName, newPriority)
{
...
GORO.QueueManager.prototype.Serialize = function()
{
	let accounts = {};
	let queues = {};
	for (let q in this.queues)
...
GORO.QueueManager.prototype.Deserialize = function(gameState, data)
{
