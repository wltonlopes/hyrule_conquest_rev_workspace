 ...
 */
GORO.DiplomacyManager = function(Config)
{
	this.Config = Config;
 ...
 */
GORO.DiplomacyManager.prototype.init = function(gameState)
{
 ...
 */
GORO.DiplomacyManager.prototype.tributes = function(gameState)
{
	...
	for (let i = 1; i < gameState.sharedScript.playersData.length; ++i)
	{
			...
			else if (this.Config.chat && availableResources[res] === 0 && allyResources[res] > totalResources[res] + 600)
			{
				...
				for (let k = 0; k < mostNeeded.length; ++k)
				{
					if (mostNeeded[k].type == res && mostNeeded[k].wanted > 0)
					{
						...
						GORO.chatRequestTribute(gameState, res);
		...
		if (this.Config.chat)
			GORO.chatSentTribute(gameState, i);
		Engine.PostCommand(PlayerID, { "type": "tribute", "player": i, "amounts": tribute });
	}
};

GORO.DiplomacyManager.prototype.checkEvents = function(gameState, events)
{
	...
	for (let evt of events.DiplomacyChanged)
	{
		...
		else if (gameState.sharedScript.playersData[evt.player].isAlly[PlayerID] && gameState.isPlayerEnemy(evt.player))
		{
			...
			GORO.chatAnswerRequestDiplomacy(gameState, evt.player, "ally", response);
		}
 ...
 */
GORO.DiplomacyManager.prototype.lastManStandingCheck = function(gameState)
{
	...
	if (playerToTurnAgainst)
	{
		...
		{
			if (request.status === "waitingForTribute")
				GORO.chatAnswerRequestDiplomacy(gameState, player, request.requestType, "decline");
 ...
 */
GORO.DiplomacyManager.prototype.handleDiplomacyRequest = function(gameState, player, requestType)
{
	...
	GORO.chatAnswerRequestDiplomacy(gameState, player, requestType, response, requiredTribute);
};

GORO.DiplomacyManager.prototype.changePlayerDiplomacy = function(gameState, player, newDiplomaticStance)
{
	...
	if (this.Config.chat)
		GORO.chatNewDiplomacy(gameState, player, newDiplomaticStance);
};

GORO.DiplomacyManager.prototype.checkRequestedTributes = function(gameState)
{
	for (let [player, data] of this.receivedDiplomacyRequests)
		if (data.status === "waitingForTribute" && gameState.ai.elapsedTime > data.warnTime)
		{
			if (data.sentWarning)
			{
				...
				GORO.chatAnswerRequestDiplomacy(gameState, player, data.requestType, "decline");
			}
			else
			{
				data.sentWarning = true;
				data.warnTime = gameState.ai.elapsedTime + 60;
				GORO.chatAnswerRequestDiplomacy(gameState, player, data.requestType, "waitingForTribute", {
					"wanted": data.wanted,
					"type": data.type
				});
			}
 ...
 */
GORO.DiplomacyManager.prototype.sendDiplomacyRequest = function(gameState)
{
	let player;
	let max = 0;
	for (let i = 1; i < gameState.sharedScript.playersData.length; ++i)
	...
	GORO.chatNewRequestDiplomacy(gameState, player, requestType, "sendRequest");
};

GORO.DiplomacyManager.prototype.checkSentDiplomacyRequests = function(gameState)
{
	for (let [player, data] of this.sentDiplomacyRequests)
		...
		{
			GORO.chatNewRequestDiplomacy(gameState, player, data.requestType, "requestExpired");
			this.sentDiplomacyRequests.delete(player);
		}
};

GORO.DiplomacyManager.prototype.update = function(gameState, events)
{
...
GORO.DiplomacyManager.prototype.Serialize = function()
{
	return {
...
GORO.DiplomacyManager.prototype.Deserialize = function(data)
{
	for (let key in data)
		this[key] = data[key];
};
