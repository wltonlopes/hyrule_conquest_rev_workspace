...
GORO.TERRITORY_PLAYER_MASK = 0x1F;
GORO.TERRITORY_BLINKING_MASK = 0x40;

GORO.createObstructionMap = function(gameState, accessIndex, template)
{
	...
	{
		let tilePlayer = territoryMap.data[k] & GORO.TERRITORY_PLAYER_MASK;
		let isConnected = (territoryMap.data[k] & GORO.TERRITORY_BLINKING_MASK) == 0;
...
GORO.createTerritoryMap = function(gameState)
{
	let map = gameState.ai.territoryMap;

	let ret = new API3.Map(gameState.sharedScript, "territory", map.data);
	ret.getOwner = function(p) { return this.point(p) & GORO.TERRITORY_PLAYER_MASK; };
	ret.getOwnerIndex = function(p) { return this.map[p] & GORO.TERRITORY_PLAYER_MASK; };
	ret.isBlinking = function(p) { return (this.point(p) & GORO.TERRITORY_BLINKING_MASK) != 0; };
	return ret;
};
...
GORO.outside_Mask = 1;
GORO.border_Mask = 2;
GORO.fullBorder_Mask = GORO.outside_Mask | GORO.border_Mask;
GORO.narrowFrontier_Mask = 4;
GORO.largeFrontier_Mask = 8;
GORO.fullFrontier_Mask = GORO.narrowFrontier_Mask | GORO.largeFrontier_Mask;

GORO.createBorderMap = function(gameState)
{
...
GORO.debugMap = function(gameState, map)
{
	let width = map.width;
