 ...
 */
HYRULE.AttackPlan = function(gameState, Config, uniqueID, type = HYRULE.AttackPlan.TYPE_DEFAULT, data)
{
	this.Config = Config;
	this.name = uniqueID;
	this.type = type;
	this.state = HYRULE.AttackPlan.STATE_UNEXECUTED;
	...
	for (const base of gameState.ai.HQ.baseManagers())
	{
		if (!base.anchor || !base.anchor.position())
			continue;
		let access = HYRULE.getLandAccess(gameState, base.anchor);
		if (!rallyPoint)
		{
	...
	if (!rallyPoint)	// no base ?  take the position of any of our entities
	{
		for (let ent of gameState.getOwnEntities().values())
		{
			if (!ent.position())
				continue;
			let access = HYRULE.getLandAccess(gameState, ent);
	...
	if (gameState.ai.HQ.navalMap)
	{
		for (let structure of gameState.getEnemyStructures().values())
		{
			...
			if (!structure.position())
				continue;
			let access = HYRULE.getLandAccess(gameState, structure);
			...
			else if (!this.overseas)
			{
				...
				if (!sea)
				{
					if (this.target)
					{
						API3.warn("HYRULE: " + this.type + " " + this.name + " has an inaccessible target " +
	...
	if (type === HYRULE.AttackPlan.TYPE_RUSH)
	{
		priority = 150;
	...
	else if (type === HYRULE.AttackPlan.TYPE_RAID)
	{
		priority = 250;
		this.unitStat.FastMoving = { "priority": 3, "minSize": 15, "targetSize": 20, "batchSize": 5, "classes": ["FastMoving+CitizenSoldier"],
			"interests": [ ["strength", 1] ] };
		this.unitStat.Infantry = { "priority": 2, "minSize": 10, "targetSize": 15, "batchSize": 5, "classes": ["Ranged"],
			"interests": [ ["strength", 1] ] };
	...
	let variation = randFloat(0.5, 1.5);
	// and lower priority and smaller sizes for easier difficulty levels
	if (this.Config.difficulty < HYRULE.DIFFICULTY_EASY)
	{
		priority *= 0.4;
		variation *= 0.2;
	}
	else if (this.Config.difficulty < HYRULE.DIFFICULTY_MEDIUM)
	{
		priority *= 0.8;
		variation *= 0.6;
	}
	...
	this.siegeState = HYRULE.AttackPlan.SIEGE_NOT_TESTED;
...
HYRULE.AttackPlan.PREPARATION_FAILED = 0;
HYRULE.AttackPlan.PREPARATION_KEEP_GOING = 1;
HYRULE.AttackPlan.PREPARATION_START = 2;

HYRULE.AttackPlan.SIEGE_NOT_TESTED = 0;
HYRULE.AttackPlan.SIEGE_NO_TRAINER = 1;

/**
 * Siege added in build orders
 */
HYRULE.AttackPlan.SIEGE_ADDED = 2;

HYRULE.AttackPlan.STATE_UNEXECUTED = "unexecuted";
HYRULE.AttackPlan.STATE_COMPLETING = "completing";
HYRULE.AttackPlan.STATE_ARRIVED = "arrived";

HYRULE.AttackPlan.TYPE_DEFAULT = "Attack";
HYRULE.AttackPlan.TYPE_HUGE_ATTACK = "HugeAttack";
HYRULE.AttackPlan.TYPE_RAID = "Raid";
HYRULE.AttackPlan.TYPE_RUSH = "Rush";

HYRULE.AttackPlan.prototype.init = function(gameState)
{
...
HYRULE.AttackPlan.prototype.getName = function()
{
	return this.name;
};

HYRULE.AttackPlan.prototype.getType = function()
{
	return this.type;
};

HYRULE.AttackPlan.prototype.isStarted = function()
{
	return this.state !== HYRULE.AttackPlan.STATE_UNEXECUTED && this.state !== HYRULE.AttackPlan.STATE_COMPLETING;
};

HYRULE.AttackPlan.prototype.isPaused = function()
{
	return this.paused;
};

HYRULE.AttackPlan.prototype.setPaused = function(boolValue)
{
 ...
 */
HYRULE.AttackPlan.prototype.canStart = function()
{
	if (!this.canBuildUnits)
		return true;
...
HYRULE.AttackPlan.prototype.mustStart = function()
{
	if (this.isPaused())
		return false;
	...
	if (MinReachedEverywhere)
		return this.type === HYRULE.AttackPlan.TYPE_RAID && this.target && this.target.foundationProgress() &&
		                                             this.target.foundationProgress() > 50;
	return false;
};

HYRULE.AttackPlan.prototype.forceStart = function()
{
	for (let unitCat in this.unitStat)
	{
		let Unit = this.unitStat[unitCat];
...
HYRULE.AttackPlan.prototype.emptyQueues = function()
{
	this.queue.empty();
...
HYRULE.AttackPlan.prototype.removeQueues = function(gameState)
{
...
HYRULE.AttackPlan.prototype.addBuildOrder = function(gameState, name, unitStats, resetQueue)
{
	if (!this.isStarted())
	{
...
HYRULE.AttackPlan.prototype.addSiegeUnits = function(gameState)
{
	if (this.siegeState === HYRULE.AttackPlan.SIEGE_ADDED || this.state !== HYRULE.AttackPlan.STATE_UNEXECUTED)
		return false;
	...
	{
		this.siegeState = HYRULE.AttackPlan.SIEGE_NO_TRAINER;
		return false;
	}
	...
	this.siegeState = HYRULE.AttackPlan.SIEGE_ADDED;
	let targetSize;
	if (this.Config.difficulty < HYRULE.DIFFICULTY_MEDIUM)
		targetSize = this.type === HYRULE.AttackPlan.TYPE_HUGE_ATTACK ? Math.max(this.Config.difficulty, 1) : Math.max(this.Config.difficulty - 1, 0);
	else
		targetSize = this.type === HYRULE.AttackPlan.TYPE_HUGE_ATTACK ? this.Config.difficulty + 1 : this.Config.difficulty - 1;
	targetSize = Math.max(Math.round(this.Config.popScaling * targetSize), this.type === HYRULE.AttackPlan.TYPE_HUGE_ATTACK ? 1 : 0);
	if (!targetSize)
		return true;
	// no minsize as we don't want the plan to fail at the last minute though.
	let stat = { "priority": 1, "minSize": 0, "targetSize": targetSize, "batchSize": Math.min(targetSize, 3),
...
HYRULE.AttackPlan.prototype.updatePreparation = function(gameState)
{
	...
	if (this.state === HYRULE.AttackPlan.STATE_COMPLETING)
	{
		...
		else
		{
			...
			if (this.waitingForTransport())
				return HYRULE.AttackPlan.PREPARATION_KEEP_GOING;
			...
			if (gameState.ai.elapsedTime < this.maxCompletingTime && this.hasForceOrder())
				return HYRULE.AttackPlan.PREPARATION_KEEP_GOING;
			return HYRULE.AttackPlan.PREPARATION_START;
		}
	}
	...
	if (this.type !== HYRULE.AttackPlan.TYPE_RAID || !this.forced)    // Forced Raids have special purposes (as relic capture)
		this.assignUnits(gameState);
	if (this.type !== HYRULE.AttackPlan.TYPE_RAID && gameState.ai.HQ.attackManager.getAttackInPreparation(HYRULE.AttackPlan.TYPE_RAID) !== undefined)
	...
	if (gameState.getPopulationMax() - gameState.getPopulation() < 5)
	{
		...
		else	// Abort the plan so that its units will be reassigned to other plans.
		{
			if (this.Config.debug > 1)
			{
				...
				API3.warn(" attacks upcoming: raid " + am.upcomingAttacks[HYRULE.AttackPlan.TYPE_RAID].length +
					  " rush " + am.upcomingAttacks[HYRULE.AttackPlan.TYPE_RUSH].length +
					  " attack " + am.upcomingAttacks[HYRULE.AttackPlan.TYPE_DEFAULT].length +
					  " huge " + am.upcomingAttacks[HYRULE.AttackPlan.TYPE_HUGE_ATTACK].length);
				API3.warn(" attacks started: raid " + am.startedAttacks[HYRULE.AttackPlan.TYPE_RAID].length +
					  " rush " + am.startedAttacks[HYRULE.AttackPlan.TYPE_RUSH].length +
					  " attack " + am.startedAttacks[HYRULE.AttackPlan.TYPE_DEFAULT].length +
					  " huge " + am.startedAttacks[HYRULE.AttackPlan.TYPE_HUGE_ATTACK].length);
			}
			return HYRULE.AttackPlan.PREPARATION_FAILED;
		}
	}
	...
	else
	{
		if (this.canBuildUnits)
		{
			...
			if (this.siegeState === HYRULE.AttackPlan.SIEGE_NOT_TESTED ||
				this.siegeState === HYRULE.AttackPlan.SIEGE_NO_TRAINER && gameState.ai.playedTurn % 5 == 0)
			...
			if (!this.buildOrders.length)
				return HYRULE.AttackPlan.PREPARATION_FAILED;	// will abort the plan
		}
		return HYRULE.AttackPlan.PREPARATION_KEEP_GOING;
	}

	// if we're here, it means we must start
	this.state = HYRULE.AttackPlan.STATE_COMPLETING;
	...
	if (this.type === HYRULE.AttackPlan.TYPE_RAID)
		this.maxCompletingTime = this.forced ? 0 : gameState.ai.elapsedTime + 20;
	else
	{
		if (this.type === HYRULE.AttackPlan.TYPE_RUSH || this.forced)
	...
	for (let ent of this.unitCollection.values())
	{
		...
		ent.setMetadata(PlayerID, "role", HYRULE.Worker.ROLE_ATTACK);
		ent.setMetadata(PlayerID, "subrole", HYRULE.Worker.SUBROLE_COMPLETING);
		let queued = false;
		if (ent.resourceCarrying() && ent.resourceCarrying().length)
			queued = HYRULE.returnResources(gameState, ent);
		let index = HYRULE.getLandAccess(gameState, ent);
		if (index == rallyIndex)
...
HYRULE.AttackPlan.prototype.trainMoreUnits = function(gameState)
{
	...
	if (firstOrder[0] < firstOrder[3].targetSize)
	{
		...
		if (queue.length() <= 5)
		{
			...
			else
			{
				...
				data.role = gameState.getTemplate(template).hasClass("CitizenSoldier") ? HYRULE.Worker.ROLE_WORKER : HYRULE.Worker.ROLE_ATTACK;
				let trainingPlan = new HYRULE.TrainingPlan(gameState, template, data, max, max);
...
HYRULE.AttackPlan.prototype.assignUnits = function(gameState)
{
	let plan = this.name;
	let added = false;
	...
	let num = 0;
	const numbase = {};
	let keep = this.type !== HYRULE.AttackPlan.TYPE_RUSH ?
		6 + 4 * gameState.getNumPlayerEnemies() + 8 * this.Config.personality.defensive : 8;
	keep = Math.round(this.Config.popScaling * keep);
	for (const ent of gameState.getOwnEntitiesByRole(HYRULE.Worker.ROLE_WORKER, true).values())
	{
		...
		{
			API3.warn("HYRULE problem ent without base ");
			HYRULE.dumpEntity(ent);
			continue;
		}
		if (num++ < keep || numbase[baseID] < 5)
			continue;
		if (this.type !== HYRULE.AttackPlan.TYPE_RUSH && ent.getMetadata(PlayerID, "subrole") !== HYRULE.Worker.SUBROLE_IDLE)
			continue;
		ent.setMetadata(PlayerID, "plan", plan);
		this.unitCollection.updateEnt(ent);
		added = true;
...
HYRULE.AttackPlan.prototype.isAvailableUnit = function(gameState, ent)
{
	if (!ent.position())
		return false;
...
HYRULE.AttackPlan.prototype.reassignFastUnit = function(gameState)
{
	for (let ent of this.unitCollection.values())
	{
		...
		if (!ent.hasClasses(["FastMoving", "CitizenSoldier"]))
			continue;
		const raid = gameState.ai.HQ.attackManager.getAttackInPreparation(HYRULE.AttackPlan.TYPE_RAID);
...
HYRULE.AttackPlan.prototype.chooseTarget = function(gameState)
{
	...
	let targetIndex = HYRULE.getLandAccess(gameState, this.target);
 ...
 */
HYRULE.AttackPlan.prototype.getNearestTarget = function(gameState, position, sameLand)
{
	...
	else
	{
		...
		else if (this.type === HYRULE.AttackPlan.TYPE_RUSH || this.type === HYRULE.AttackPlan.TYPE_DEFAULT)
		{
	...
	for (let ent of targets.values())
	{
		...
		if (this.type !== HYRULE.AttackPlan.TYPE_RUSH && this.type !== HYRULE.AttackPlan.TYPE_RAID && ent.hasClass("Field"))
			dist += 100000;
		if (dist < minDist)
		{
			minDist = dist;
 ...
 */
HYRULE.AttackPlan.prototype.defaultTargetFinder = function(gameState, playerEnemy)
{
...
HYRULE.AttackPlan.prototype.isValidTarget = function(ent)
{
	if (!ent.position())
		return false;
	if (this.sameLand && HYRULE.getLandAccess(this.gameState, ent) != this.sameLand)
		return false;
...
HYRULE.AttackPlan.prototype.rushTargetFinder = function(gameState, playerEnemy)
{
	let targets = new API3.EntityCollection(gameState.sharedScript);
	let buildings;
	...
	if (!targets.hasEntities() && this.type === HYRULE.AttackPlan.TYPE_RUSH && playerEnemy)
...
HYRULE.AttackPlan.prototype.raidTargetFinder = function(gameState)
{
	let targets = new API3.EntityCollection(gameState.sharedScript);
 ...
 */
HYRULE.AttackPlan.prototype.checkTargetObstruction = function(gameState, target, position)
{
	if (HYRULE.getLandAccess(gameState, target) != gameState.ai.accessibility.getAccessValue(position))
		return target;
...
HYRULE.AttackPlan.prototype.getPathToTarget = function(gameState, fixedRallyPoint = false)
{
	let startAccess = gameState.ai.accessibility.getAccessValue(this.rallyPoint);
	let endAccess = HYRULE.getLandAccess(gameState, this.target);
...
HYRULE.AttackPlan.prototype.setRallyPoint = function(gameState)
{
 ...
 */
HYRULE.AttackPlan.prototype.StartAttack = function(gameState)
{
	if (this.Config.debug > 1)
	...
	for (let ent of this.unitCollection.values())
	{
		ent.setMetadata(PlayerID, "subrole", HYRULE.Worker.SUBROLE_WALKING);
	...
	let targetAccess = HYRULE.getLandAccess(gameState, this.target);
...
HYRULE.AttackPlan.prototype.update = function(gameState, events)
{
	...
	if (this.state === HYRULE.AttackPlan.STATE_ARRIVED)
	{
		...
		this.unitCollection.forEach(ent => {
			ent.stopMoving();
			ent.setMetadata(PlayerID, "subrole", HYRULE.Worker.SUBROLE_ATTACKING);
		});
		if (this.type === HYRULE.AttackPlan.TYPE_RUSH)   // try to find a better target for rush
		{
	...
	if (this.state == "")
	{
		...
		for (let evt of events.Attacked)
		{
			...
			{	// if our siege units are attacked, we'll send some units to deal with enemies.
				...
				{
					if (HYRULE.isSiegeUnit(ent))	// needed as mauryan elephants are not filtered out
						continue;

					let allowCapture = HYRULE.allowCapture(gameState, ent, attacker);
				...
				if (HYRULE.isSiegeUnit(attacker) && attacker.hasClass("Melee") && ourUnit.hasClass("Melee") && ourUnit.canAttackTarget(attacker, HYRULE.allowCapture(gameState, ourUnit, attacker)))
				{
					ourUnit.attack(attacker.id(), HYRULE.allowCapture(gameState, ourUnit, attacker));
			...
			else
			{
				...
				else if (HYRULE.isSiegeUnit(attacker))
				...
				else
				{
					...
					{
						let allowCapture = HYRULE.allowCapture(gameState, ent, attacker);
						if (HYRULE.isSiegeUnit(ent) || !ent.canAttackTarget(attacker, allowCapture))
							continue;
					...
					let allowCapture = HYRULE.allowCapture(gameState, ourUnit, attacker);
		...
		if (this.type === HYRULE.AttackPlan.TYPE_RUSH)
		...
		for (let check = 0; check < lgth; check++)
		{
			...
			let siegeUnit = HYRULE.isSiegeUnit(ent);
			if (ent.isIdle())
				needsUpdate = true;
			...
			if (this.isBlocked)
			{
				...
				range += 30;
			range *= range;
			let entAccess = HYRULE.getLandAccess(gameState, ent);
			// Checking for gates if we're a siege unit.
			if (siegeUnit)
			{
				let mStruct = enemyStructures.filter(enemy => {
					if (!enemy.position() || !ent.canAttackTarget(enemy, HYRULE.allowCapture(gameState, ent, enemy)))
						return false;
					if (API3.SquareVectorDistance(enemy.position(), ent.position()) > range)
						return false;
					if (enemy.foundationProgress() == 0)
						return false;
					if (HYRULE.getLandAccess(gameState, enemy) != entAccess)
						return false;
					return true;
				}).toEntityArray();
				if (mStruct.length)
				{
					...
					if (mStruct[0].hasClass("Gate"))
						ent.attack(mStruct[0].id(), HYRULE.allowCapture(gameState, ent, mStruct[0]));
					else
					{
						let rand = randIntExclusive(0, mStruct.length * 0.2);
						ent.attack(mStruct[rand].id(), HYRULE.allowCapture(gameState, ent, mStruct[rand]));
					}
				}
				else
				{
			...
			else
			{
				...
				if (mUnit.length)
				{
					...
					ent.attack(mUnit[rand].id(), HYRULE.allowCapture(gameState, ent, mUnit[rand]));
				}
					...
					else if (needsUpdate)  // really nothing   let's try to help our nearest unit
					{
						...
						this.unitCollection.forEach(unit => {
							...
							if (dist > distmin)
								return;
							distmin = dist;
							if (!ent.canAttackTarget(target, HYRULE.allowCapture(gameState, ent, target)))
								return;
							attacker = target;
						});
						if (attacker)
							ent.attack(attacker.id(), HYRULE.allowCapture(gameState, ent, attacker));
					}
				}
			}
		}
...
HYRULE.AttackPlan.prototype.UpdateTransporting = function(gameState, events)
{
	let done = true;
	for (let ent of this.unitCollection.values())
	{
	...
	if (done)
	{
		this.state = HYRULE.AttackPlan.STATE_ARRIVED;
		return;
	}
...
HYRULE.AttackPlan.prototype.UpdateWalking = function(gameState, events)
{
...
HYRULE.AttackPlan.prototype.UpdateTarget = function(gameState)
{
	...
	if (this.target && this.target.hasClass("Unit"))
	{
		...
		if (!this.targetPos)
		{
			let holder = HYRULE.getHolder(gameState, this.target);
			...
			for (let attackType in attackManager.startedAttacks)
			{
				for (let attack of attackManager.startedAttacks[attackType])
				{
					...
					if (!attack.target || !gameState.getEntityById(attack.target.id()) ||
						...
						continue;
					if (accessIndex != HYRULE.getLandAccess(gameState, attack.target))
						continue;
...
/** reset any units */
HYRULE.AttackPlan.prototype.Abort = function(gameState)
{
	...
	if (this.unitCollection.hasEntities())
	{
		...
		if (this.isStarted())
		{
			...
			for (const base of gameState.ai.HQ.baseManagers())
			{
				if (!base.anchor || !base.anchor.position())
					continue;
				if (HYRULE.getLandAccess(gameState, base.anchor) != access)
					continue;
		...
		for (let ent of this.unitCollection.values())
		{
			if (ent.getMetadata(PlayerID, "role") === HYRULE.Worker.ROLE_ATTACK)
				ent.stopMoving();
			if (rallyPoint)
...
HYRULE.AttackPlan.prototype.removeUnit = function(ent, update)
{
	if (ent.getMetadata(PlayerID, "role") === HYRULE.Worker.ROLE_ATTACK)
	{
		if (ent.hasClass("CitizenSoldier"))
			ent.setMetadata(PlayerID, "role", HYRULE.Worker.ROLE_WORKER);
		else
...
HYRULE.AttackPlan.prototype.checkEvents = function(gameState, events)
{
	for (let evt of events.EntityRenamed)
	{
		if (!this.target || this.target.id() != evt.entity)
			continue;
		if (this.type === HYRULE.AttackPlan.TYPE_RAID && !this.isStarted())
			this.target = undefined;
		else
	...
	if (!this.overseas || this.state !== HYRULE.AttackPlan.STATE_UNEXECUTED)
		return;
...
HYRULE.AttackPlan.prototype.waitingForTransport = function()
{
	for (let ent of this.unitCollection.values())
		if (ent.getMetadata(PlayerID, "transport") !== undefined)
			return true;
...
HYRULE.AttackPlan.prototype.hasSiegeUnits = function()
{
	for (let ent of this.unitCollection.values())
		if (HYRULE.isSiegeUnit(ent))
			return true;
	return false;
};

HYRULE.AttackPlan.prototype.hasForceOrder = function(data, value)
{
	for (let ent of this.unitCollection.values())
	{
 ...
 */
HYRULE.AttackPlan.prototype.getAttackAccess = function(gameState)
{
	for (let ent of this.unitCollection.filterNearest(this.position, 1).values())
		return HYRULE.getLandAccess(gameState, ent);

	return 0;
};

HYRULE.AttackPlan.prototype.debugAttack = function()
{
...
HYRULE.AttackPlan.prototype.Serialize = function()
{
	let properties = {
		"name": this.name,
		"type": this.type,
...
HYRULE.AttackPlan.prototype.Deserialize = function(gameState, data)
{
