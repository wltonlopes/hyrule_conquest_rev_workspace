 ...
 */
HYRULE.DefenseArmy = function(gameState, foeEntities, type)
{
 ...
 */
HYRULE.DefenseArmy.prototype.addFoe = function(gameState, enemyId, force)
{
 ...
 */
HYRULE.DefenseArmy.prototype.removeFoe = function(gameState, enemyId, enemyEntity)
{
 ...
 */
HYRULE.DefenseArmy.prototype.addOwn = function(gameState, id, force)
{
	...
	if (subrole === undefined || subrole !== HYRULE.Worker.SUBROLE_DEFENDER)
		ent.setMetadata(PlayerID, "formerSubrole", subrole);
	ent.setMetadata(PlayerID, "subrole", HYRULE.Worker.SUBROLE_DEFENDER);
	return true;
};

HYRULE.DefenseArmy.prototype.removeOwn = function(gameState, id, Entity)
{
	...
	if (ent.getMetadata(PlayerID, "transport") !== undefined)
	{
		...
		if (plan && plan.state === HYRULE.TransportPlan.BOARDING && ent.position())
 ...
 */
HYRULE.DefenseArmy.prototype.clear = function(gameState)
{
	...
	else
	{
		...
		for (let struct of gameState.getAllyStructures().values())
		{
			...
			if (!pos || !gameState.isPlayerMutualAlly(gameState.ai.HQ.territoryMap.getOwner(pos)))
				continue;
			if (HYRULE.getLandAccess(gameState, struct) !== armyAccess)
				continue;
...
HYRULE.DefenseArmy.prototype.assignUnit = function(gameState, entID)
{
	...
	for (let id of this.foeEntities)
	{
		...
		if (!ent.canAttackTarget(eEnt, HYRULE.allowCapture(gameState, ent, eEnt)))
			continue;
		...
		if (this.assignedAgainst[id].length > 10 ||
			this.assignedAgainst[id].length > 8 && !eEnt.hasClass("Hero") && !HYRULE.isSiegeUnit(eEnt))
			continue;
	...
	let ownIndex = HYRULE.getLandAccess(gameState, ent);
	...
	if (ownIndex == foeIndex || ent.hasClass("Ship"))
	{
		...
		ent.attack(idFoe, HYRULE.allowCapture(gameState, ent, foeEnt), queued);
	}
	else
...
HYRULE.DefenseArmy.prototype.getType = function()
{
	return this.type;
};

HYRULE.DefenseArmy.prototype.getState = function()
{
 ...
 */
HYRULE.DefenseArmy.prototype.merge = function(gameState, otherArmy)
{
...
HYRULE.DefenseArmy.prototype.needsDefenders = function(gameState)
{
	let defenseRatio;
...
HYRULE.DefenseArmy.prototype.recalculatePosition = function(gameState, force)
{
...
HYRULE.DefenseArmy.prototype.recalculateStrengths = function(gameState)
{
	this.ownStrength = 0;
	this.foeStrength = 0;
...
HYRULE.DefenseArmy.prototype.evaluateStrength = function(ent, isOwn, remove)
{
	if (!ent)
		return;
	...
	if (ent.hasClass("Structure"))
	{
		...
		entStrength = HYRULE.getMaxStrength(ent, this.Config.debug, this.Config.DamageTypeImportance);
...
HYRULE.DefenseArmy.prototype.checkEvents = function(gameState, events)
{
...
HYRULE.DefenseArmy.prototype.update = function(gameState)
{
	for (let entId of this.ownEntities)
	{
		let ent = gameState.getEntityById(entId);
		...
		else if (orderData.length && orderData[0].target && orderData[0].attackType && orderData[0].attackType === "Capture")
		{
			...
			if (target && !HYRULE.allowCapture(gameState, ent, target))
...
HYRULE.DefenseArmy.prototype.Serialize = function()
{
	return {
		"ID": this.ID,
		"type": this.type,
...
HYRULE.DefenseArmy.prototype.Deserialize = function(data)
{
	for (let key in data)
		this[key] = data[key];
};
