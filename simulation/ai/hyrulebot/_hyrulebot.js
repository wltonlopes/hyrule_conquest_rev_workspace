...
HYRULE.HyruleBot = function(settings)
{
	...
	this.Config = new HYRULE.Config(settings.difficulty, settings.behavior);

	this.savedEvents = {};
};

HYRULE.HyruleBot.prototype = Object.create(API3.BaseAI.prototype);

HYRULE.HyruleBot.prototype.CustomInit = function(gameState)
{
	...
	else
	{
		...
		for (let i in this.Config.priorities)
			this.queues[i] = new HYRULE.Queue();

		this.queueManager = new HYRULE.QueueManager(this.Config, this.queues);
...
HYRULE.HyruleBot.prototype.OnUpdate = function(sharedScript)
{
	...
	{
		Engine.ProfileStart("HyruleBot bot (player " + this.player +")");
...
HYRULE.HyruleBot.prototype.Serialize = function()
{
	let savedEvents = {};
	for (let key in this.savedEvents)
	{
...
HYRULE.HyruleBot.prototype.Deserialize = function(data, sharedScript)
{
