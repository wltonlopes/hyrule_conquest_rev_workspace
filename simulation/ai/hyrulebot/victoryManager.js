...
HYRULE.VictoryManager = function(Config)
{
	this.Config = Config;
 ...
 */
HYRULE.VictoryManager.prototype.init = function(gameState)
{
 ...
 */
HYRULE.VictoryManager.prototype.checkEvents = function(gameState, events)
{
	if (gameState.getVictoryConditions().has("wonder"))
	{
	...
	if (gameState.getVictoryConditions().has("regicide"))
	{
		...
		for (let evt of events.TrainingFinished)
			for (let entId of evt.entities)
			{
				...
				if (ent && ent.isOwn(PlayerID) && ent.getMetadata(PlayerID, "role") === HYRULE.Worker.ROLE_CRITICAL_ENT_HEALER)
	...
	for (let evt of events.UnGarrison)
	{
		...
		if ((ent.getMetadata(PlayerID, "role") === HYRULE.Worker.ROLE_CRITICAL_ENT_HEALER ||
		     ent.getMetadata(PlayerID, "role") === HYRULE.Worker.ROLE_CRITICAL_ENT_GUARD) && !this.guardEnts.get(evt.entity))
		{
...
HYRULE.VictoryManager.prototype.removeCriticalEnt = function(gameState, criticalEntId)
{
 ...
 */
HYRULE.VictoryManager.prototype.manageCriticalEntHealers = function(gameState, queues)
{
	...
	for (let data of this.criticalEnts.values())
	{
		...
		queues.healer.addPlan(new HYRULE.TrainingPlan(gameState, template, { "role": HYRULE.Worker.ROLE_CRITICAL_ENT_HEALER, "base": 0 }, 1, 1));
		return;
	}
};
 ...
 */
HYRULE.VictoryManager.prototype.manageCriticalEntGuards = function(gameState)
{
	let numWorkers = gameState.getOwnEntitiesByRole(HYRULE.Worker.ROLE_WORKER, true).length;
	if (numWorkers < 20)
	{
		for (let data of this.criticalEnts.values())
		{
			for (let guardId of data.guards.keys())
			{
				...
				if (!guardEnt || !guardEnt.hasClass("CitizenSoldier") ||
				    guardEnt.getMetadata(PlayerID, "role") !== HYRULE.Worker.ROLE_CRITICAL_ENT_GUARD)
					continue;
...
HYRULE.VictoryManager.prototype.tryAssignMilitaryGuard = function(gameState, guardEnt, criticalEnt, checkForSameAccess)
{
	if (guardEnt.getMetadata(PlayerID, "plan") !== undefined ||
	    ...
	    HYRULE.getLandAccess(gameState, criticalEnt) != HYRULE.getLandAccess(gameState, guardEnt)))
		return false;
	...
	guardEnt.setMetadata(PlayerID, "role", HYRULE.Worker.ROLE_CRITICAL_ENT_GUARD);
	return true;
};

HYRULE.VictoryManager.prototype.pickCriticalEntRetreatLocation = function(gameState, criticalEnt, emergency)
{
	...
	let accessIndex = HYRULE.getLandAccess(gameState, criticalEnt);
	let bestBase = HYRULE.getBestBase(gameState, criticalEnt, true);
 ...
 */
HYRULE.VictoryManager.prototype.assignGuardToCriticalEnt = function(gameState, guardEnt, criticalEntId)
{
	...
	let guardEntAccess = HYRULE.getLandAccess(gameState, guardEnt);
	let criticalEntAccess = HYRULE.getLandAccess(gameState, criticalEnt);
	if (guardEntAccess == criticalEntAccess)
	{
		let queued = HYRULE.returnResources(gameState, guardEnt);
		guardEnt.guard(criticalEnt, queued);
		const guardRole = guardEnt.getMetadata(PlayerID, "role") === HYRULE.Worker.ROLE_CRITICAL_ENT_HEALER ? "healer" : "guard";
...
HYRULE.VictoryManager.prototype.resetCaptureGaiaRelic = function(gameState)
{
...
HYRULE.VictoryManager.prototype.update = function(gameState, events, queues)
{
 ...
 */
HYRULE.VictoryManager.prototype.captureGaiaRelic = function(gameState, relic)
{
	...
	for (let plan of plans)
	{
		...
		for (let ent of attack.unitCollection.values())
			capture += ent.captureStrength() * HYRULE.getAttackBonus(ent, relic, "Capture");
	}
	// No need to make a new attack if already enough units
	if (capture > sumCapturePoints / 50)
		return;
	let relicPosition = relic.position();
	let access = HYRULE.getLandAccess(gameState, relic);
	let units = gameState.getOwnUnits().filter(ent => {
		...
		{
			let attack = gameState.ai.HQ.attackManager.getPlan(plan);
			if (attack && (attack.state !== HYRULE.AttackPlan.STATE_UNEXECUTED || attack.type === HYRULE.AttackPlan.TYPE_RAID))
				return false;
		}
...
HYRULE.VictoryManager.prototype.abortCaptureGaiaRelic = function(gameState, relicId)
{
...
HYRULE.VictoryManager.prototype.Serialize = function()
{
	return {
...
HYRULE.VictoryManager.prototype.Deserialize = function(data)
{
	for (let key in data)
		this[key] = data[key];
};
