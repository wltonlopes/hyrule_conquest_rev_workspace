...
HYRULE.HQ = function(Config)
{
	...
	this.basesManager = new HYRULE.BasesManager(this.Config);
	this.attackManager = new HYRULE.AttackManager(this.Config);
	this.buildManager = new HYRULE.BuildManager();
	this.defenseManager = new HYRULE.DefenseManager(this.Config);
	this.tradeManager = new HYRULE.TradeManager(this.Config);
	this.navalManager = new HYRULE.NavalManager(this.Config);
	this.researchManager = new HYRULE.ResearchManager(this.Config);
	this.diplomacyManager = new HYRULE.DiplomacyManager(this.Config);
	this.garrisonManager = new HYRULE.GarrisonManager(this.Config);
	this.victoryManager = new HYRULE.VictoryManager(this.Config);
	this.emergencyManager = new HYRULE.EmergencyManager(this.Config);
...
HYRULE.HQ.prototype.init = function(gameState, queues)
{
	this.territoryMap = HYRULE.createTerritoryMap(gameState);
	...
	this.borderMap = HYRULE.createBorderMap(gameState);
 ...
 */
HYRULE.HQ.prototype.postinit = function(gameState)
{
 ...
 */
HYRULE.HQ.prototype.getSeaBetweenIndices = function(gameState, index1, index2)
{
...
HYRULE.HQ.prototype.checkEvents = function(gameState, events)
{
	...
	for (let evt of events.OwnershipChanged)   // capture events
	{
		...
		{
			ent.setMetadata(PlayerID, "role", HYRULE.Worker.ROLE_TRADER);
			ent.setMetadata(PlayerID, "route", undefined);
		}
		if (ent.hasClass("Worker"))
		{
			ent.setMetadata(PlayerID, "role", HYRULE.Worker.ROLE_WORKER);
			ent.setMetadata(PlayerID, "subrole", HYRULE.Worker.SUBROLE_IDLE);
		}
		if (ent.hasClass("Ship"))
			HYRULE.setSeaAccess(gameState, ent);
	...
	for (let evt of events.TrainingFinished)
	{
		for (let entId of evt.entities)
		{
			...
			if (plan !== undefined && plan >= 0)
			{
				...
				if (!attack || attack.state !== HYRULE.AttackPlan.STATE_UNEXECUTED)
					ent.setMetadata(PlayerID, "plan", -1);
			}
		}
	}
...
HYRULE.HQ.prototype.handleNewBase = function(gameState)
{
...
HYRULE.HQ.prototype.checkPhaseRequirements = function(gameState, queues)
{
	...
	for (let entityReq of requirements)
	{
		...
		case "Town":
			    ...
			    !queues.militaryBuilding.hasQueuedUnits())
			{
				...
				{
					plan = new HYRULE.ConstructionPlan(gameState, "structures/{civ}/market", { "phaseUp": true });
				...
				{
					plan = new HYRULE.ConstructionPlan(gameState, "structures/{civ}/temple", { "phaseUp": true });
					queue = "economicBuilding";
					break;
				}
				...
				{
					plan = new HYRULE.ConstructionPlan(gameState, "structures/{civ}/forge", { "phaseUp": true });
		...
		default:
			...
			if (!queues[queue].hasQueuedUnits())
			{
				...
				if (structure && this.canBuild(gameState, structure))
					plan = new HYRULE.ConstructionPlan(gameState, structure, { "phaseUp": true });
			}
		}
...
HYRULE.HQ.prototype.OnPhaseUp = function(gameState, phase)
{
};

/** This code trains citizen workers, trying to keep close to a ratio of worker/soldiers */
HYRULE.HQ.prototype.trainMoreWorkers = function(gameState, queues)
{
	// default template
	...
	gameState.getOwnUnits().forEach(ent => {
		if (ent.getMetadata(PlayerID, "role") === HYRULE.Worker.ROLE_WORKER && ent.getMetadata(PlayerID, "plan") === undefined)
		{
			++numberOfWorkers;
	...
	gameState.getOwnTrainingFacilities().forEach(function(ent) {
		for (let item of ent.trainingQueue())
		{
			...
			if (item.metadata && item.metadata.role && item.metadata.role === HYRULE.Worker.ROLE_WORKER &&
	...
	if (this.attackManager.rushNumber < this.attackManager.maxRushes || this.attackManager.upcomingAttacks[HYRULE.AttackPlan.TYPE_RUSH].length)
		alpha = 0.7;
	...
	if (!template && templateDef)
		queues.villager.addPlan(new HYRULE.TrainingPlan(gameState, templateDef, { "role": HYRULE.Worker.ROLE_WORKER, "base": 0, "support": true }, size, size));
	else if (template)
		queues.citizenSoldier.addPlan(new HYRULE.TrainingPlan(gameState, template, { "role": HYRULE.Worker.ROLE_WORKER, "base": 0 }, size, size));
};

/** picks the best template based on parameters and classes */
HYRULE.HQ.prototype.findBestTrainableUnit = function(gameState, classes, requirements)
{
	let units;
	...
	units.sort((a, b) => {
		...
		for (let param of parameters)
		{
			if (param[0] == "strength")
			{
				aValue += HYRULE.getMaxStrength(a[1], gameState.ai.Config.debug, gameState.ai.Config.DamageTypeImportance) * param[1];
				bValue += HYRULE.getMaxStrength(b[1], gameState.ai.Config.debug, gameState.ai.Config.DamageTypeImportance) * param[1];
			}
			else if (param[0] == "siegeStrength")
			{
				aValue += HYRULE.getMaxStrength(a[1], gameState.ai.Config.debug, gameState.ai.Config.DamageTypeImportance, "Structure") * param[1];
				bValue += HYRULE.getMaxStrength(b[1], gameState.ai.Config.debug, gameState.ai.Config.DamageTypeImportance, "Structure") * param[1];
			}
 ...
 */
HYRULE.HQ.prototype.bulkPickWorkers = function(gameState, baseRef, number)
{
	return this.basesManager.bulkPickWorkers(gameState, baseRef, number);
};

HYRULE.HQ.prototype.getTotalResourceLevel = function(gameState, resources, proximity)
{
 ...
 */
HYRULE.HQ.prototype.GetCurrentGatherRates = function(gameState)
{
 ...
 */
HYRULE.HQ.prototype.GetWantedGatherRates = function(gameState)
{
 ...
 */
HYRULE.HQ.prototype.pickMostNeededResources = function(gameState, allowedResources = [])
{
 ...
 */
HYRULE.HQ.prototype.findEconomicCCLocation = function(gameState, template, resource, proximity, fromStrategic)
{
	...
	// obstruction map
	let obstructions = HYRULE.createObstructionMap(gameState, 0, template);
	let halfSize = 0;
	...
	for (let j = 0; j < this.territoryMap.length; ++j)
	{
		...
		else
		{
			...
			for (let cc of ccList)
			{
				...
				if (dist < minDist)
					minDist = dist;
				accessible = accessible || index == HYRULE.getLandAccess(gameState, cc.ent);
			}
			if (norm == 0)
				continue;
			...
			oversea = !accessible && dpList.some(dp => HYRULE.getLandAccess(gameState, dp.ent) == index);
			if (!oversea)
			{
		...
		if (this.borderMap.map[j] & HYRULE.fullBorder_Mask)	// disfavor the borders of the map
			norm *= 0.5;
 ...
 */
HYRULE.HQ.prototype.findStrategicCCLocation = function(gameState, template)
{
	...
	for (let j = 0; j < this.territoryMap.length; ++j)
	{
		...
		if (this.borderMap.map[j] & HYRULE.fullBorder_Mask)
			currentVal += 10000;
 ...
 */
HYRULE.HQ.prototype.findMarketLocation = function(gameState, template)
{
	...
	for (let j = 0; j < this.territoryMap.length; ++j)
	{
		...
		if (this.borderMap.map[j] & HYRULE.narrowFrontier_Mask)
			continue;
		...
		for (let market of markets)
		{
			...
			{
				if (HYRULE.getSeaAccess(gameState, market) != gameState.ai.accessibility.getAccessValue(pos, true))
					continue;
				gainMultiplier = traderTemplatesGains.navalGainMultiplier;
			}
			else if (HYRULE.getLandAccess(gameState, market) == index &&
				!HYRULE.isLineInsideEnemyTerritory(gameState, market.position(), pos))
 ...
 */
HYRULE.HQ.prototype.findDefensiveLocation = function(gameState, template)
{
	...
	for (let j = 0; j < this.territoryMap.length; ++j)
	{
		if (!wonderMode)
		{
			...
			if (!(this.borderMap.map[j] & HYRULE.fullFrontier_Mask))
				continue;
			if (this.borderMap.map[j] & HYRULE.largeFrontier_Mask && isTower)
				continue;
		}
...
HYRULE.HQ.prototype.buildTemple = function(gameState, queues)
{
	...
	if (this.canBuild(gameState, "structures/{civ}/temple_vesta"))
		...
		return;
	queues.economicBuilding.addPlan(new HYRULE.ConstructionPlan(gameState, templateName));
};

HYRULE.HQ.prototype.buildMarket = function(gameState, queues)
{
	if (gameState.getOwnEntitiesByClass("Market", true).hasEntities() ||
		!this.canBuild(gameState, "structures/{civ}/market"))
		return;
	...
	let plan = new HYRULE.ConstructionPlan(gameState, "structures/{civ}/market");
	plan.queueToReset = "economicBuilding";
	queues.economicBuilding.addPlan(plan);
};

/** Build a farmstead */
HYRULE.HQ.prototype.buildFarmstead = function(gameState, queues)
{
	...
	queues.economicBuilding.addPlan(new HYRULE.ConstructionPlan(gameState, "structures/{civ}/farmstead"));
};
 ...
 */
HYRULE.HQ.prototype.buildWonder = function(gameState, queues, force = false)
{
	...
	queues.wonder.addPlan(new HYRULE.ConstructionPlan(gameState, "structures/{civ}/wonder"));
};

/** Build a corral, and train animals there */
HYRULE.HQ.prototype.manageCorral = function(gameState, queues)
{
	                ...
	                nCorral < this.currentPhase && gameState.getPopulation() > 30 * nCorral)
	{
		...
		{
			queues.corral.addPlan(new HYRULE.ConstructionPlan(gameState, "structures/{civ}/corral"));
			return;
		}
		if (!nCorral)
			return;
	...
	for (let corral of gameState.getOwnEntitiesByClass("Corral", true).values())
	{
		...
		for (let trainable of trainables)
		{
			...
			if (count > nCorral)
				continue;
			queues.corral.addPlan(new HYRULE.TrainingPlan(gameState, trainable, { "trainer": corral.id() }));
			return;
		}
	}
};
 ...
 */
HYRULE.HQ.prototype.buildMoreHouses = function(gameState, queues)
{
	...
	{
		let plan = new HYRULE.ConstructionPlan(gameState, houseTemplateString);
...
HYRULE.HQ.prototype.checkBaseExpansion = function(gameState, queues)
{
...
HYRULE.HQ.prototype.buildNewBase = function(gameState, queues, resource)
{
	...
	queues.civilCentre.addPlan(new HYRULE.ConstructionPlan(gameState, template, { "base": -1, "resource": resource }));
	return true;
};

/** Deals with building fortresses and towers along our border with enemies. */
HYRULE.HQ.prototype.buildDefenses = function(gameState, queues)
{
	...
	if (!this.saveResources && (this.currentPhase > 2 || gameState.isResearching(gameState.getPhaseName(3))))
	{
		...
		if (this.canBuild(gameState, "structures/{civ}/fortress"))
		{
				...
				gameState.getOwnFoundationsByClass("Fortress").length < 2)
			{
				...
				let plan = new HYRULE.ConstructionPlan(gameState, "structures/{civ}/fortress");
	...
	if (this.Config.Military.numSentryTowers && this.currentPhase < 2 && this.canBuild(gameState, "structures/{civ}/sentry_tower"))
	{
		...
		if (numTowers < this.Config.Military.numSentryTowers && gameState.ai.elapsedTime > towerLapseTime + this.fortStartTime)
		{
			...
			queues.defenseBuilding.addPlan(new HYRULE.ConstructionPlan(gameState, "structures/{civ}/sentry_tower"));
		}
		return;
	}
		...
		gameState.getOwnFoundationsByClass("Tower").length < 3)
	{
		...
		let plan = new HYRULE.ConstructionPlan(gameState, "structures/{civ}/defense_tower");
		plan.queueToReset = "defenseBuilding";
		queues.defenseBuilding.addPlan(plan);
	}
};

HYRULE.HQ.prototype.buildForge = function(gameState, queues)
{
	...
	if (this.canBuild(gameState, "structures/{civ}/forge"))
		queues.militaryBuilding.addPlan(new HYRULE.ConstructionPlan(gameState, "structures/{civ}/forge"));
};
 ...
 */
HYRULE.HQ.prototype.constructTrainingBuildings = function(gameState, queues)
{
	    ...
	    this.phasing == 2 && gameState.getOwnStructures().filter(API3.Filters.byClass("Village")).length < 5)
	{
		...
		if (numBarracks + numRanges == 0)
		{
			...
			if (template)
			{
				...
				let plan = new HYRULE.ConstructionPlan(gameState, template, { "militaryBase": true });
	...
	if (!nAdvanced || nAdvanced < this.bAdvanced.length && this.getAccountedPopulation(gameState) > 110)
	{
		for (let advanced of this.bAdvanced)
		{
			...
			if (template.hasDefensiveFire() || template.trainableEntities(civ))
				queues.militaryBuilding.addPlan(new HYRULE.ConstructionPlan(gameState, advanced, { "militaryBase": true }));
			else	// not a military building, but still use this queue
				queues.militaryBuilding.addPlan(new HYRULE.ConstructionPlan(gameState, advanced));
			return;
		}
	}
};
...
HYRULE.HQ.prototype.findBestBaseForMilitary = function(gameState)
{
	...
	for (let cce of ccEnts)
	{
		...
		if (enemyFound && !gameState.isPlayerEnemy(cce.owner()))
			continue;
		let access = HYRULE.getLandAccess(gameState, cce);
		let isEnemy = gameState.isPlayerEnemy(cce.owner());
		for (let cc of ccEnts)
		{
			if (cc.owner() != PlayerID)
				continue;
			if (HYRULE.getLandAccess(gameState, cc) != access)
				continue;
 ...
 */
HYRULE.HQ.prototype.trainEmergencyUnits = function(gameState, positions)
{
	...
	const metadata = { "role": HYRULE.Worker.ROLE_WORKER, "base": nearestAnchor.getMetadata(PlayerID, "base"), "plan": -1, "trainer": nearestAnchor.id() };
	if (autogarrison)
		metadata.garrisonType = HYRULE.GarrisonManager.TYPE_PROTECTION;
	gameState.ai.queues.emergency.addPlan(new HYRULE.TrainingPlan(gameState, templateFound[0], metadata, 1, 1));
	return true;
};

HYRULE.HQ.prototype.canBuild = function(gameState, structure)
{
...
HYRULE.HQ.prototype.updateTerritories = function(gameState)
{
	...
	for (let j = 0; j < this.territoryMap.length; ++j)
	{
		if (this.borderMap.map[j] & HYRULE.outside_Mask)
			continue;
		if (this.borderMap.map[j] & HYRULE.fullFrontier_Mask)
			this.borderMap.map[j] &= ~HYRULE.fullFrontier_Mask;	// reset the frontier
		...
		else
		{
			...
			for (let a of around)
			{
				...
				if (jz < 0 || jz >= width)
					continue;
				if (this.borderMap.map[jx+width*jz] & HYRULE.outside_Mask)
					continue;
				...
				{
					this.borderMap.map[j] |= HYRULE.narrowFrontier_Mask;
					break;
				}
			...
			if (onFrontier && !(this.borderMap.map[j] & HYRULE.narrowFrontier_Mask))
				this.borderMap.map[j] |= HYRULE.largeFrontier_Mask;
 ...
 */
HYRULE.HQ.prototype.getBaseByID = function(baseID)
{
 ...
 */
HYRULE.HQ.prototype.numActiveBases = function()
{
	return this.basesManager.numActiveBases();
};

HYRULE.HQ.prototype.hasActiveBase = function()
{
	return this.basesManager.hasActiveBase();
};

HYRULE.HQ.prototype.numPotentialBases = function()
{
	return this.basesManager.numPotentialBases();
};

HYRULE.HQ.prototype.hasPotentialBase = function()
{
	return this.basesManager.hasPotentialBase();
};

HYRULE.HQ.prototype.isDangerousLocation = function(gameState, pos, radius)
{
...
HYRULE.HQ.prototype.isNearInvadingArmy = function(pos)
{
...
HYRULE.HQ.prototype.isUnderEnemyFire = function(gameState, pos, radius = 0)
{
...
HYRULE.HQ.prototype.updateCaptureStrength = function(gameState)
{
	...
	for (let ent of gameState.getOwnUnits().values())
	{
		...
		if (!this.capturableTargets.has(targetId))
			this.capturableTargets.set(targetId, {
				"strength": ent.captureStrength() * HYRULE.getAttackBonus(ent, target, "Capture"),
				"ents": new Set([ent.id()])
			});
		else
		{
			let capturableTarget = this.capturableTargets.get(target.id());
			capturableTarget.strength += ent.captureStrength() * HYRULE.getAttackBonus(ent, target, "Capture");
	...
	for (let [targetId, capturableTarget] of this.capturableTargets)
	{
		...
		for (let entId of capturableTarget.ents)
		{
			...
			if (allowCapture === undefined)
				allowCapture = HYRULE.allowCapture(gameState, ent, target);
			let orderData = ent.unitAIOrderData();
 ...
 */
HYRULE.HQ.prototype.isDefendable = function(ent)
{
 ...
 */
HYRULE.HQ.prototype.getAccountedPopulation = function(gameState)
{
 ...
 */
HYRULE.HQ.prototype.getAccountedWorkers = function(gameState)
{
	if (this.turnCache.accountedWorkers == undefined)
	{
		let workers = gameState.getOwnEntitiesByRole(HYRULE.Worker.ROLE_WORKER, true).length;
		for (let ent of gameState.getOwnTrainingFacilities().values())
		{
			for (let item of ent.trainingQueue())
			{
...
HYRULE.HQ.prototype.baseManagers = function()
{
 ...
 */
HYRULE.HQ.prototype.baseAtIndex = function(territoryIndex)
{
 ...
 */
HYRULE.HQ.prototype.update = function(gameState, queues, events)
{
	...
	*/
	...
	if (gameState.ai.playedTurn % 3 == 0)
	{
		...
		if (this.Config.difficulty > HYRULE.DIFFICULTY_SANDBOX)
	...
	if (this.Config.difficulty > HYRULE.DIFFICULTY_SANDBOX && (this.hasActiveBase() || !this.canBuildUnits))
...
HYRULE.HQ.prototype.Serialize = function()
{
	let properties = {
		"phasing": this.phasing,
...
HYRULE.HQ.prototype.Deserialize = function(gameState, data)
{
