HYRULE.DefenseManager = function(Config)
{
...
HYRULE.DefenseManager.prototype.update = function(gameState, events)
{
	...
	for (let enemy in this.attackingUnits)
	{
		...
		{
			if (this.attackingUnits[enemy][ally] < 10)
				continue;
...
HYRULE.DefenseManager.prototype.makeIntoArmy = function(gameState, entityID, type = "default")
{
	if (type == "default")
	{
	...
	this.armies.push(new HYRULE.DefenseArmy(gameState, [entityID], type));
};

HYRULE.DefenseManager.prototype.getArmy = function(partOfArmy)
{
	return this.armies.find(army => army.ID == partOfArmy);
};

HYRULE.DefenseManager.prototype.isDangerous = function(gameState, entity)
{
	if (!entity.position())
		return false;
...
HYRULE.DefenseManager.prototype.checkEnemyUnits = function(gameState)
{
...
HYRULE.DefenseManager.prototype.checkEnemyArmies = function(gameState)
{
...
HYRULE.DefenseManager.prototype.assignDefenders = function(gameState)
{
	if (!this.armies.length)
		return;
	...
	for (let army of this.armies)
	{
		...
		for (let entId of army.foeEntities)
		{
			...
			if (!ent || !ent.position())
				continue;
			armyAccess = HYRULE.getLandAccess(gameState, ent);
			break;
		}
		if (!armyAccess)
			API3.warn(" HYRULE error: attacking army " + army.ID + " without access");
		army.recalculatePosition(gameState);
	...
	gameState.getOwnUnits().forEach(function(ent) {
		...
		if (ent.getMetadata(PlayerID, "plan") !== undefined && ent.getMetadata(PlayerID, "plan") != -1)
		{
			...
			if (subrole &&
				(subrole === HYRULE.Worker.SUBROLE_COMPLETING || subrole === HYRULE.Worker.SUBROLE_WALKING || subrole === HYRULE.Worker.SUBROLE_ATTACKING))
				return;
		}
	...
	for (let ipass = 0; ipass < 2; ++ipass)
	{
		...
		for (let i = 0; i < potentialDefenders.length; ++i)
		{
			...
			let aMin;
			let distMin;
			let access = ipass == 0 ? HYRULE.getLandAccess(gameState, ent) : undefined;
			for (let a = 0; a < armiesNeeding.length; ++a)
			{
				...
				if (!armiesNeeding[a].army.foeEntities.some(eEnt => {
					...
					return ent.canAttackTarget(eEntID, HYRULE.allowCapture(gameState, ent, eEntID));
					}))
					continue;
			...
			if (backup < 50 && (aMin == undefined || distMin > 40000 &&
			...
			armiesNeeding[aMin].need -= HYRULE.getMaxStrength(ent, this.Config.debug, this.Config.DamageTypeImportance);
...
HYRULE.DefenseManager.prototype.abortArmy = function(gameState, army)
{
	army.clear(gameState);
	for (let i = 0; i < this.armies.length; ++i)
	{
 ...
 */
HYRULE.DefenseManager.prototype.checkEvents = function(gameState, events)
{
		...
		if (target.hasClass("Unit") && attacker.hasClass("Unit"))
		{
			...
			if (currentTarget)
			{
				...
				if (unitAIStateOrder === "REPAIR" && currentTarget.hasDefensiveFire())
					continue;
				if (unitAIStateOrder === "COMBAT" && !HYRULE.isSiegeUnit(currentTarget) &&
				    gameState.ai.HQ.capturableTargets.has(orderData[0].target))
				{
					...
					for (let entId of capturableTarget.ents)
					{
						if (allAttacked[entId])
							continue;
						let ent = gameState.getEntityById(entId);
						if (!ent || !ent.position() || !ent.canAttackTarget(attacker, HYRULE.allowCapture(gameState, ent, attacker)))
							continue;
					...
					if (minEnt)
					{
						...
						minEnt.attack(attacker.id(), HYRULE.allowCapture(gameState, minEnt, attacker));
					}
				}
			}
			let allowCapture = HYRULE.allowCapture(gameState, target, attacker);
...
HYRULE.DefenseManager.prototype.garrisonUnitsInside = function(gameState, target, data)
{
	...
	let access = HYRULE.getLandAccess(gameState, target);
	let garrisonManager = gameState.ai.HQ.garrisonManager;
	let garrisonArrowClasses = target.getGarrisonArrowClasses();
	const typeGarrison = data.type || HYRULE.GarrisonManager.TYPE_PROTECTION;
	...
	if (allowMelee === undefined)
	{
		...
		if (data.attacker)
			allowMelee = data.attacker.hasClass("Structure") ? data.attacker.attackRange("Ranged") : !HYRULE.isSiegeUnit(data.attacker);
		else
			allowMelee = true;
	}
	let units = gameState.getOwnUnits().filter(ent => {
		...
		if (!ent.hasClasses(garrisonArrowClasses))
			return false;
		if (typeGarrison !== HYRULE.GarrisonManager.TYPE_DECAY && !allowMelee && ent.attackTypes().indexOf("Melee") != -1)
			return false;
		if (ent.getMetadata(PlayerID, "transport") !== undefined)
			return false;
		...
		if (ent.getMetadata(PlayerID, "plan") !== undefined && ent.getMetadata(PlayerID, "plan") >= 0)
		{
			...
			if (typeGarrison !== HYRULE.GarrisonManager.TYPE_DECAY && subrole &&
				(subrole === HYRULE.Worker.SUBROLE_COMPLETING || subrole === HYRULE.Worker.SUBROLE_WALKING || subrole === HYRULE.Worker.SUBROLE_ATTACKING))
				return false;
		}
		if (HYRULE.getLandAccess(gameState, ent) != access)
			return false;
		return true;
...
HYRULE.DefenseManager.prototype.garrisonSiegeUnit = function(gameState, unit)
{
	let distmin = Math.min();
	let nearest;
	let unitAccess = HYRULE.getLandAccess(gameState, unit);
	let garrisonManager = gameState.ai.HQ.garrisonManager;
	for (let ent of gameState.getAllyStructures().values())
	{
		...
		if (ent.hitpoints() < ent.garrisonEjectHealth() * ent.maxHitpoints())
			continue;
		if (HYRULE.getLandAccess(gameState, ent) != unitAccess)
			continue;
	...
	if (nearest)
		garrisonManager.garrison(gameState, unit, nearest, HYRULE.GarrisonManager.TYPE_PROTECTION);
 ...
 */
HYRULE.DefenseManager.prototype.garrisonAttackedUnit = function(gameState, unit, emergency = false)
{
	let distmin = Math.min();
	let nearest;
	let unitAccess = HYRULE.getLandAccess(gameState, unit);
	...
	garrisonManager.garrison(gameState, unit, nearest, nearest.buffHeal() ? HYRULE.GarrisonManager.TYPE_PROTECTION : HYRULE.GarrisonManager.TYPE_EMERGENCY);
	return true;
};
 ...
 */
HYRULE.DefenseManager.prototype.GetCooperationLevel = function(ally)
{
	let cooperation = this.Config.personality.cooperative;
	if (this.attackedAllies[ally] && this.attackedAllies[ally] > 1)
		cooperation += 0.8 * (this.attackedAllies[ally] - 1);
	return cooperation;
};
 ...
 */
HYRULE.DefenseManager.prototype.switchToAttack = function(gameState, army)
{
	if (!army)
		return;
	for (let targetId of this.targetList)
	{
		...
		{
			let ent = gameState.getEntityById(entId);
			if (!ent || !ent.position() || HYRULE.getLandAccess(gameState, ent) != targetAccess)
				continue;
...
HYRULE.DefenseManager.prototype.Serialize = function()
{
	let properties = {
...
HYRULE.DefenseManager.prototype.Deserialize = function(gameState, data)
{
	...
	{
		let army = new HYRULE.DefenseArmy(gameState, []);
