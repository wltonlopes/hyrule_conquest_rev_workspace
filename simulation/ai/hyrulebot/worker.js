...
HYRULE.Worker.ROLE_ATTACK = "attack";
HYRULE.Worker.ROLE_TRADER = "trader";
HYRULE.Worker.ROLE_SWITCH_TO_TRADER = "switchToTrader";
HYRULE.Worker.ROLE_WORKER = "worker";
HYRULE.Worker.ROLE_CRITICAL_ENT_GUARD = "criticalEntGuard";
HYRULE.Worker.ROLE_CRITICAL_ENT_HEALER = "criticalEntHealer";

HYRULE.Worker.SUBROLE_DEFENDER = "defender";
HYRULE.Worker.SUBROLE_IDLE = "idle";
HYRULE.Worker.SUBROLE_BUILDER = "builder";
HYRULE.Worker.SUBROLE_COMPLETING = "completing";
HYRULE.Worker.SUBROLE_WALKING = "walking";
HYRULE.Worker.SUBROLE_ATTACKING = "attacking";
HYRULE.Worker.SUBROLE_GATHERER = "gatherer";
HYRULE.Worker.SUBROLE_HUNTER = "hunter";
HYRULE.Worker.SUBROLE_FISHER = "fisher";
HYRULE.Worker.SUBROLE_GARRISONING = "garrisoning";

HYRULE.Worker.prototype.update = function(gameState, ent)
{
	...
	if (ent.getMetadata(PlayerID, "transport") !== undefined)
	{
		...
		if (subrole === HYRULE.Worker.SUBROLE_BUILDER && ent.getMetadata(PlayerID, "target-foundation") !== undefined)
		{
			...
			if (!target && plan && plan.state === HYRULE.TransportPlan.BOARDING && ent.position())
				plan.removeUnit(gameState, ent);
		}
		// and gatherer if there are no more dropsite accessible in the base the ent is going to
		if (subrole === HYRULE.Worker.SUBROLE_GATHERER || subrole === HYRULE.Worker.SUBROLE_HUNTER)
		{
			let plan = gameState.ai.HQ.navalManager.getPlan(ent.getMetadata(PlayerID, "transport"));
			if (plan.state === HYRULE.TransportPlan.BOARDING && ent.position())
			{
				...
				{
					if (HYRULE.getLandAccess(gameState, structure) != plan.endIndex)
						continue;
					let resourceDropsiteTypes = HYRULE.getBuiltEntity(gameState, structure).resourceDropsiteTypes();
				...
				if (!hasDropsite)
				{
					...
					{
						if (!unit.position() || HYRULE.getLandAccess(gameState, unit) != plan.endIndex)
							continue;
	...
	this.entAccess = HYRULE.getLandAccess(gameState, ent);
	...
	if ((subrole === HYRULE.Worker.SUBROLE_HUNTER || subrole === HYRULE.Worker.SUBROLE_GATHERER) &&
		...
		if (unitAIState == "INDIVIDUAL.COMBAT.APPROACHING" && ent.unitAIOrderData().length)
		{
			...
			if (orderData && orderData.target)
			{
				...
				if (target && target.resourceSupplyType() && target.resourceSupplyType().generic == "food")
				{
					...
					{
						let distanceSquare = HYRULE.isFastMoving(ent) ? 90000 : 30000;
						...
						for (let dropsite of foodDropsites.values())
						{
							...
							if (owner != PlayerID && (!dropsite.isSharedDropsite() || !gameState.isPlayerMutualAlly(owner)))
								continue;
							if (targetAccess != HYRULE.getLandAccess(gameState, dropsite))
								continue;
	...
	{
		if (subrole === HYRULE.Worker.SUBROLE_FISHER)
			...
			!ent.getMetadata(PlayerID, "PartOfArmy"))
		{
			...
			{
				let target = gameState.getEntityById(orderData.target);
				if (target && (!target.position() || HYRULE.getLandAccess(gameState, target) != this.entAccess))
				{
					if (this.retryWorking(gameState, subrole))
						return;
					ent.stopMoving();
			...
			!ent.getMetadata(PlayerID, "PartOfArmy"))
		{
			...
			if (orderData && orderData.target && orderData.attackType && orderData.attackType == "Capture")
			{
				...
				if (target && target.owner() > 0 && !gameState.isPlayerAlly(target.owner()))
					ent.attack(orderData.target, HYRULE.allowCapture(gameState, ent, target));
			}
		}
		return;
	}
	...
	if (subrole === HYRULE.Worker.SUBROLE_GATHERER)
	{
		if (ent.isIdle())
		{
			...
			else if (!HYRULE.returnResources(gameState, ent))     // try to deposit resources
			{
		...
		else if (unitAIStateOrder == "GATHER")
		{
			...
			if (unitAIState == "INDIVIDUAL.GATHER.RETURNINGRESOURCE.APPROACHING")
			{
				if (gameState.ai.playedTurn % 10 == 0)
				{
					...
					if (dropsite && dropsite.position() && this.entAccess != HYRULE.getLandAccess(gameState, dropsite))
						HYRULE.returnResources(gameState, this.ent);
				}
	...
	else if (subrole === HYRULE.Worker.SUBROLE_BUILDER)
	{
		...
		{
			let goalAccess = HYRULE.getLandAccess(gameState, target);
			let queued = HYRULE.returnResources(gameState, ent);
	...
	else if (subrole === HYRULE.Worker.SUBROLE_HUNTER)
	{
		...
		else	// Perform some sanity checks
		{
			if (unitAIStateOrder == "GATHER")
			{
				...
				else if (unitAIState == "INDIVIDUAL.GATHER.RETURNINGRESOURCE.APPROACHING")
				{
					...
					let dropsite = gameState.getEntityById(ent.unitAIOrderData()[0].target);
					if (dropsite && dropsite.position() && this.entAccess != HYRULE.getLandAccess(gameState, dropsite))
						HYRULE.returnResources(gameState, ent);
				}
			}
		}
	}
	else if (subrole === HYRULE.Worker.SUBROLE_FISHER)
	{
		if (ent.isIdle())
			this.startFishing(gameState);
...
HYRULE.Worker.prototype.retryWorking = function(gameState, subrole)
{
	switch (subrole)
	{
	case HYRULE.Worker.SUBROLE_GATHERER:
		return this.startGathering(gameState);
	case HYRULE.Worker.SUBROLE_HUNTER:
		return this.startHunting(gameState);
	case HYRULE.Worker.SUBROLE_FISHER:
		return this.startFishing(gameState);
	case HYRULE.Worker.SUBROLE_BUILDER:
...
HYRULE.Worker.prototype.startBuilding = function(gameState)
{
	let target = gameState.getEntityById(this.ent.getMetadata(PlayerID, "target-foundation"));
	if (!target || target.foundationProgress() === undefined && target.needsRepair() == false)
		return false;
	if (HYRULE.getLandAccess(gameState, target) != this.entAccess)
		return false;
	this.ent.repair(target, target.hasClass("House"));  // autocontinue=true for houses
	return true;
};

HYRULE.Worker.prototype.startGathering = function(gameState)
{
	// First look for possible treasure if any
	if (HYRULE.gatherTreasure(gameState, this.ent))
		return true;
	...
	const findSupply = function(worker, supplies) {
		...
		for (let i = 0; i < supplies.length; ++i)
		{
			...
			if (HYRULE.IsSupplyFull(gameState, supplies[i].ent))
				continue;
	...
	let shouldBuild = this.ent.isBuilder() && foundations.some(function(foundation) {
		if (!foundation || HYRULE.getLandAccess(gameState, foundation) != this.entAccess)
			return false;
		...
		if (structure.resourceDropsiteTypes() && structure.resourceDropsiteTypes().indexOf(resource) != -1)
		{
			...
			this.ent.setMetadata(PlayerID, "subrole", HYRULE.Worker.SUBROLE_BUILDER);
	...
	shouldBuild = this.ent.isBuilder() && foundations.some(function(foundation) {
		...
		if (structure.resourceDropsiteTypes() && structure.resourceDropsiteTypes().indexOf(resource) != -1)
		{
			let foundationAccess = HYRULE.getLandAccess(gameState, foundation);
	...
	this.ent.setMetadata(PlayerID, "subrole", HYRULE.Worker.SUBROLE_IDLE);
	return false;
};
 ...
 */
HYRULE.Worker.prototype.startHunting = function(gameState, position)
{
	// First look for possible treasure if any
	if (!position && HYRULE.gatherTreasure(gameState, this.ent))
		return true;
	...
	let isFastMoving = HYRULE.isFastMoving(this.ent);
	...
	let hasFoodDropsiteWithinDistance = function(supplyPosition, supplyAccess, distSquare)
	{
		for (let dropsite of foodDropsites.values())
		{
			...
			if (owner != PlayerID && (!dropsite.isSharedDropsite() || !gameState.isPlayerMutualAlly(owner)))
				continue;
			if (supplyAccess != HYRULE.getLandAccess(gameState, dropsite))
				continue;
	...
	for (let supply of resources.values())
	{
		...
		if (HYRULE.IsSupplyFull(gameState, supply))
			continue;
		// Check if available resource is worth one additionnal gatherer (except for farms).
		...
		let supplyAccess = HYRULE.getLandAccess(gameState, supply);
...
HYRULE.Worker.prototype.startFishing = function(gameState)
{
	...
	let fisherSea = HYRULE.getSeaAccess(gameState, this.ent);
	...
	let nearestDropsiteDist = function(supply) {
		...
		for (let dropsite of fishDropsites)
		{
			...
			if (owner != PlayerID && (!dropsite.isSharedDropsite() || !gameState.isPlayerMutualAlly(owner)))
				continue;
			if (fisherSea != HYRULE.getSeaAccess(gameState, dropsite))
				continue;
	...
	if (this.ent.getMetadata(PlayerID, "subrole") === HYRULE.Worker.SUBROLE_FISHER)
		this.ent.setMetadata(PlayerID, "subrole", HYRULE.Worker.SUBROLE_IDLE);
	return false;
};

HYRULE.Worker.prototype.gatherNearestField = function(gameState, baseID)
{
	...
	{
		if (HYRULE.IsSupplyFull(gameState, field))
			continue;
 ...
 */
HYRULE.Worker.prototype.buildAnyField = function(gameState, baseID)
{
 ...
 */
HYRULE.Worker.prototype.moveToGatherer = function(gameState, ent, forced)
{
	...
	if (!forced && gameState.ai.elapsedTime < (ent.getMetadata(PlayerID, "nextMoveToGatherer") || 5))
		return;
	const gatherers = this.base.workersBySubrole(gameState, HYRULE.Worker.SUBROLE_GATHERER);
	let dist = Math.min();
	let destination;
	...
	for (let gatherer of gatherers.values())
	{
		...
		if (!gatherer.position() || gatherer.getMetadata(PlayerID, "transport") !== undefined ||
		    HYRULE.getLandAccess(gameState, gatherer) != access || gatherer.isIdle())
			continue;
 ...
 */
HYRULE.Worker.prototype.isInaccessibleSupply = function(gameState)
{
