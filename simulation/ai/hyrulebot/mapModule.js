...
HYRULE.TERRITORY_PLAYER_MASK = 0x1F;
HYRULE.TERRITORY_BLINKING_MASK = 0x40;

HYRULE.createObstructionMap = function(gameState, accessIndex, template)
{
	...
	{
		let tilePlayer = territoryMap.data[k] & HYRULE.TERRITORY_PLAYER_MASK;
		let isConnected = (territoryMap.data[k] & HYRULE.TERRITORY_BLINKING_MASK) == 0;
...
HYRULE.createTerritoryMap = function(gameState)
{
	let map = gameState.ai.territoryMap;

	let ret = new API3.Map(gameState.sharedScript, "territory", map.data);
	ret.getOwner = function(p) { return this.point(p) & HYRULE.TERRITORY_PLAYER_MASK; };
	ret.getOwnerIndex = function(p) { return this.map[p] & HYRULE.TERRITORY_PLAYER_MASK; };
	ret.isBlinking = function(p) { return (this.point(p) & HYRULE.TERRITORY_BLINKING_MASK) != 0; };
	return ret;
};
...
HYRULE.outside_Mask = 1;
HYRULE.border_Mask = 2;
HYRULE.fullBorder_Mask = HYRULE.outside_Mask | HYRULE.border_Mask;
HYRULE.narrowFrontier_Mask = 4;
HYRULE.largeFrontier_Mask = 8;
HYRULE.fullFrontier_Mask = HYRULE.narrowFrontier_Mask | HYRULE.largeFrontier_Mask;

HYRULE.createBorderMap = function(gameState)
{
...
HYRULE.debugMap = function(gameState, map)
{
	let width = map.width;
