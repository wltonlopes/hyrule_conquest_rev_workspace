 ...
 */
HYRULE.DiplomacyManager = function(Config)
{
	this.Config = Config;
	this.nextTributeUpdate = 100;
	this.nextTributeRequest = new Map();
	this.nextTributeRequest.set("all", 250);
	...
	this.sentDiplomacyRequestLapseTime = 150 + randFloat(50, 150);
};
 ...
 */
HYRULE.DiplomacyManager.prototype.init = function(gameState)
{
 ...
 */
HYRULE.DiplomacyManager.prototype.tributes = function(gameState)
{
	this.nextTributeUpdate = gameState.ai.elapsedTime + 50;
	...
	for (let i = 1; i < gameState.sharedScript.playersData.length; ++i)
	{
		...
		for (let res of resTribCodes)
		{
			...
			else if (this.Config.chat && availableResources[res] === 0 && allyResources[res] > totalResources[res] + 600)
			{
				...
				for (let k = 0; k < mostNeeded.length; ++k)
				{
					...
					{
						this.nextTributeRequest.set("all", gameState.ai.elapsedTime + 100);
						this.nextTributeRequest.set(res, gameState.ai.elapsedTime + 250);
						HYRULE.chatRequestTribute(gameState, res);
		...
		if (this.Config.chat)
			HYRULE.chatSentTribute(gameState, i);
		Engine.PostCommand(PlayerID, { "type": "tribute", "player": i, "amounts": tribute });
	}
};

HYRULE.DiplomacyManager.prototype.checkEvents = function(gameState, events)
{
	...
	for (let evt of events.DiplomacyChanged)
	{
		...
		else if (gameState.sharedScript.playersData[evt.player].isAlly[PlayerID] && gameState.isPlayerEnemy(evt.player))
		{
			...
			HYRULE.chatAnswerRequestDiplomacy(gameState, evt.player, "ally", response);
		}
 ...
 */
HYRULE.DiplomacyManager.prototype.lastManStandingCheck = function(gameState)
{
	...
	{
		this.betrayLapseTime = gameState.ai.elapsedTime + randFloat(50, 150);
	...
	if (playerToTurnAgainst)
	{
		...
		{
			if (request.status === "waitingForTribute")
				HYRULE.chatAnswerRequestDiplomacy(gameState, player, request.requestType, "decline");
 ...
 */
HYRULE.DiplomacyManager.prototype.handleDiplomacyRequest = function(gameState, player, requestType)
{
	...
	HYRULE.chatAnswerRequestDiplomacy(gameState, player, requestType, response, requiredTribute);
};

HYRULE.DiplomacyManager.prototype.changePlayerDiplomacy = function(gameState, player, newDiplomaticStance)
{
	...
	if (this.Config.chat)
		HYRULE.chatNewDiplomacy(gameState, player, newDiplomaticStance);
};

HYRULE.DiplomacyManager.prototype.checkRequestedTributes = function(gameState)
{
	for (let [player, data] of this.receivedDiplomacyRequests)
		if (data.status === "waitingForTribute" && gameState.ai.elapsedTime > data.warnTime)
		{
			if (data.sentWarning)
			{
				...
				HYRULE.chatAnswerRequestDiplomacy(gameState, player, data.requestType, "decline");
			}
			else
			{
				data.sentWarning = true;
				data.warnTime = gameState.ai.elapsedTime + 60;
				HYRULE.chatAnswerRequestDiplomacy(gameState, player, data.requestType, "waitingForTribute", {
					"wanted": data.wanted,
					"type": data.type
				});
			}
 ...
 */
HYRULE.DiplomacyManager.prototype.sendDiplomacyRequest = function(gameState)
{
	let player;
	let max = 0;
	for (let i = 1; i < gameState.sharedScript.playersData.length; ++i)
	...
	HYRULE.chatNewRequestDiplomacy(gameState, player, requestType, "sendRequest");
};

HYRULE.DiplomacyManager.prototype.checkSentDiplomacyRequests = function(gameState)
{
	for (let [player, data] of this.sentDiplomacyRequests)
		if (gameState.ai.elapsedTime > data.timeSent + 60 && !gameState.ai.HQ.saveResources &&
		    gameState.getPopulation() > 80)
		{
			HYRULE.chatNewRequestDiplomacy(gameState, player, data.requestType, "requestExpired");
			this.sentDiplomacyRequests.delete(player);
		}
};

HYRULE.DiplomacyManager.prototype.update = function(gameState, events)
{
...
HYRULE.DiplomacyManager.prototype.Serialize = function()
{
	return {
...
HYRULE.DiplomacyManager.prototype.Deserialize = function(data)
{
	for (let key in data)
		this[key] = data[key];
};
