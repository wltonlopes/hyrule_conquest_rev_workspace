...
HYRULE.BasesManager = function(Config)
{
	this.Config = Config;
...
HYRULE.BasesManager.prototype.init = function(gameState)
{
	...
	this.noBase = new HYRULE.BaseManager(gameState, this);
	this.noBase.init(gameState, HYRULE.BaseManager.STATE_WITH_ANCHOR);
	...
	for (const cc of gameState.getOwnStructures().filter(API3.Filters.byClass("CivCentre")).values())
		if (cc.foundationProgress() === undefined)
			this.createBase(gameState, cc, HYRULE.BaseManager.STATE_WITH_ANCHOR);
		else
			this.createBase(gameState, cc, HYRULE.BaseManager.STATE_UNCONSTRUCTED);
};

/**
 * Initialization needed after deserialization (only called when deserialising).
 */
HYRULE.BasesManager.prototype.postinit = function(gameState)
{
 ...
 */
HYRULE.BasesManager.prototype.createBase = function(gameState, ent, type = HYRULE.BaseManager.STATE_WITH_ANCHOR)
{
	const access = HYRULE.getLandAccess(gameState, ent);
	let newbase;
	for (const base of this.baseManagers)
	{
		if (base.accessIndex != access)
			continue;
		if (type !== HYRULE.BaseManager.STATE_ANCHORLESS && base.anchor)
			continue;
		if (type !== HYRULE.BaseManager.STATE_ANCHORLESS)
		{
	...
	if (!newbase)
	{
		newbase = new HYRULE.BaseManager(gameState, this);
...
HYRULE.BasesManager.prototype.checkEvents = function(gameState, events)
{
	let addBase = false;
	...
	for (const evt of events.EntityRenamed)
	{
		...
		if (ent.getMetadata(PlayerID, "role") === HYRULE.Worker.ROLE_WORKER)
		{
	...
	for (const evt of events.Create)
	{
		...
		if (ent.getMetadata(PlayerID, "base") == -1)	// Standard base around a cc
		{
			...
			const newbase = this.createBase(gameState, ent, HYRULE.BaseManager.STATE_UNCONSTRUCTED);
		...
		{
			const newbase = this.createBase(gameState, ent, HYRULE.BaseManager.STATE_ANCHORLESS);
			// Let's get a few units from other bases there to build this.
	...
	for (const evt of events.OwnershipChanged)
	{
		...
		{
			HYRULE.getBestBase(gameState, ent).assignEntity(gameState, ent);
			continue;
		}
		if (ent.hasClass("CivCentre"))   // build a new base around it
		{
			...
			if (ent.foundationProgress() !== undefined)
				newbase = this.createBase(gameState, ent, HYRULE.BaseManager.STATE_UNCONSTRUCTED);
			else
			{
				newbase = this.createBase(gameState, ent, HYRULE.BaseManager.STATE_CAPTURED);
				addBase = true;
			}
		...
		else
		{
			...
			if (!ent.decaying() && ent.resourceDropsiteTypes())
				base = this.createBase(gameState, ent, HYRULE.BaseManager.STATE_ANCHORLESS);
			else
				base = HYRULE.getBestBase(gameState, ent) || this.noBase;
	...
	for (const evt of events.TrainingFinished)
	{
		for (const entId of evt.entities)
		{
			...
			if (ent.getMetadata(PlayerID, "role") === HYRULE.Worker.ROLE_WORKER)
			{
				...
				if (ent.getMetadata(PlayerID, "base") === undefined)
				{
					base = HYRULE.getBestBase(gameState, ent);
					base.assignEntity(gameState, ent);
				}
				else
			...
			else if (ent.resourceSupplyType() && ent.position())
			{
				...
				{
					if (!dropsite.position() || HYRULE.getLandAccess(gameState, dropsite) != access)
						continue;
 ...
 */
HYRULE.BasesManager.prototype.bulkPickWorkers = function(gameState, baseRef, number)
{
 ...
 */
HYRULE.BasesManager.prototype.getTotalResourceLevel = function(gameState, resources = Resources.GetCodes(), proximity = ["nearby", "medium"])
{
	const total = {};
 ...
 */
HYRULE.BasesManager.prototype.GetCurrentGatherRates = function(gameState)
{
...
HYRULE.BasesManager.prototype.AddTCGatherer = function(supplyID)
{
...
HYRULE.BasesManager.prototype.RemoveTCGatherer = function(supplyID)
{
...
HYRULE.BasesManager.prototype.GetTCGatherer = function(supplyID)
{
	if (this.turnCache.resourceGatherer && this.turnCache.resourceGatherer[supplyID])
...
HYRULE.BasesManager.prototype.AddTCResGatherer = function(resource)
{
...
HYRULE.BasesManager.prototype.GetTCResGatherer = function(resource)
{
	const check = "resourceGatherer-" + resource;
	if (this.turnCache[check])
 ...
 */
HYRULE.BasesManager.prototype.isResourceExhausted = function(resource)
{
 ...
 */
HYRULE.BasesManager.prototype.numActiveBases = function()
{
...
HYRULE.BasesManager.prototype.hasActiveBase = function()
{
	return !!this.numActiveBases();
};

HYRULE.BasesManager.prototype.numPotentialBases = function()
{
	if (!this.turnCache.base)
		this.updateBaseCache();
...
HYRULE.BasesManager.prototype.hasPotentialBase = function()
{
 ...
 */
HYRULE.BasesManager.prototype.updateBaseCache = function()
{
...
HYRULE.BasesManager.prototype.resetBaseCache = function()
{
	this.turnCache.base = undefined;
};

HYRULE.BasesManager.prototype.baselessBase = function()
{
	return this.noBase;
};
 ...
 */
HYRULE.BasesManager.prototype.getBaseByID = function(baseID)
{
 ...
 */
HYRULE.BasesManager.prototype.assignGatherers = function()
{
	for (const base of this.baseManagers)
 ...
 */
HYRULE.BasesManager.prototype.assignEntity = function(gameState, ent, territoryIndex)
{
	let bestbase;
	for (const base of this.baseManagers)
	{
	...
	if (!bestbase)	// entity outside our territory
	{
		if (ent.hasClass("Structure") && !ent.decaying() && ent.resourceDropsiteTypes())
			bestbase = this.createBase(gameState, ent, HYRULE.BaseManager.STATE_ANCHORLESS);
		else
			bestbase = HYRULE.getBestBase(gameState, ent) || this.noBase;
 ...
 */
HYRULE.BasesManager.prototype.addGatherRates = function(gameState, rates)
{
	for (const base of this.baseManagers)
 ...
 */
HYRULE.BasesManager.prototype.baseAtIndex = function(territoryIndex)
{
 ...
 * @param {number} territoryIndex
 */
HYRULE.BasesManager.prototype.removeBaseFromTerritoryIndex = function(territoryIndex)
{
 ...
 */
HYRULE.BasesManager.prototype.addTerritoryIndexToBase = function(gameState, territoryIndex, passabilityMap)
{
...
HYRULE.BasesManager.prototype.reassignTerritories = function(deletedBase, territoryMap)
{
	...
	for (let j = 0; j < territoryMap.length; ++j)
	{
		...
		{
			API3.warn("HYRULE reassignTerritories: should never happen");
 ...
 */
HYRULE.BasesManager.prototype.update = function(gameState, queues, events)
{
...
HYRULE.BasesManager.prototype.Serialize = function()
{
	const properties = {
...
HYRULE.BasesManager.prototype.Deserialize = function(gameState, data)
{
	...
	for (const basedata of data.baseManagers)
	{
		...
		const newbase = new HYRULE.BaseManager(gameState, this);
		newbase.Deserialize(gameState, basedata);
		newbase.init(gameState, HYRULE.BaseManager.STATE_WITH_ANCHOR);
		newbase.Deserialize(gameState, basedata);
		this.baseManagers.push(newbase);
	}
};
