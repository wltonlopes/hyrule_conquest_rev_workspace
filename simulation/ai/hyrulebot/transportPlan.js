...
HYRULE.TransportPlan = function(gameState, units, startIndex, endIndex, endPos, ship)
{
	...
	this.state = HYRULE.TransportPlan.BOARDING;
	this.boardingPos = {};
 ...
 */
HYRULE.TransportPlan.BOARDING = "boarding";
/**
 * We're moving ships and eventually unload units.
 */
HYRULE.TransportPlan.SAILING = "sailing";

HYRULE.TransportPlan.prototype.init = function(gameState)
{
...
HYRULE.TransportPlan.prototype.countFreeSlots = function()
{
	let slots = 0;
...
HYRULE.TransportPlan.prototype.countFreeSlotsOnShip = function(ship)
{
...
HYRULE.TransportPlan.prototype.assignUnitToShip = function(gameState, ent)
{
...
HYRULE.TransportPlan.prototype.assignShip = function(gameState)
{
	let pos;
...
HYRULE.TransportPlan.prototype.addUnit = function(unit, endPos)
{
...
HYRULE.TransportPlan.prototype.removeUnit = function(gameState, unit)
{
...
HYRULE.TransportPlan.prototype.releaseShip = function(ship)
{
	if (ship.getMetadata(PlayerID, "transporter") != this.ID)
	{
		API3.warn(" HYRULE: try removing a transporter ship with " + ship.getMetadata(PlayerID, "transporter") +
	...
	if (ship.getMetadata(PlayerID, "role") === HYRULE.Worker.ROLE_SWITCH_TO_TRADER)
		ship.setMetadata(PlayerID, "role", HYRULE.Worker.ROLE_TRADER);
};

HYRULE.TransportPlan.prototype.releaseAll = function()
{
...
HYRULE.TransportPlan.prototype.cancelTransport = function(gameState)
{
 ...
 */
HYRULE.TransportPlan.prototype.update = function(gameState)
{
	if (this.state === HYRULE.TransportPlan.BOARDING)
		this.onBoarding(gameState);
	else if (this.state === HYRULE.TransportPlan.SAILING)
		this.onSailing(gameState);

	return this.units.length;
};

HYRULE.TransportPlan.prototype.onBoarding = function(gameState)
{
	let ready = true;
	...
	this.state = HYRULE.TransportPlan.SAILING;
	this.nTry = {};
	this.unloaded = [];
	this.recovered = [];
};
...
HYRULE.TransportPlan.prototype.isOnBoard = function(ent)
{
	for (let ship of this.transportShips.values())
	{
...
HYRULE.TransportPlan.prototype.getBoardingPos = function(gameState, ship, landIndex, seaIndex, destination, avoidEnnemy)
{
...
HYRULE.TransportPlan.prototype.onSailing = function(gameState)
{
	...
	for (let entId of this.unloaded)
	{
		...
		else if (!ent.position())  // unloading failed
		{
			...
			if (ship)
			{
				...
				{
					API3.warn("HYRULE transportPlan problem: unit not on ship without position ???");
					this.resetUnit(gameState, ent);
					ent.destroy();
				}
			}
			...
			{
				API3.warn("HYRULE transportPlan problem: unit on ship, but no ship ???");
				this.resetUnit(gameState, ent);
				ent.destroy();
			}
		}
		else if (HYRULE.getLandAccess(gameState, ent) != this.endIndex)
		{
...
HYRULE.TransportPlan.prototype.resetUnit = function(gameState, ent)
{
	ent.setMetadata(PlayerID, "transport", undefined);
	ent.setMetadata(PlayerID, "onBoard", undefined);
	ent.setMetadata(PlayerID, "endPos", undefined);
...
HYRULE.TransportPlan.prototype.Serialize = function()
{
	return {
		"ID": this.ID,
...
HYRULE.TransportPlan.prototype.Deserialize = function(data)
{
	for (let key in data)
		this[key] = data[key];
