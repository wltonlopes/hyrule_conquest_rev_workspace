 ...
 * Manage the trade
 */
HYRULE.TradeManager = function(Config)
{
	this.Config = Config;
...
HYRULE.TradeManager.prototype.init = function(gameState)
{
	this.traders = gameState.getOwnUnits().filter(API3.Filters.byMetadata(PlayerID, "role", HYRULE.Worker.ROLE_TRADER));
	this.traders.registerUpdates();
	this.minimalGain = gameState.ai.HQ.navalMap ? 3 : 5;
};

HYRULE.TradeManager.prototype.hasTradeRoute = function()
{
	return this.tradeRoute !== undefined;
};

HYRULE.TradeManager.prototype.assignTrader = function(ent)
{
	ent.setMetadata(PlayerID, "role", HYRULE.Worker.ROLE_TRADER);
	this.traders.updateEnt(ent);
};

HYRULE.TradeManager.prototype.trainMoreTraders = function(gameState, queues)
{
	...
	gameState.getOwnTrainingFacilities().forEach(function(ent) {
		...
		{
			if (!item.metadata || !item.metadata.role || item.metadata.role !== HYRULE.Worker.ROLE_TRADER)
				continue;
	...
	let template;
	const metadata = { "role": HYRULE.Worker.ROLE_TRADER };
	if (this.tradeRoute.sea)
	{
		...
		gameState.ai.HQ.navalManager.seaTransportShips[this.tradeRoute.sea].forEach(function(ship) {
			if (already || !ship.hasClass("Trader"))
				return;
			if (ship.getMetadata(PlayerID, "role") === HYRULE.Worker.ROLE_SWITCH_TO_TRADER)
			{
				already = true;
				return;
			}
		...
		if (shipToSwitch)
		{
			if (shipToSwitch.getMetadata(PlayerID, "transporter") === undefined)
				shipToSwitch.setMetadata(PlayerID, "role", HYRULE.Worker.ROLE_TRADER);
			else
				shipToSwitch.setMetadata(PlayerID, "role", HYRULE.Worker.ROLE_SWITCH_TO_TRADER);
			return;
		}
	...
	if (!gameState.getTemplate(template))
	{
		if (this.Config.debug > 0)
			API3.warn("HYRULE error: trying to train " + template + " for civ " +
			          gameState.getPlayerCiv() + " but no template found.");
		return;
	}
	queues.trader.addPlan(new HYRULE.TrainingPlan(gameState, template, metadata, 1, 1));
};

HYRULE.TradeManager.prototype.updateTrader = function(gameState, ent)
{
	if (ent.hasClass("Ship") && gameState.ai.playedTurn % 5 == 0 &&
	    !ent.unitAIState().startsWith("INDIVIDUAL.COLLECTTREASURE") &&
	    HYRULE.gatherTreasure(gameState, ent, true))
		return;
	...
	let access = ent.hasClass("Ship") ? HYRULE.getSeaAccess(gameState, ent) : HYRULE.getLandAccess(gameState, ent);
...
HYRULE.TradeManager.prototype.setTradingGoods = function(gameState)
{
 ...
 */
HYRULE.TradeManager.prototype.performBarter = function(gameState)
{
...
HYRULE.TradeManager.prototype.checkEvents = function(gameState, events)
{
...
HYRULE.TradeManager.prototype.activateProspection = function(gameState)
{
 ...
 */
HYRULE.TradeManager.prototype.checkRoutes = function(gameState, accessIndex)
{
	...
	for (let m1 of market1.values())
	{
		if (!m1.position())
			continue;
		let access1 = HYRULE.getLandAccess(gameState, m1);
		let sea1 = m1.hasClass("Naval") ? HYRULE.getSeaAccess(gameState, m1) : undefined;
		for (let m2 of market2.values())
		{
			...
			if (!m2.position())
				continue;
			let access2 = HYRULE.getLandAccess(gameState, m2);
			let sea2 = m2.hasClass("Naval") ? HYRULE.getSeaAccess(gameState, m2) : undefined;
			let land = access1 == access2 ? access1 : undefined;
			let sea = sea1 && sea1 == sea2 ? sea1 : undefined;
			if (!land && !sea)
				continue;
			if (land && HYRULE.isLineInsideEnemyTerritory(gameState, m1.position(), m2.position()))
				continue;
			let gainMultiplier;
	...
	if (this.Config.chat)
	{
		...
		{	// Warn an ally that we have a trade route with him
			HYRULE.chatNewTradeRoute(gameState, owner);
...
HYRULE.TradeManager.prototype.checkTrader = function(gameState, ent)
{
	    ...
	    possibleRoute.target.id() != presentRoute.source && possibleRoute.target.id() != presentRoute.target)
	{
		...
		{
			let closestBase = HYRULE.getBestBase(gameState, ent, true);
...
HYRULE.TradeManager.prototype.prospectForNewMarket = function(gameState, queues)
{
...
HYRULE.TradeManager.prototype.isNewMarketWorth = function(expectedGain)
{
	if (!Resources.GetTradableCodes().length)
		return false;
...
HYRULE.TradeManager.prototype.update = function(gameState, events, queues)
{
	if (gameState.ai.HQ.canBarter && Resources.GetBarterableCodes().length)
		this.performBarter(gameState);

	if (this.Config.difficulty <= HYRULE.DIFFICULTY_VERY_EASY)
		return;
...
HYRULE.TradeManager.prototype.routeEntToId = function(route)
{
	if (!route)
		return undefined;
...
HYRULE.TradeManager.prototype.routeIdToEnt = function(gameState, route)
{
	if (!route)
		return undefined;
...
HYRULE.TradeManager.prototype.Serialize = function()
{
	return {
...
HYRULE.TradeManager.prototype.Deserialize = function(gameState, data)
{
	for (let key in data)
	{
