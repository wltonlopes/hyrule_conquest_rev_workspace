...
HYRULE.AttackManager = function(Config)
{
	...
	this.upcomingAttacks = {
		[HYRULE.AttackPlan.TYPE_RUSH]: [],
		[HYRULE.AttackPlan.TYPE_RAID]: [],
		[HYRULE.AttackPlan.TYPE_DEFAULT]: [],
		[HYRULE.AttackPlan.TYPE_HUGE_ATTACK]: []
	};
	this.startedAttacks = {
		[HYRULE.AttackPlan.TYPE_RUSH]: [],
		[HYRULE.AttackPlan.TYPE_RAID]: [],
...
HYRULE.AttackManager.prototype.checkEvents = function(gameState, events)
{
	...
	for (let evt of events.AttackRequest)
	{
		...
		for (let attackType in this.upcomingAttacks)
		{
			...
			{
				if (attack.state === HYRULE.AttackPlan.STATE_COMPLETING)
				{
	...
	if (targetPlayer !== undefined)
		HYRULE.chatAnswerRequestAttack(gameState, targetPlayer, answer, other);
 ...
 */
HYRULE.AttackManager.prototype.assignBombers = function(gameState)
{
	...
	for (let ent of bombers.values())
	{
		...
		if (ent.getMetadata(PlayerID, "plan") !== undefined && ent.getMetadata(PlayerID, "plan") != -1)
		{
			...
			if (subrole && (subrole === HYRULE.Worker.SUBROLE_COMPLETING || subrole === HYRULE.Worker.SUBROLE_WALKING || subrole === HYRULE.Worker.SUBROLE_ATTACKING))
				continue;
		}
		...
		{
			if (!ent.canAttackTarget(struct, HYRULE.allowCapture(gameState, ent, struct)))
				continue;
 ...
 */
HYRULE.AttackManager.prototype.update = function(gameState, queues, events)
{
	...
	const unexecutedAttacks = {
		[HYRULE.AttackPlan.TYPE_RUSH]: 0,
		[HYRULE.AttackPlan.TYPE_RAID]: 0,
		[HYRULE.AttackPlan.TYPE_DEFAULT]: 0,
		[HYRULE.AttackPlan.TYPE_HUGE_ATTACK]: 0
	};
	for (let attackType in this.upcomingAttacks)
	{
		for (let i = 0; i < this.upcomingAttacks[attackType].length; ++i)
		{
			...
			if (attack.isStarted())
				API3.warn("HYRULE problem in attackManager: attack in preparation has already started ???");

			let updateStep = attack.updatePreparation(gameState);
			// now we're gonna check if the preparation time is over
			if (updateStep === HYRULE.AttackPlan.PREPARATION_KEEP_GOING || attack.isPaused())
			{
				// just chillin'
				if (attack.state === HYRULE.AttackPlan.STATE_UNEXECUTED)
					++unexecutedAttacks[attackType];
			}
			else if (updateStep === HYRULE.AttackPlan.PREPARATION_FAILED)
			{
			...
			else if (updateStep === HYRULE.AttackPlan.PREPARATION_START)
			{
				if (attack.StartAttack(gameState))
				{
					...
					if (this.Config.chat)
						HYRULE.chatLaunchAttack(gameState, attack.targetPlayer, attack.getType());
	...
	{
		if (unexecutedAttacks[HYRULE.AttackPlan.TYPE_RUSH] === 0)
		{
			// we have a barracks and we want to rush, rush.
			let data = { "targetSize": this.rushSize[this.rushNumber] };
			const attackPlan = new HYRULE.AttackPlan(gameState, this.Config, this.totalNumber, HYRULE.AttackPlan.TYPE_RUSH, data);
			if (!attackPlan.failed)
			{
				...
				this.upcomingAttacks[HYRULE.AttackPlan.TYPE_RUSH].push(attackPlan);
			}
			this.rushNumber++;
		}
	}
	else if (unexecutedAttacks[HYRULE.AttackPlan.TYPE_DEFAULT] == 0 && unexecutedAttacks[HYRULE.AttackPlan.TYPE_HUGE_ATTACK] == 0 &&
		this.startedAttacks[HYRULE.AttackPlan.TYPE_DEFAULT].length + this.startedAttacks[HYRULE.AttackPlan.TYPE_HUGE_ATTACK].length <
			Math.min(2, 1 + Math.round(gameState.getPopulationMax() / 150)) &&
		(this.startedAttacks[HYRULE.AttackPlan.TYPE_DEFAULT].length + this.startedAttacks[HYRULE.AttackPlan.TYPE_HUGE_ATTACK].length == 0 ||
		gameState.getPopulationMax() - gameState.getPopulation() > 15))
	{
		...
		{
			const type = this.attackNumber < 2 || this.startedAttacks[HYRULE.AttackPlan.TYPE_HUGE_ATTACK].length > 0 ? HYRULE.AttackPlan.TYPE_DEFAULT : HYRULE.AttackPlan.TYPE_HUGE_ATTACK;
			let attackPlan = new HYRULE.AttackPlan(gameState, this.Config, this.totalNumber, type);
			if (attackPlan.failed)
	...
	if (unexecutedAttacks[HYRULE.AttackPlan.TYPE_RAID] === 0 && gameState.ai.HQ.defenseManager.targetList.length)
	{
		let target;
	...
	if (this.Config.difficulty > HYRULE.DIFFICULTY_VERY_EASY && gameState.ai.playedTurn % 5 == 0)
		this.assignBombers(gameState);
};

HYRULE.AttackManager.prototype.getPlan = function(planName)
{
	for (let attackType in this.upcomingAttacks)
	{
		for (let attack of this.upcomingAttacks[attackType])
...
HYRULE.AttackManager.prototype.pausePlan = function(planName)
{
...
HYRULE.AttackManager.prototype.unpausePlan = function(planName)
{
	let attack = this.getPlan(planName);
	if (attack)
...
HYRULE.AttackManager.prototype.pauseAllPlans = function()
{
	for (let attackType in this.upcomingAttacks)
		for (let attack of this.upcomingAttacks[attackType])
			attack.setPaused(true);
...
HYRULE.AttackManager.prototype.unpauseAllPlans = function()
{
	for (let attackType in this.upcomingAttacks)
		for (let attack of this.upcomingAttacks[attackType])
			attack.setPaused(false);
...
HYRULE.AttackManager.prototype.getAttackInPreparation = function(type)
{
 ...
 * for future attacks.
 */
HYRULE.AttackManager.prototype.getEnemyPlayer = function(gameState, attack)
{
	let enemyPlayer;
	...
	if (attack.type === HYRULE.AttackPlan.TYPE_RUSH)
	{
	...
	if (attack.type !== HYRULE.AttackPlan.TYPE_HUGE_ATTACK)
	{
		...
		for (let ourcc of ccEnts.values())
		{
			...
			let access = HYRULE.getLandAccess(gameState, ourcc);
			for (let enemycc of ccEnts.values())
			{
				...
				if (!gameState.isPlayerEnemy(enemycc.owner()))
					continue;
				if (access !== HYRULE.getLandAccess(gameState, enemycc))
					continue;
 ...
 */
HYRULE.AttackManager.prototype.getWonderEnemyPlayer = function(gameState, attack)
{
	let enemyPlayer;
	let enemyWonder;
	let moreAdvanced;
 ...
 */
HYRULE.AttackManager.prototype.getRelicEnemyPlayer = function(gameState, attack)
{
	let enemyPlayer;
...
HYRULE.AttackManager.prototype.cancelAttacksAgainstPlayer = function(gameState, player)
{
	for (let attackType in this.upcomingAttacks)
		for (let attack of this.upcomingAttacks[attackType])
...
HYRULE.AttackManager.prototype.raidTargetEntity = function(gameState, ent)
{
	let data = { "target": ent };
	const attackPlan = new HYRULE.AttackPlan(gameState, this.Config, this.totalNumber, HYRULE.AttackPlan.TYPE_RAID, data);
	if (attackPlan.failed)
		return null;
	if (this.Config.debug > 1)
	...
	this.totalNumber++;
	attackPlan.init(gameState);
	this.upcomingAttacks[HYRULE.AttackPlan.TYPE_RAID].push(attackPlan);
	return attackPlan;
};
 ...
 */
HYRULE.AttackManager.prototype.numAttackingUnitsAround = function(pos, dist)
{
	let num = 0;
	for (let attackType in this.startedAttacks)
		for (let attack of this.startedAttacks[attackType])
 ...
 */
HYRULE.AttackManager.prototype.switchDefenseToAttack = function(gameState, target, data)
{
	...
	const attackType = HYRULE.AttackPlan.TYPE_DEFAULT;
	let attackPlan = new HYRULE.AttackPlan(gameState, this.Config, this.totalNumber, attackType, attackData);
	if (attackPlan.failed)
		return false;
	this.totalNumber++;
	attackPlan.init(gameState);
	...
	let targetAccess = HYRULE.getLandAccess(gameState, target);
	for (let army of gameState.ai.HQ.defenseManager.armies)
	{
		...
		while (army.ownEntities.length > 0)
		{
			...
			let accessOk = unit.getMetadata(PlayerID, "transport") !== undefined ||
			               unit.position() && HYRULE.getLandAccess(gameState, unit) == targetAccess;
			if (unit && accessOk && attackPlan.isAvailableUnit(gameState, unit))
			{
				unit.setMetadata(PlayerID, "plan", attackPlan.name);
				unit.setMetadata(PlayerID, "role", HYRULE.Worker.ROLE_ATTACK);
	...
	attackPlan.state = HYRULE.AttackPlan.STATE_ARRIVED;
	return true;
};

HYRULE.AttackManager.prototype.Serialize = function()
{
	let properties = {
...
HYRULE.AttackManager.prototype.Deserialize = function(gameState, data)
{
	...
	for (let key in data.upcomingAttacks)
	{
		...
		{
			let attack = new HYRULE.AttackPlan(gameState, this.Config, dataAttack.properties.name);
