...
HYRULE.HQ.prototype.gameAnalysis = function(gameState)
{
	...
	this.canExpand = this.Config.difficulty != HYRULE.DIFFICULTY_SANDBOX;
 ...
 */
HYRULE.HQ.prototype.assignStartingEntities = function(gameState)
{
	for (let ent of gameState.getOwnEntities().values())
	{
		...
		if (!pos)
		{
			...
			if (ent.isGarrisonHolder() && ent.garrisoned().length)
				API3.warn("HYRULE warning: support for garrisoned units inside garrisoned holders not yet implemented");
			continue;
		}
 ...
 */
HYRULE.HQ.prototype.regionAnalysis = function(gameState)
{
	...
	{
		API3.warn("HYRULE error: it does not know how to interpret this map");
		return false;
	}
 ...
 */
HYRULE.HQ.prototype.structureAnalysis = function(gameState)
{
 ...
 */
HYRULE.HQ.prototype.buildFirstBase = function(gameState)
{
	...
	if (!total.canAfford(new API3.Resources(template.cost())))
	{
		...
		for (let ent of gameState.getOwnUnits().values())
		{
			...
			if (ent.isIdle())
				HYRULE.gatherTreasure(gameState, ent);
	...
	if (goal == "dock")
	{
		...
		gameState.ai.queues.dock.addPlan(new HYRULE.ConstructionPlan(gameState, "structures/{civ}/dock", { "sea": sea, "proximity": startingPoint[imax].pos }));
	}
	else
		gameState.ai.queues.civilCentre.addPlan(new HYRULE.ConstructionPlan(gameState, "structures/{civ}/civil_centre", { "base": -1, "resource": "wood", "proximity": startingPoint[imax].pos }));
};
 ...
 */
HYRULE.HQ.prototype.dispatchUnits = function(gameState)
{
	...
	if (allycc.length)
	{
		...
		units.filter(API3.Filters.byClasses(["Infantry+Ranged"])).forEach(ent => {
			...
			{
				if (!cc.position() || HYRULE.getLandAccess(gameState, cc) != access)
					continue;
				--num;
				--num1;
 ...
 */
HYRULE.HQ.prototype.configFirstBase = function(gameState)
{
	if (!this.hasPotentialBase())
		return;
	...
	if (!gameState.getOwnEntitiesByClass("DropsiteWood", true).hasEntities())
	{
		...
		if (newDP.quality > 40 && this.canBuild(gameState, newDP.templateName))
		{
			...
			gameState.ai.queues.dropsites.addPlan(new HYRULE.ConstructionPlan(gameState, newDP.templateName, { "base": this.baseManagers()[0].ID }, newDP.pos));
		}
	}
	...
	if (this.needCorral)
	{
		...
		if (!gameState.getOwnEntitiesByClass("Corral", true).hasEntities() && this.canBuild(gameState, template))
			gameState.ai.queues.corral.addPlan(new HYRULE.ConstructionPlan(gameState, template, { "base": this.baseManagers()[0].ID }));
	}
};
