 ...
 * Manage the research
 */
HYRULE.ResearchManager = function(Config)
{
	this.Config = Config;
};
 ...
 */
HYRULE.ResearchManager.prototype.checkPhase = function(gameState, queues)
{
	...
	if (HYRULERequirements && gameState.hasResearchers(nextPhaseName, true))
	{
		...
		queues.majorTech.addPlan(new HYRULE.ResearchPlan(gameState, nextPhaseName, true));
	}
};

HYRULE.ResearchManager.prototype.researchPopulationBonus = function(gameState, queues)
{
	...
	for (let tech of techs)
	{
		...
		if (tech[1]._template.modifications[0].value !== "Population/Bonus")
			continue;
		queues.minorTech.addPlan(new HYRULE.ResearchPlan(gameState, tech[0]));
		break;
	}
};

HYRULE.ResearchManager.prototype.researchTradeBonus = function(gameState, queues)
{
	if (queues.minorTech.hasQueuedUnits())
		return;
...
HYRULE.ResearchManager.prototype.researchWantedTechs = function(gameState, techs)
{
	let phase1 = gameState.currentPhase() === 1;
	let available = phase1 ? gameState.ai.queueManager.getAvailableResources(gameState) : null;
	const numWorkers = phase1 ? gameState.getOwnEntitiesByRole(HYRULE.Worker.ROLE_WORKER, true).length : 0;
	for (let tech of techs)
	{
...
HYRULE.ResearchManager.prototype.researchPreferredTechs = function(gameState, techs)
{
	let phase2 = gameState.currentPhase() === 2;
	let available = phase2 ? gameState.ai.queueManager.getAvailableResources(gameState) : null;
	const numWorkers = phase2 ? gameState.getOwnEntitiesByRole(HYRULE.Worker.ROLE_WORKER, true).length : 0;
	for (let tech of techs)
	{
		if (!tech[1]._template.modifications)
			continue;
...
HYRULE.ResearchManager.prototype.update = function(gameState, queues)
{
	...
	if (techName)
	{
		if (techName.increasePriority)
		{
			...
			let plan = new HYRULE.ResearchPlan(gameState, techName.name);
			plan.queueToReset = "minorTech";
			queues.minorTech.addPlan(plan);
		}
		else
			queues.minorTech.addPlan(new HYRULE.ResearchPlan(gameState, techName.name));
		return;
	}
	...
	queues.minorTech.addPlan(new HYRULE.ResearchPlan(gameState, pickRandom(techs)[0]));
};

HYRULE.ResearchManager.prototype.CostSum = function(cost)
{
	let costSum = 0;
	for (let res in cost)
		costSum += cost[res];
...
HYRULE.ResearchManager.prototype.Serialize = function()
{
	return {};
};

HYRULE.ResearchManager.prototype.Deserialize = function(data)
{
};
