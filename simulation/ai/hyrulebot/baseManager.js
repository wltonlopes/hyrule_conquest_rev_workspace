...
HYRULE.BaseManager = function(gameState, basesManager)
{
	...
	this.neededDefenders = this.Config.difficulty > HYRULE.DIFFICULTY_EASY ? 3 + 2*(this.Config.difficulty - 3) : 0;
...
HYRULE.BaseManager.STATE_WITH_ANCHOR = "anchored";

/**
 * New base with a foundation anchor.
 */
HYRULE.BaseManager.STATE_UNCONSTRUCTED = "unconstructed";

/**
 * Captured base with an anchor.
 */
HYRULE.BaseManager.STATE_CAPTURED = "captured";

/**
 * Anchorless base, currently with dock.
 */
HYRULE.BaseManager.STATE_ANCHORLESS = "anchorless";

HYRULE.BaseManager.prototype.init = function(gameState, state)
{
	if (state === HYRULE.BaseManager.STATE_UNCONSTRUCTED)
		this.constructing = true;
	else if (state !== HYRULE.BaseManager.STATE_CAPTURED)
		this.neededDefenders = 0;
	this.workerObject = new HYRULE.Worker(this);
	// entitycollections
	this.units = gameState.getOwnUnits().filter(API3.Filters.byMetadata(PlayerID, "base", this.ID));
	this.workers = this.units.filter(API3.Filters.byMetadata(PlayerID, "role", HYRULE.Worker.ROLE_WORKER));
...
HYRULE.BaseManager.prototype.reset = function(gameState, state)
{
	if (state === HYRULE.BaseManager.STATE_UNCONSTRUCTED)
		this.constructing = true;
	else
		this.constructing = false;
	if (state !== HYRULE.BaseManager.STATE_CAPTURED || this.Config.difficulty < HYRULE.DIFFICULTY_MEDIUM)
		this.neededDefenders = 0;
	else
		this.neededDefenders = 3 + 2 * (this.Config.difficulty - 3);
};

HYRULE.BaseManager.prototype.assignEntity = function(gameState, ent)
{
...
HYRULE.BaseManager.prototype.setAnchor = function(gameState, anchorEntity)
{
	if (!anchorEntity.hasClass("CivCentre"))
		API3.warn("Error: HYRULE base " + this.ID + " has been assigned " + ent.templateName() + " as anchor.");
	else
	{
	...
	this.accessIndex = HYRULE.getLandAccess(gameState, anchorEntity);
	return true;
};

/* we lost our anchor. Let's reassign our units and buildings */
HYRULE.BaseManager.prototype.anchorLost = function(gameState, ent)
{
	this.anchor = undefined;
	this.anchorId = undefined;
	this.neededDefenders = 0;
...
HYRULE.BaseManager.prototype.setAnchorlessEntity = function(gameState, ent)
{
	if (!this.buildings.hasEntities())
	{
		if (!HYRULE.getBuiltEntity(gameState, ent).resourceDropsiteTypes())
			API3.warn("Error: HYRULE base " + this.ID + " has been assigned " + ent.templateName() + " as origin.");
		this.accessIndex = HYRULE.getLandAccess(gameState, ent);
	}
	else if (this.accessIndex !== HYRULE.getLandAccess(gameState, ent))
		API3.warn(" Error: HYRULE base " + this.ID + " with access " + this.accessIndex +
		          " has been assigned " + ent.templateName() + " with access" + HYRULE.getLandAccess(gameState, ent));
 ...
 */
HYRULE.BaseManager.prototype.assignResourceToDropsite = function(gameState, dropsite)
{
	...
	if (this.ID == this.basesManager.baselessBase().ID)
		accessIndex = HYRULE.getLandAccess(gameState, dropsite);
	...
	for (let type of dropsite.resourceDropsiteTypes())
	{
		...
		resources.forEach(function(supply)
		{
			...
			if (HYRULE.getLandAccess(gameState, supply) != accessIndex)
				return;
...
HYRULE.BaseManager.prototype.removeFromAssignedDropsite = function(ent)
{
...
HYRULE.BaseManager.prototype.removeDropsite = function(gameState, ent)
{
	if (!ent.id())
		return;
 ...
 */
HYRULE.BaseManager.prototype.findBestDropsiteAndLocation = function(gameState, resource)
{
	let bestResult = {
		"quality": 0,
		"pos": [0, 0]
 ...
 */
HYRULE.BaseManager.prototype.findBestDropsiteLocation = function(gameState, resource, templateName)
{
	...
	let obstructions = HYRULE.createObstructionMap(gameState, this.accessIndex, template);
	...
	for (const foundation of gameState.getOwnFoundations().toEntityArray())
		if (HYRULE.getBuiltEntity(gameState, foundation).isResourceDropsite(resource))
...
HYRULE.BaseManager.prototype.getResourceLevel = function(gameState, type, distances = ["nearby", "medium", "faraway"])
{
	let count = 0;
	let check = {};
...
HYRULE.BaseManager.prototype.checkResourceLevels = function(gameState, queues)
{
		...
		if (type == "food")
		{
			...
			if (gameState.ai.HQ.canBuild(gameState, "structures/{civ}/field"))	// let's see if we need to add new farms.
			{
				...
				if (numFarms + numQueue == 0)	// starting game, rely on fruits as long as we have enough of them
				{
					if (count < 600)
					{
						queues.field.addPlan(new HYRULE.ConstructionPlan(gameState, "structures/{civ}/field", { "favoredBase": this.ID }));
				...
				else if (!gameState.ai.HQ.maxFields || numFarms + numQueue < gameState.ai.HQ.maxFields)
				{
					...
					queues.corral.addPlan(new HYRULE.ConstructionPlan(gameState, "structures/{civ}/corral", { "favoredBase": this.ID }));
				continue;
			}
		...
		if (total > 150 || total > 60 && type != "wood")
		{
			...
			if (ratio > 0.15)
			{
				...
				if (newDP.quality > 50 && gameState.ai.HQ.canBuild(gameState, newDP.templateName))
					queues.dropsites.addPlan(new HYRULE.ConstructionPlan(gameState, newDP.templateName, { "base": this.ID, "type": type }, newDP.pos));
...
HYRULE.BaseManager.prototype.addGatherRates = function(gameState, currentRates)
{
	for (let res in currentRates)
	{
		...
		if (res == "food")
		{
			this.workersBySubrole(gameState, HYRULE.Worker.SUBROLE_HUNTER).forEach(ent => {
				if (ent.isIdle() || !ent.position())
					return;
				let gRate = ent.currentGatherRate();
				if (gRate)
			...
			this.workersBySubrole(gameState, HYRULE.Worker.SUBROLE_FISHER).forEach(ent => {
				if (ent.isIdle() || !ent.position())
					return;
				let gRate = ent.currentGatherRate();
				if (gRate)
...
HYRULE.BaseManager.prototype.assignRolelessUnits = function(gameState, roleless)
{
	if (!roleless)
 ...
 */
HYRULE.BaseManager.prototype.setWorkersIdleByPriority = function(gameState)
{
 ...
 */
HYRULE.BaseManager.prototype.switchGatherer = function(gameState, from, to, number)
{
	let num = number;
	let only;
...
HYRULE.BaseManager.prototype.reassignIdleWorkers = function(gameState, idleWorkers)
{
	// Search for idle workers, and tell them to gather resources based on demand
	if (!idleWorkers)
	{
		const filter = API3.Filters.byMetadata(PlayerID, "subrole", HYRULE.Worker.SUBROLE_IDLE);
	...
	for (let ent of idleWorkers)
	{
			...
			else if (ent.isGatherer())
			{
				...
				for (let needed of mostNeeded)
				{
					...
					if (needed.type != "food" && this.basesManager.isResourceExhausted(needed.type))
						continue;
					ent.setMetadata(PlayerID, "subrole", HYRULE.Worker.SUBROLE_GATHERER);
		...
		else if (HYRULE.isFastMoving(ent) && ent.canGather("food") && ent.canAttackClass("Animal"))
			ent.setMetadata(PlayerID, "subrole", HYRULE.Worker.SUBROLE_HUNTER);
		else if (ent.hasClass("FishingBoat"))
			ent.setMetadata(PlayerID, "subrole", HYRULE.Worker.SUBROLE_FISHER);
	}
};

HYRULE.BaseManager.prototype.workersBySubrole = function(gameState, subrole)
{
...
HYRULE.BaseManager.prototype.gatherersByType = function(gameState, type)
{
	return gameState.updatingCollection("workers-gathering-" + type +"-base-" + this.ID, API3.Filters.byMetadata(PlayerID, "gather-type", type), this.workersBySubrole(gameState, HYRULE.Worker.SUBROLE_GATHERER));
};
 ...
 */
HYRULE.BaseManager.prototype.pickBuilders = function(gameState, workers, number)
{
	...
	availableWorkers.sort((a, b) => {
		let vala = 0;
		let valb = 0;
		if (a.getMetadata(PlayerID, "subrole") === HYRULE.Worker.SUBROLE_BUILDER)
			vala = 100;
		if (b.getMetadata(PlayerID, "subrole") === HYRULE.Worker.SUBROLE_BUILDER)
			valb = 100;
		if (a.getMetadata(PlayerID, "subrole") === HYRULE.Worker.SUBROLE_IDLE)
			vala = -50;
		if (b.getMetadata(PlayerID, "subrole") === HYRULE.Worker.SUBROLE_IDLE)
			valb = -50;
	...
	for (let i = 0; i < needed; ++i)
	{
		...
		availableWorkers[i].setMetadata(PlayerID, "subrole", HYRULE.Worker.SUBROLE_IDLE);
 ...
 */
HYRULE.BaseManager.prototype.assignToFoundations = function(gameState, noRepair)
{
	...
	const builderWorkers = this.workersBySubrole(gameState, HYRULE.Worker.SUBROLE_BUILDER);
	...
	if (workers.length < 3)
	{
		...
		if (fromOtherBase)
		{
			...
			fromOtherBase.forEach(worker => {
				...
				worker.setMetadata(PlayerID, "subrole", HYRULE.Worker.SUBROLE_BUILDER);
	...
	for (let target of foundations.values())
	{
		...
		if (HYRULE.isNotWorthBuilding(gameState, target))
			continue;
		...
		let nonBuilderWorkers = workers.filter(function(ent) {
			if (ent.getMetadata(PlayerID, "subrole") === HYRULE.Worker.SUBROLE_BUILDER)
				return false;
			if (!ent.position())
				return false;
			if (ent.getMetadata(PlayerID, "plan") == -2 || ent.getMetadata(PlayerID, "plan") == -3)
		...
		while (assigned < targetNB && builderTot < maxTotalBuilders && current < nonBuilderTot)
		{
			...
			ent.stopMoving();
			ent.setMetadata(PlayerID, "subrole", HYRULE.Worker.SUBROLE_BUILDER);
			ent.setMetadata(PlayerID, "target-foundation", target.id());
		}
	}
...
HYRULE.BaseManager.prototype.update = function(gameState, queues, events)
{
	if (this.ID == this.basesManager.baselessBase().ID)
	{
		...
		if (this.basesManager.hasActiveBase())
		{
			...
			{
				let bestBase = HYRULE.getBestBase(gameState, ent);
			...
			for (let ent of this.buildings.values())
			{
				...
				if (!bestBase)
				{
					if (ent.hasClass("Dock"))
						API3.warn("HYRULE: dock in 'noBase' baseManager. It may be useful to do an anchorless base for " + ent.templateName());
					continue;
				}
	...
	if (!this.anchor)   // This anchor has been destroyed, but the base may still be usable
	{
		if (!this.buildings.hasEntities())
		{
			...
			for (let ent of this.units.values())
			{
				let base = HYRULE.getBestBase(gameState, ent, false, this.ID);
		...
		for (let ent of this.buildings.values())
		{
			if (!ent.position())
				continue;
			let base = HYRULE.getBestBase(gameState, ent);
			if (base.anchor)
				reassignedBase = base;
			break;
		}
...
HYRULE.BaseManager.prototype.AddTCGatherer = function(supplyID)
{
	return this.basesManager.AddTCGatherer(supplyID);
};

HYRULE.BaseManager.prototype.RemoveTCGatherer = function(supplyID)
{
	this.basesManager.RemoveTCGatherer(supplyID);
};

HYRULE.BaseManager.prototype.GetTCGatherer = function(supplyID)
{
	return this.basesManager.GetTCGatherer(supplyID);
};

HYRULE.BaseManager.prototype.Serialize = function()
{
	return {
		"ID": this.ID,
...
HYRULE.BaseManager.prototype.Deserialize = function(gameState, data)
{
	for (let key in data)
		this[key] = data[key];
