...
HYRULE.QueueManager = function(Config, queues)
{
	this.Config = Config;
	this.queues = queues;
	this.priorities = {};
...
HYRULE.QueueManager.prototype.getAvailableResources = function(gameState)
{
...
HYRULE.QueueManager.prototype.getTotalAccountedResources = function()
{
...
HYRULE.QueueManager.prototype.currentNeeds = function(gameState)
{
...
// TODO: many things.
HYRULE.QueueManager.prototype.wantedGatherRates = function(gameState)
{
...
HYRULE.QueueManager.prototype.printQueues = function(gameState)
{
	let numWorkers = 0;
...
HYRULE.QueueManager.prototype.clear = function()
{
	for (let i in this.queues)
		this.queues[i].empty();
};
 ...
 */
HYRULE.QueueManager.prototype.setAccounts = function(gameState, cost, i)
{
 ...
 */
HYRULE.QueueManager.prototype.transferAccounts = function(cost, i, j)
{
	for (let res of Resources.GetCodes())
	{
 ...
 */
HYRULE.QueueManager.prototype.distributeResources = function(gameState)
{
	...
	for (let res of Resources.GetCodes())
	{
		...
		if (available < 0)
			API3.warn("HYRULE: problem with remaining " + res + " in queueManager " + available);
	}
};

HYRULE.QueueManager.prototype.switchResource = function(gameState, res)
{
...
HYRULE.QueueManager.prototype.startNextItems = function(gameState)
{
	for (let q of this.queueArrays)
	{
		let name = q[0];
...
HYRULE.QueueManager.prototype.update = function(gameState)
{
...
HYRULE.QueueManager.prototype.checkPausedQueues = function(gameState)
{
	const numWorkers = gameState.countOwnEntitiesAndQueuedWithRole(HYRULE.Worker.ROLE_WORKER);
...
HYRULE.QueueManager.prototype.canAfford = function(queue, cost)
{
...
HYRULE.QueueManager.prototype.pauseQueue = function(queue, scrapAccounts)
{
...
HYRULE.QueueManager.prototype.unpauseQueue = function(queue)
{
	if (this.queues[queue])
		this.queues[queue].paused = false;
};

HYRULE.QueueManager.prototype.pauseAll = function(scrapAccounts, but)
{
	for (let q in this.queues)
	{
		if (q == but)
...
HYRULE.QueueManager.prototype.unpauseAll = function(but)
{
	for (let q in this.queues)
		if (q != but)
...
HYRULE.QueueManager.prototype.addQueue = function(queueName, priority)
{
	if (this.queues[queueName] !== undefined)
		return;

	this.queues[queueName] = new HYRULE.Queue();
...
HYRULE.QueueManager.prototype.removeQueue = function(queueName)
{
	if (this.queues[queueName] === undefined)
		return;
...
HYRULE.QueueManager.prototype.getPriority = function(queueName)
{
	return this.priorities[queueName];
};

HYRULE.QueueManager.prototype.changePriority = function(queueName, newPriority)
{
...
HYRULE.QueueManager.prototype.Serialize = function()
{
	let accounts = {};
	let queues = {};
	for (let q in this.queues)
...
HYRULE.QueueManager.prototype.Deserialize = function(gameState, data)
{
	...
	{
		this.queues[q] = new HYRULE.Queue();
