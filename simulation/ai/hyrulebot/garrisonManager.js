...
HYRULE.GarrisonManager = function(Config)
{
	this.Config = Config;
...
HYRULE.GarrisonManager.TYPE_FORCE = "force";
HYRULE.GarrisonManager.TYPE_TRADE = "trade";
HYRULE.GarrisonManager.TYPE_PROTECTION = "protection";
HYRULE.GarrisonManager.TYPE_DECAY = "decay";
HYRULE.GarrisonManager.TYPE_EMERGENCY = "emergency";

HYRULE.GarrisonManager.prototype.update = function(gameState, events)
{
	...
	for (let [id, data] of this.holders.entries())
	{
		...
		for (let j = 0; j < list.length; ++j)
		{
			...
			else
			{
				...
				else
				{
					...
					{
						API3.warn("HYRULE garrison error: unit " + ent.id() + " (" + ent.genericName() +
		...
		if (gameState.ai.elapsedTime - holder.getMetadata(PlayerID, "holderTimeUpdate") > 3)
		{
			...
			for (let ent of gameState.getEnemyEntities().values())
			{
				...
				else if (HYRULE.isSiegeUnit(ent))
				{
	...
	for (let [id, gmin] of this.decayingStructures.entries())
	{
		...
		if (!ent || ent.owner() !== PlayerID)
			...
			gameState.ai.HQ.defenseManager.garrisonUnitsInside(gameState, ent, { "min": gmin, "type": HYRULE.GarrisonManager.TYPE_DECAY });
	}
};

/** TODO should add the units garrisoned inside garrisoned units */
HYRULE.GarrisonManager.prototype.numberOfGarrisonedUnits = function(holder)
{
...
/** TODO should add the units garrisoned inside garrisoned units */
HYRULE.GarrisonManager.prototype.numberOfGarrisonedSlots = function(holder)
{
	if (!this.holders.has(holder.id()))
...
HYRULE.GarrisonManager.prototype.allowMelee = function(holder)
{
	if (!this.holders.has(holder.id()))
		return undefined;
...
HYRULE.GarrisonManager.prototype.garrison = function(gameState, ent, holder, type)
{
	...
	ent.setMetadata(PlayerID, "subrole", HYRULE.Worker.SUBROLE_GARRISONING);
 ...
 */
HYRULE.GarrisonManager.prototype.leaveGarrison = function(ent)
{
...
HYRULE.GarrisonManager.prototype.cancelGarrison = function(ent)
{
	ent.stopMoving();
	this.leaveGarrison(ent);
...
HYRULE.GarrisonManager.prototype.keepGarrisoned = function(ent, holder, around)
{
	switch (ent.getMetadata(PlayerID, "garrisonType"))
	{
	case HYRULE.GarrisonManager.TYPE_FORCE:           // force the ungarrisoning
		return false;
	case HYRULE.GarrisonManager.TYPE_TRADE:		// trader garrisoned in ship
		return true;
	case HYRULE.GarrisonManager.TYPE_PROTECTION:	// hurt unit for healing or infantry for defense
		...
		if (ent.attackTypes() && ent.attackTypes().indexOf("Melee") !== -1)
			return false;
		if (around.unit)
			return ent.hasClass("Support") || HYRULE.isSiegeUnit(ent);	// only ranged siege here and below as melee siege already released above
		if (HYRULE.isSiegeUnit(ent))
			return around.meleeSiege;
		return holder.buffHeal() && ent.needsHeal();
	case HYRULE.GarrisonManager.TYPE_DECAY:
		return ent.captureStrength() && this.decayingStructures.has(holder.id());
	case HYRULE.GarrisonManager.TYPE_EMERGENCY: // f.e. hero in regicide mode
		if (holder.buffHeal() && ent.isHealable() && ent.healthLevel() < this.Config.garrisonHealthLevel.high)
			return true;
	...
	default:
		...
		ent.setMetadata(PlayerID, "garrisonType", HYRULE.GarrisonManager.TYPE_PROTECTION);
		return true;
	}
};

/** Add this holder in the list managed by the garrisonManager */
HYRULE.GarrisonManager.prototype.registerHolder = function(gameState, holder)
{
 ...
 * in which case we also do it for structures useful for unit trainings
 */
HYRULE.GarrisonManager.prototype.addDecayingStructure = function(gameState, entId, justCaptured)
{
	if (this.decayingStructures.has(entId))
		return true;
	let ent = gameState.getEntityById(entId);
	if (!ent || !(ent.hasClasses("Barracks", "Fortress", "Tower", "Arsenal", "Embassy", "Range", "Stable") && justCaptured) && !ent.hasDefensiveFire())
		return false;
...
HYRULE.GarrisonManager.prototype.removeDecayingStructure = function(entId)
{
	if (!this.decayingStructures.has(entId))
		return;
...
HYRULE.GarrisonManager.prototype.Serialize = function()
{
	return { "holders": this.holders, "decayingStructures": this.decayingStructures };
};

HYRULE.GarrisonManager.prototype.Deserialize = function(data)
{
	for (let key in data)
		this[key] = data[key];
};
