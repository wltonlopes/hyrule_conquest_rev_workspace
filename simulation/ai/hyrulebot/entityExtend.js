...
HYRULE.isFastMoving = function(ent)
{
...
HYRULE.getMaxStrength = function(ent, debugLevel, DamageTypeImportance, againstClass)
{
	let strength = 0;
	...
	for (let type of attackTypes)
	{
		...
		for (let str in attackStrength)
		{
			...
			if (DamageTypeImportance[str])
				...
				API3.warn("HYRULE: " + str + " unknown attackStrength in getMaxStrength (please add " + str + "  to config.js).");
		}
		...
		for (let str in attackTimes)
		{
			...
			case "prepare":
				...
				break;
			default:
				API3.warn("HYRULE: " + str + " unknown attackTimes in getMaxStrength");
			}
		}
	}
	...
	if (resistanceStrength.Damage)
		for (let str in resistanceStrength.Damage)
		{
			...
			if (DamageTypeImportance[str])
				...
				API3.warn("HYRULE: " + str + " unknown resistanceStrength in getMaxStrength (please add " + str + "  to config.js).");
		}
...
HYRULE.getLandAccess = function(gameState, ent)
{
	if (ent.hasClass("Unit"))
	{
		...
		if (!pos)
		{
			let holder = HYRULE.getHolder(gameState, ent);
			if (holder)
				return HYRULE.getLandAccess(gameState, holder);

			API3.warn("HYRULE error: entity without position, but not garrisoned");
...
HYRULE.getSeaAccess = function(gameState, ent)
{
...
HYRULE.setSeaAccess = function(gameState, ent)
{
	HYRULE.getSeaAccess(gameState, ent);
};

/** Decide if we should try to capture (returns true) or destroy (return false) */
HYRULE.allowCapture = function(gameState, ent, target)
{
	...
	{
		capture = ent.captureStrength() * HYRULE.getAttackBonus(ent, target, "Capture");
	...
	else
	{
		...
		{
			capturable.strength += ent.captureStrength() * HYRULE.getAttackBonus(ent, target, "Capture");
...
HYRULE.getAttackBonus = function(ent, target, type)
{
	let attackBonus = 1;
...
HYRULE.returnResources = function(gameState, ent)
{
	...
	for (let dropsite of dropsiteCollection.values())
	{
		...
		if (owner !== PlayerID && (!dropsite.isSharedDropsite() || !gameState.isPlayerMutualAlly(owner)))
			continue;
		if (HYRULE.getLandAccess(gameState, dropsite) != access)
			continue;
...
HYRULE.IsSupplyFull = function(gameState, ent)
{
 ...
 */
HYRULE.getBestBase = function(gameState, ent, onlyConstructedBase = false, exclude = false)
{
	let pos = ent.position();
	let accessIndex;
	if (!pos)
	{
		...
		pos = holder.position();
		accessIndex = HYRULE.getLandAccess(gameState, holder);
	}
	else
		accessIndex = HYRULE.getLandAccess(gameState, ent);
...
HYRULE.getHolder = function(gameState, ent)
{
...
HYRULE.getBuiltEntity = function(gameState, ent)
{
 ...
 */
HYRULE.isNotWorthBuilding = function(gameState, ent)
{
 ...
 */
HYRULE.isLineInsideEnemyTerritory = function(gameState, pos1, pos2, step=70)
{
...
HYRULE.gatherTreasure = function(gameState, ent, water = false)
{
	...
	let treasureFound;
	let distmin = Math.min();
	let access = water ? HYRULE.getSeaAccess(gameState, ent) : HYRULE.getLandAccess(gameState, ent);
	for (let treasure of gameState.ai.HQ.treasures.values())
	{
		...
		if (lastGathered && gameState.ai.elapsedTime - lastGathered < 20)
			continue;
		if (!water && access != HYRULE.getLandAccess(gameState, treasure))
			continue;
		if (water && access != HYRULE.getSeaAccess(gameState, treasure))
			continue;
...
HYRULE.dumpEntity = function(ent)
{
	if (!ent)
		return;
